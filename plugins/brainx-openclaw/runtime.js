...
export const BRAINX_OPENCLAW_TOOLS = Object.freeze([
  ...
  { name: 'brainx_openmai_search', purpose: () => 'candidate_review', parameters: object({ job_id: string(), criteria: string({ maxLength: 2000 }), continue_search: boolean() }, ['job_id']), description: '为当前顾问有权查看的职位启动 OpenMai 搜索；criteria 是可选补充条件。项目群点击继续找人时仅首次调用传 continue_search=true，后续轮询必须传 false 或省略，后端会自动排除历史 TTC 候选编号。' },
  { name: 'brainx_supermai_scout', purpose: () => 'candidate_review', parameters: object({ job_id: string(), criteria: string({ maxLength: 2000 }), continue_search: boolean() }), description: 'SuperMai 在猎聘、脉脉等渠道按判据找人。项目群入口传 job_id；点击继续找人时仅首次调用传 continue_search=true，后续轮询必须传 false 或省略，后端会自动排除历史 TTC 候选编号。无职位自由找人必须传 criteria。' },
  ...
  { name: 'brainx_pending_judgments', purpose: () => 'job_fact_review', parameters: object({ limit: integer(1, 20) }), description: '在私聊中列出当前顾问所在已登记群的待确认业务判断（客户偏好/硬性要求/否决原因等，specs/016）。' },
  { name: 'brainx_review_judgment', purpose: () => 'job_fact_review', parameters: object({ draft_id: string(), action: string({ enum: ['confirm', 'reject'] }), job_id: string(), confirm: boolean() }, ['draft_id', 'action', 'confirm']), description: '在私聊中经用户明确确认后确认或拒绝一条本人可见的业务判断草稿；确认时可传 job_id 关联职位。' },
  ...
  description: '经用户确认后把授权候选人加入项目、发送 TTC 人才链接、管理共享重点名单、创建候选人决策群，并记录后续阶段。' },
