...
`WorkspaceShell` 是第一阶段主页面结构审核组件：主导航只表达四项日常工作，设置中心位于侧栏底部，顶部不显示同步状态或提醒，BrainX 助手按需展开。该组件当前只进入 Storybook，用户确认前不替换正式工作台外壳。

`SettingsCenterReview` 是第二阶段设置中心审核组件：按个人资料、数据连接、推荐策略和同步诊断组织现有能力，只使用个人资料、飞书授权、TTC 连接、人才库健康、工作台策略版本和 TTC 字段报告的真实接口形状。没有明确后端契约的团队角色管理不进入审核稿，推荐六维配置也留给独立阶段审核；Storybook 使用脱敏示例，不请求真实接口。
