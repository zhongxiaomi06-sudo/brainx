...
export function CommitmentLoopPanel({job,state,outcomes,mode,legal,onCommand,onUpdated,notify}:{job:JobRef;state:EngagementState;outcomes:Outcome[];mode:Mode;legal:EngagementCommand[];onCommand:(command:EngagementCommand)=>void;onUpdated:()=>Promise<void>;notify:(text:string)=>void}){
 ...
 const timeline=[...snapshot.actionHistory.map(a=>({id:a.actionId,title:a.status==="DONE"?"行动完成":"行动取消",detail:a.completionNote||a.title,at:a.completedAt||a.createdAt})),...outcomes.map(o=>({id:o.id,title:o.stage,detail:o.note||"结果已记录",at:o.at}))].sort((a,b)=>String(b.at).localeCompare(String(a.at)));

 return <div className="commitment-loop">
  {mode==="offline"&&<p className="commitment-local-note">演示数据 · 仅保存在当前浏览器</p>}
  {displayState!=="ACCEPTED"&&displayState!=="COMPLETED"?<section className="commitment-onboarding"><span>{stateLabel[displayState]}</span><h2>先确认本轮承接，再开始行动</h2><p>接单会同时固定目标、第一条行动和截止时间；规则建议只有在你确认后才成为事实。</p><div className="commitment-command-row">{legal.includes("ACCEPT")&&<button className="primary" onClick={()=>setEditor("accept")}>接单并设定行动</button>}{legal.filter(a=>["WATCH","UNWATCH","DISMISS"].includes(a)).map(a=><button key={a} onClick={()=>onCommand(a)}>{a==="WATCH"?"加入关注":a==="UNWATCH"?"取消关注":"暂不考虑"}</button>)}</div></section>:null}
  ...
  {displayState==="COMPLETED"&&<section className="commitment-completed"><span>已完成</span><h2>{snapshot.terminalResultMissing?"终局结果待补录":"承接已形成闭环"}</h2><p>{snapshot.terminalResultMissing?"保留原完成时间，只追加一条入职或关闭结果。":"结果已归档，将作为下一轮判断依据。"}</p>{snapshot.terminalResultMissing&&<button className="primary" onClick={()=>setEditor("terminal")}>补录终局结果</button>}</section>}

  {editor&&<section className="commitment-editor">
   <div className="commitment-editor-head"><div><span>{editor==="accept"?"建立承接":editor==="progress"?"回写进展":editor==="blocked"?"记录阻塞":editor==="terminal"?"终局收口":"释放承接"}</span><h2>{editor==="progress"||editor==="blocked"?step===1?"1 · 填写本次结果":"2 · 确认下一行动":"确认后写入行动账本"}</h2></div><button aria-label="关闭编辑" onClick={()=>{setEditor(null);setError("")}}>×</button></div>
   ...
   {(editor==="progress"||editor==="blocked")&&step===2&&<div className="commitment-fields"><p className="rule-draft">规则草案 · 可修改，确认后才成为事实</p><label>下一行动<input value={nextTitle} onChange={e=>{setNextTitle(e.target.value);setNextSource("MANUAL")}}/></label><label>截止时间<input type="datetime-local" value={nextDue} onChange={e=>setNextDue(e.target.value)}/></label>{deadlineButtons(setNextDue)}<div className="commitment-editor-actions"><button onClick={()=>setStep(1)}>返回修改结果</button><button className="primary" disabled={busy} onClick={submitProgress}>{busy?"提交中…":"确认结果与下一行动"}</button></div></div>}
  ...
  <section className="commitment-timeline"><div><span>行动与结果</span><h2>统一时间线</h2></div>{timeline.length?timeline.map(item=><article key={`${item.id}-${item.at}`}><i/><div><b>{item.title}</b><p>{item.detail}</p></div><time>{formatAt(item.at)}</time></article>):<p className="muted">接单后，行动、进展和结果会按时间归档。</p>}</section>
  <p className="commitment-cycle-note">结果提交后会纳入下一轮判断依据；只有真正重新计算后，界面才会显示评分变化。</p>
 </div>
}
