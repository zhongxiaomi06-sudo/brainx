...
- **卡片改版**（`openmai-delivery.js`）：候选人行由两列改三列——名字按钮（2/7）｜匹配与背景（4/7）｜**TTC 人才库链接**（1/7，`multi_url` 纯跳转、不带回调 value）。纯跳转不经过授权判定，**非项目成员的群成员也能打开 TTC 人才页**——正是 mia 在 linda 群里当前唯一可用且合理的动作。编号无效候选人不出链接。说明 note 同步为「点名字加入重点名单并收到人才卡；右侧 TTC 直达人才库链接」。
- 验证：`openmai-delivery` 17/17（三列布局、链接 URL、无回调 value、表头三格）；渲染基线重建并人眼复核（TTC 按钮不截断、斑马纹正常）。
