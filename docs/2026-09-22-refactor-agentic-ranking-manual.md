...
> 日期：2026-09-23；版本：施工设计 v1；状态：施工中，阶段 00 部分完成，阶段 01 已通过完整门禁，阶段 02 已完成持久化失败关闭与容量盘点，清理路径隔离及只读 dry-run 待完整门禁。
...
- [ ] 实现保留策略的 dry-run、分批归档/删除、停止开关、审计与失败恢复；先验证恢复再执行真实清理。
  - 进度：已停止推荐事务隐式裁剪并禁用旧 apply；`retention-plan-v1` 仅聚合推荐与节流候选，保护五类引用，缺最新 schema 即阻断。分批归档/删除、停止开关、删除审计与恢复仍未实现，因此本项保持未完成。
