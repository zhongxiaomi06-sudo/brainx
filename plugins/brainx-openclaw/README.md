...
候选表格的姓名直接打开 TTC 人才详情，右侧保留“初筛通过”和“加入 reloop”。“初筛通过”通过 `KEEP_FOR_REVIEW` 写入项目共享重点名单，并在来源项目群幂等发送一条 TTC 人才链接，由飞书的平台能力自动展开；BrainX 不再自行拼装第二张人才卡。后续项目群问答从 `brainx_candidate_shortlist.focused_candidates` 读取，不依赖易丢失的模型会话记忆。明确取消时使用 `REMOVE_FROM_REVIEW`。
