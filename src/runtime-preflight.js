...
export function validateRuntimeConfig({ agent = {}, worker = {}, openclaw = {} }) {
  ...
  requireKeys(agent, [
    ...
    'BRAINX_DB', 'BRAINX_TALENT_BACKEND', 'BRAINX_MYSQL_HOST', 'BRAINX_MYSQL_DATABASE', 'BRAINX_MYSQL_USER',
  ...
  requireKeys(worker, [
    ...
    'BRAINX_TALENT_BACKEND', 'BRAINX_MYSQL_HOST', 'BRAINX_MYSQL_DATABASE',
    'BRAINX_MYSQL_USER', 'BRAINX_MYSQL_PASSWORD',
    'BRAINX_MYSQL_SSL',
  ], 'worker.env', errors);
  for (const [file, source] of [['agent.env', agent], ['worker.env', worker]]) {
    if (source.BRAINX_TALENT_BACKEND && source.BRAINX_TALENT_BACKEND !== 'mysql') {
      errors.push(`${file}:BRAINX_TALENT_BACKEND:NOT_MYSQL`);
    }
  }
