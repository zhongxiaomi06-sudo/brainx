...
export default function DecisionWorkbench(){
 ...
 const undoFeedback=async(job:DecisionJob)=>{setInlineFeedbackJobId(current=>current===job.id?null:current);if(brainxMode!=="connected")return;try{await undoRecommendationFeedback(job.id)}catch(error){notify(`撤销失败：${error instanceof Error?error.message:"后端未响应"}`,undefined,4000)}};
 const feedbackJob=async(job:DecisionJob,reason?:string)=>{if(!reason){setInlineFeedbackJobId(job.id);return}const clean=reason.trim().slice(0,200);setInlineFeedbackJobId(job.id);try{if(brainxMode==="connected"){await sendRecommendationFeedback(job.id,clean,brainxRun.snapshotId,makeIdempotencyKey(`recommendation-feedback-reason:${job.id}:${clean}`))}}catch(error){setInlineFeedbackJobId(current=>current===job.id?null:current);notify(`反馈失败：${error instanceof Error?error.message:"后端未响应"}`)}};
