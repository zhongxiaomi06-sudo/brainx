...
> 2026-09-24 更正：本文的 SuperMai “与 OpenMai 共用引擎”是 9 月 12 日的历史行为，现已被真实桌面 relay 取代，不得再用作当前产品口径。当前验收见 [SuperMai 桌面 Relay 运行手册](2026-09-24-supermai-desktop-relay-runbook.md)。
...
| SuperMai 服务端链路 | 历史通过／现行已废弃 | 当时实际是 OpenMai criteria 模式，不是 SuperMai；2026-09-24 起必须经顾问桌面 SuperMai/Sourcing 执行 |
...
4. **SuperMai 通道**：先在 BrainX 连接中心配对本人电脑并在 SuperMai 官方页面登录平台；群内点「SuperMai 找人」后，任务由本人桌面执行并回到原群。不再使用 OpenMai 结果代替。
