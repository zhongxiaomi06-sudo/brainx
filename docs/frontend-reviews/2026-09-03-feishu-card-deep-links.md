...
- 后端依赖：生产 `BRAINX_BASE_URL` 必须是 HTTPS；本机真机联调可显式设置
  `BRAINX_ALLOW_HTTP_LOOPBACK=1`，且只放行 `127.0.0.1`/`localhost`。工作台 API 继续验证
  HttpOnly 飞书登录会话和对象可见性。
...
- 用户要求先在本人电脑和飞书账号跑通项目群，再上传服务器。
- 本机卡片允许指向显式开启的回环 HTTP 地址；局域网 IP、普通 HTTP 域名和带账号密码的 URL
  仍失败关闭。该开关只服务同一台电脑的飞书桌面端验收，不代表手机或异机可访问。
