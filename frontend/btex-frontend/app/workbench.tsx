...
type SavedWorkbenchState = Partial<{done:number[];snoozed:number[];extraTasks:string[];weights:number[];decisionActions:string[];tray:string[];folders:PickFolder[];folderMode:boolean;engagement:Record<string,EngagementState>;events:Record<string,DecisionEvent[]>;outcomes:Record<string,Outcome[]>;sync:SyncStatus;auth:AuthStatus;notifications:Notification[]}>;
...
export default function DecisionWorkbench(){
 ...
 useEffect(()=>{if(!hydrated||brainxMode==="connected")return;localStorage.setItem("decision-workbench",JSON.stringify({done,snoozed,extraTasks,weights,decisionActions,tray,folders,folderMode,engagement,events:decisionEvents,outcomes,sync,auth,notifications}))},[hydrated,brainxMode,done,snoozed,extraTasks,weights,decisionActions,tray,folders,folderMode,engagement,decisionEvents,outcomes,sync,auth,notifications]);
...
function EngagementPanel({job,state,outcomes,mode,legalMap,onCommand,onUpdated,notify}:{job:DecisionJob;state:EngagementState;outcomes:Outcome[];mode:"connecting"|"connected"|"offline";legalMap:Record<string,EngagementCommand[]>;onCommand:(job:DecisionJob,command:EngagementCommand)=>void;onUpdated:()=>Promise<void>;notify:(text:string)=>void}){
 const actions=mode==="connected"?legalMap[job.id]||[]:legalActions(job,state).filter(action=>action!=="COMPLETE"&&action!=="RELEASE");
 return <CommitmentLoopPanel job={job} state={state} outcomes={outcomes} mode={mode} legal={actions} onCommand={command=>onCommand(job,command)} onUpdated={onUpdated} notify={notify}/>;
}
