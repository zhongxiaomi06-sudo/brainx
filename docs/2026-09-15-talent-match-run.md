...
- 2026-09-15 本地连接真实 RDS 执行 dry-run：人才池 401 人、活跃职位 91 个；仅 36 个职位有
  阈值以上候选，55 个职位为零，总命中 147。目标样本 `JHTSEQJ / 荆华密算 / 销售` 为零命中。
- 数据核查发现 `talent_tag` 当前没有任何标签关联；401 人中有 67 份 `summary` 包含销售、商务、
  市场、营销、公关或运营文字，但算法的技能与意向标签维度合计占 80%，摘要兜底仅占 20%，
  导致真实数据覆盖不足。修复标签映射或匹配口径并复验前，不得执行生产 `--write`，也不得把
  “模块测试通过”标记为真实短名单链路已打通。
