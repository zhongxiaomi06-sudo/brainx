...
export const BRAINX_OPENCLAW_TOOLS = Object.freeze([
  ...
  { name: 'brainx_accept_job', purpose: () => 'job_action', parameters: object({ job_id: string(), goal: string({ maxLength: 240 }), action_title: string({ maxLength: 240 }), due_at: string(), idempotency_key: string(), confirm: boolean() }, ['job_id', 'confirm']), description: '经用户确认后正式接单、建立首个行动并创建或复用项目群；不会自动启动付费找人。只需 job_id 与 confirm=true，目标/首条行动/截止时间/幂等键由服务端自动生成。' },
