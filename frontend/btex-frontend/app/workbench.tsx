...
import { BACKEND_WEIGHTS, FALLBACK_DISMISS_REASONS, brainxFetch, connectSSE, fetchJobDetail, getSnapshot, makeIdempotencyKey, mapReplayData, mapRadarRow, mapClientRow, getRadar, getClients, getTalentSupply, updateOpportunityFacts, updateWorkbenchPreferences, sendRecommendationFeedback, undoRecommendationFeedback, streamAssistant, rerunOpenmai, BrainxApiError, type TalentSupplySnapshot, type AssistantMessage, type BackendConsultants, type BackendEngagementResponse, type BackendOutcomeResponse, type BackendProfileUpdate, type BackendRecommendationRun, type BackendReplay, type BackendSessionStatus, type BrainxReplay, type BrainxSnapshot, type ManualFactField, type OpenmaiResult, type RadarJob, type RadarClient } from "./brainx-api";
...
const nav = [
 ["today","工作台",Sparkles],["jobs","职位市场",BriefcaseBusiness],["clients","客户",Users],
 ["accepted","项目管理",ClipboardCheck],["sources","数据源",Database],["rules","设置中心",Settings2],
] as const;
...
// 可拖拽侧边栏（PR#5 评审恢复）：折叠 68px 图标轨 ⇄ 展开 154-336px，拖右边沿或键盘 ←/→ 调整
type SidebarResize = {startX:number;startWidth:number;opensCollapsed:boolean};
const SIDEBAR_MIN_WIDTH=154;
...
export default function DecisionWorkbench(){
 ...
 const [navOpen,setNavOpen]=useState(true); // concept 设计默认展开（带文字标签），可拖回 68px 图标轨
 const [sidebarWidth,setSidebarWidth]=useState(154);
 ...
 const assistantPrefLoaded=useRef(false);
 // 助手抽屉默认开合（PR#5 评审修正）：用户显式选择（localStorage）优先于断点默认值，
 // 不再监听断点 change 强制顶开——用户手动关闭后跨断点不得复活。
 useEffect(()=>{let saved:string|null=null;try{saved=localStorage.getItem("brainx-assistant-open")}catch{}setAssistantOpen(saved!==null?saved==="1":window.matchMedia("(min-width: 1280px)").matches);assistantPrefLoaded.current=true},[]);
 useEffect(()=>{if(!assistantPrefLoaded.current)return;try{localStorage.setItem("brainx-assistant-open",assistantOpen?"1":"0")}catch{}},[assistantOpen]);
 // 侧边栏拖拽（PR#5 评审恢复）：拖右边沿调宽；从折叠态向右拖 >12px 展开，展开态拖到 <MIN-36px 松手折叠
 ...
 // 工作区切换菜单：点击外部关闭（PR#5 评审修正，此前仅 Esc/跳转可关）
 useEffect(()=>{if(!workspaceMenuOpen)return;const closeOnOutside=(event:PointerEvent)=>{if(!(event.target as HTMLElement).closest(".concept-workspace-switcher"))setWorkspaceMenuOpen(false)};document.addEventListener("pointerdown",closeOnOutside);return()=>document.removeEventListener("pointerdown",closeOnOutside)},[workspaceMenuOpen]);
   ...
   <header className="topbar">
    <div className="concept-workspace-switcher">
     ...
     {workspaceMenuOpen&&<div className="concept-workspace-menu" role="menu" aria-label="切换工作区">
      ...
      <button type="button" role="menuitem" className={page==="clients"?"active":""} onClick={()=>go("clients")}><span>客户</span><small>查看客户与人才洞察</small>{page==="clients"&&<Check aria-hidden="true"/>}</button>
     </div>}
    </div>
    {page==="today"?<><button className="btex-person identity-trigger" onClick={()=>openPanel({kind:"identity"})}><span className="reference-avatar">{auth.consultant.slice(0,1)}</span><span><b>{auth.consultant}</b><small>Consultant</small></span></button><button className={`sync sync-trigger ${auth.needsReauth?"auth_expired":sync.state.toLowerCase()}`} onClick={()=>openPanel(auth.needsReauth?{kind:"identity"}:{kind:"sync"})}><i/> {auth.needsReauth?"飞书授权已过期":sync.state==="READY"?`Snapshot #${brainxMode==="connected"&&brainxRun.snapshotId?brainxRun.snapshotId.slice(0,8):"1842"} · ${sync.updatedAt} 已同步`:sync.state==="RUNNING"?"同步中…":sync.state==="INCOMPLETE"?"同步不完整":sync.state==="AUTH_EXPIRED"?"授权过期":sync.state==="ERROR"?"同步失败":"尚未同步"}</button><button className="icon-btn notification-trigger" onClick={()=>openPanel({kind:"notifications"})} aria-label="今日提醒"><BellRing/>{notifications.filter(note=>!note.read).length>0&&<i>{notifications.filter(note=>!note.read).length}</i>}</button></>:<><div className="search"><Search/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="搜索客户、职位、PM…"/></div><button className="top-pill" onClick={()=>notify("全局筛选已展开")}><Filter/> 当前团队 <ChevronRight/></button><button className="icon-btn" onClick={()=>openPanel({kind:"notifications"})} aria-label="通知"><Bell/></button></>}
...
function ChatbotDrawer({messages,input,setInput,busy,onSend,onStop,onClear,onClose,mode,page,settings,setSettings,apiKey,setApiKey,contextJob}:{messages:AssistantMessage[];input:string;setInput:(value:string)=>void;busy:boolean;onSend:()=>void;onStop:()=>void;onClear:()=>void;onClose:()=>void;mode:"connecting"|"connected"|"offline";page:Page;settings:boolean;setSettings:(value:boolean)=>void;apiKey:string;setApiKey:(value:string)=>void;contextJob:DecisionJob|null}){
...
function DecisionToday({activeJobId,completed,jobs,engagement,sync,open,onAction,onCommand,onFeedback,showVerification=true,tray,onToggleTray,onRemoveTray,onConfirmTray,folders,folderMode,onFolderMode,onAssignFolder,onCreateFolder}:{activeJobId:string|null;completed:string[];jobs:DecisionJob[];engagement:Record<string,EngagementState>;sync:SyncStatus;open:(job:DecisionJob,tab?:"judgement"|"engagement"|"trail"|"replay")=>void;onAction:(job:DecisionJob,action:DecisionAction)=>void;onCommand:(job:DecisionJob,command:EngagementCommand)=>void;onFeedback:(job:DecisionJob,reason?:string)=>void;showVerification?:boolean;tray:string[];onToggleTray:(id:string)=>void;onRemoveTray:(id:string)=>void;onConfirmTray:()=>void;folders:PickFolder[];folderMode:boolean;onFolderMode:()=>void;onAssignFolder:(jobId:string,folderId:string)=>void;onCreateFolder:(name:string)=>void}){
 const [query,setQuery]=useState("");
...
function ShowcaseCard({job,selected,open,onToggle,onRemove}:{job:DecisionJob;selected:boolean;open:(job:DecisionJob)=>void;onToggle:()=>void;onRemove:()=>void}){
 const coverage=evidenceCoveragePercent(job.evidenceCoverage);
 return <article className={`showcase-card${selected?" selected":""}`} onClick={()=>open(job)}><button className="showcase-menu" type="button" onClick={event=>{event.stopPropagation();selected?onRemove():onToggle()}} aria-label={selected?`从精选盘移除 ${job.company}`:`收藏 ${job.company}`} title={selected?"移出精选盘":"收藏到精选盘"}>{selected?<Check/>:<Star/>}</button><b>{job.role}</b><span>{job.company}</span><small>{job.recentSignal}</small><div><strong>{job.finalScore}<i> 匹配分</i></strong><em>{coverage===null?"待确认":`证据 ${coverage}%`}</em></div></article>
}
...
function DecisionZone({tone,title,subtitle,jobs,isContext,completed,engagement,open,onAction,onCommand,onFeedback,tray,onToggleTray,folderMode,folders,onAssignFolder,toolbar,anchorId}:{tone:"accepted"|"pending";title:string;subtitle:string;jobs:DecisionJob[];isContext:boolean;completed:string[];engagement:Record<string,EngagementState>;open:(job:DecisionJob,tab?:"judgement"|"engagement"|"trail"|"replay")=>void;onAction:(job:DecisionJob,action:DecisionAction)=>void;onCommand:(job:DecisionJob,command:EngagementCommand)=>void;onFeedback:(job:DecisionJob,reason?:string)=>void;tray:string[];onToggleTray:(id:string)=>void;folderMode:boolean;folders:PickFolder[];onAssignFolder:(jobId:string,folderId:string)=>void;toolbar?:React.ReactNode;anchorId?:string}){return <section id={anchorId} className={`decision-zone ${tone}${isContext?" is-context":""}`}><div className="decision-group-head"><div><h2>{title}<span className="decision-zone-kicker">{tone==="accepted"?"TODAY'S COMMITMENTS":"NOT YET ACCEPTED"}</span></h2></div><span>{isContext?`当前查看 · ${jobs.length}`:`${jobs.length} 个`}</span></div>{toolbar}<div className="pick-grid">{jobs.map((job,index)=><OpportunityRow key={job.id} job={{...job,rank:index+1}} completed={completed} engagement={engagement[job.id]||"NEW"} open={open} onAction={onAction} onCommand={onCommand} onFeedback={onFeedback} inTray={tray.includes(job.id)} onToggleTray={onToggleTray} folderMode={folderMode} folders={folders} onAssignFolder={onAssignFolder}/>)}</div></section>}
