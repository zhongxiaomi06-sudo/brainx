...
项目群还必须遵守反向唯一性：**一个 `project_id` 只能有一个内部项目群**。`project_launches.consultant_id`
只记录首轮寻访发起人，不代表群归该顾问私有；其他有效 `MY_JOB / TEAM_SHARED` 协作者读取并复用同一
`chat_id` 和搜索状态。首轮搜索运行中或已完成时，协作者点击启动不得再次调用 OpenMai；失败重试由原发起人
明确确认，避免多人协作时重复建群、覆盖职位群绑定或产生隐性重复费用。数据库必须阻止新的同职位重复启动记录；
若升级前已有冲突记录，接口失败关闭并要求管理员先核对真实群，不得静默删群或任选一个覆盖事实。
