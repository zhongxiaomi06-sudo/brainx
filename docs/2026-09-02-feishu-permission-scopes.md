...
| 项目启动：拉群承载独立职位 | `im:chat:create`（或 `im:chat` 写权限） | 应用 | 客户端已实现，真实租户仍须审批与验收 |
| 候选人 Offer 报告 | 飞书后台对应的文档创建与编辑权限（Docx v1） | 应用 | 中；2026-09-10 新增，发布前必须真租户核对 scope 名称 |
| 第二版：项目 Sheet 管理列表 | `sheets:spreadsheet`（或 `drive:drive`） | 应用 | 中 |

> 历史结论：9 月 2 日会议曾决定第一版不做拉群与项目 Sheet。2026-09-07 用户已把“从工作台自动创建职位项目群”提升为第一主链当务之急，因此拉群权限改为上线必需；项目 Sheet 仍后置。后续部署不得继续按旧结论省略建群权限。

## 4. 精确 scope 清单（可直接批量导入）

飞书后台「权限管理」页面支持**批量导入 JSON**，比逐个勾选可靠。分应用身份（tenant）与用户身份（user）两组，**不要混在一起申请**。

### 4.1 应用身份（tenant）— 当前第一主链最小集

覆盖：对话交互、自动创建职位项目群、投放职位和候选人/PDF、解析说话人。

若启用 2026-09-10 的 Offer 报告，还须在飞书后台为应用增加 Docx v1 的“创建文档、编辑文档”权限并发布新版本。不同后台版本展示的 scope 名称可能不同，必须以接口权限提示和开放平台页面为准；代码不会在缺权时降级成伪报告。
...
| `im:chat` | 创建和管理职位项目群、获取群信息 | 建群接口被拒，工作台启动链路会在外部写入前后显式失败 |
...
| 权限 / 资源 | 用途 | 找谁 | 状态 |
|---|---|---|---|
| 读群全量消息：`im:message.group_msg`（应用身份）或 `im:message.group_msg:get_as_user`（用户身份） | 群信息提炼、目标检查 | 飞书管理员 / 审批人 | **9/2 晚降级为兜底方案**：York 群主邀请路径已覆盖 80% 场景（见 §6.1），剩 20% 仍需该权限 |
| 拉群：`im:chat:create` 或后台对应的 `im:chat` 写权限 | 职位启动项目群 | 审批人 | **上线必需、仍待真实租户确认**；代码已提供幂等建群，权限未发布时会显式失败 |
| 团队人才库整库共享账号 | 拿到即可省掉数据隔离模块（约两个模块开发量） | 人才库 owner | **申请中，暂未通过** |

### 6.1 「群主邀请机器人入群」低敏感路径（9/2 晚新发现，原 §6 第一条不再二选一）

**原问题**：之前把"读群全量消息"列为高敏感卡点，§1 行 1（职位录入）和 §1 行 5（offer 谈判）都被它阻塞。

**新路径**（9/2 下午 Mia 通过 York 飞书账号实证）：

- **York 是团队长档**，手下一共 **10 个团队成员**（HY/EC/JD/SN/LD/Miya/FLX/WD/Hiroshi/Frankie），DataClaw 已为他跑 8 个自建定时任务
- **York 名下 / 他是群主的所有职位群、offer 群、复盘群**，York 一句话就能把 BrainX 机器人拉进群
- 机器人进群后，**群维度**就可以读消息（不是全局，是这个群的全部消息），**不需要 `im:message.group_msg` 全局高敏感权限**
- 走的是飞书内置逻辑：**群主主动添加机器人 = 群主授权**。低敏感路径。

**这条路径覆盖**：
- §1 行 1 职位信息录入：York 拉机器人进职位群 → 机器人读群消息 → 提炼职位 → 进 `job_facts` ✅
- §1 行 5 offer 谈判：每个 Offer 小群的群主（York / Wendy / Linda 等内部人）拉机器人入群 → 机器人读小群消息 → 跟进进度 ✅（**9/2 实证：现行 7 个 offer 群，群主都是 York 或牵头人**）

**不覆盖**（仍需高敏感权限）：
- York 不是群主的群（约 20%）
- 全团队的群信息标准化（HRBP、跨团队复盘等）
- 目标检查全团队数据

**9/3 第一件事**：跑通「York 群主邀请机器人入群 → 读到群消息 → 提炼职位」端到端 demo，把这条低敏感路径坐实。
...
| 项 | 问谁 | 影响 |
|---|---|---|
| 租户是否允许开 `im:message.group_msg` 系列 | 飞书管理员 | **9/2 晚降级为兜底**：80% 场景走 York 群主邀请路径；剩 20% 才需该权限 |
| 现行 7 个 offer 群的群主是谁、是否愿意拉 BrainX 机器人 | York / Wendy / Linda 等牵头人 | 决定 §1 行 5（offer 谈判）能否完全走低敏感路径 |
