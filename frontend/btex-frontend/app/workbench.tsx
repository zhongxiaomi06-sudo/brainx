...
function DecisionCard({job,completed,engagement,open,onAction,onFeedback,feedbackJobId,onUndoFeedback,inTray,onToggleTray,folderMode,folders,onAssignFolder}:{job:DecisionJob;completed:string[];engagement:EngagementState;open:(job:DecisionJob,tab?:"judgement"|"engagement"|"logs")=>void;onAction:(job:DecisionJob,action:DecisionAction)=>void;onFeedback:(job:DecisionJob,reason?:string)=>void;feedbackJobId:string|null;onUndoFeedback:(job:DecisionJob)=>void;inTray:boolean;onToggleTray:(id:string)=>void;folderMode:boolean;folders:PickFolder[];onAssignFolder:(jobId:string,folderId:string)=>void}){
 ...
 return <article className={`pick-card${inTray?" in-tray":""}${feedbackJobId===job.id?(feedbackSubmitted?" feedback-active":" feedback-selecting"):""}`} onClick={()=>{if(feedbackJobId!==job.id)open(job)}}>
  ...
  <div className="pick-card-foot"><span className="pick-card-actions"><button className={`pick-card-action${actionComplete?" complete":""}`} onClick={e=>{e.stopPropagation();onAction(job,action)}} disabled={actionComplete}>{actionComplete?"已记录":action.label}<ChevronRight/></button></span></div>
...
function WeightChatbot({setWeights,notify}:{setWeights:(weights:number[])=>void;notify:(text:string)=>void}){const [input,setInput]=useState("");const [key,setKey]=useState("");const [reply,setReply]=useState("告诉我你现在更想优先推进什么样的岗位，我会自动配比推荐权重。");const [busy,setBusy]=useState(false);const submit=async()=>{if(!input.trim()||!key.trim()){setReply("请先输入你的偏好和 DeepSeek API Key。Key 只在当前页面使用，不会保存。");return}setBusy(true);try{const response=await fetch("https://api.deepseek.com/chat/completions",{method:"POST",headers:{"Content-Type":"application/json",Authorization:"Bearer "+key.trim()},body:JSON.stringify({model:"deepseek-chat",temperature:0.2,messages:[{role:"system",content:"你是岗位推荐权重助手。根据用户偏好输出严格 JSON，格式为 {\"weights\":[项目推进数字,探索机会数字,个人适配数字],\"reply\":\"一句中文解释\"}。三个数字必须是整数且总和100。"},{role:"user",content:input.trim()}]})});if(!response.ok)throw new Error("DeepSeek 请求失败");const data=await response.json();const content=String(data.choices?.[0]?.message?.content||"").replaceAll("```json","").replaceAll("```","");const parsed=JSON.parse(content);const weights=parsed.weights.map((n:number)=>Math.max(0,Math.round(n)));const total=weights.reduce((a:number,b:number)=>a+b,0)||100;const normalized=weights.map((n:number)=>Math.round(n*100/total));normalized[0]+=100-normalized.reduce((a:number,b:number)=>a+b,0);setWeights(normalized);setReply(parsed.reply||("已根据你的偏好完成配比：项目推进"+normalized[0]+"%，探索机会"+normalized[1]+"%，个人适配"+normalized[2]+"%。"));}catch(error){setReply("暂时无法读取 DeepSeek 结果，请检查 API Key 或网络连接。" );}finally{setBusy(false)}};return <div className="weight-chatbot"><div className="chat-message"><Sparkles/><span>{reply}</span></div><textarea className="field" value={input} onChange={e=>setInput(e.target.value)} placeholder="例如：我想多找新机会，但也要符合我的广告投放经验。"/><div className="chat-actions"><input className="field" type="password" value={key} onChange={e=>setKey(e.target.value)} placeholder="DeepSeek API Key（仅本次使用）"/><button className="btn primary" type="button" onClick={submit} disabled={busy}>{busy?"分析中…":"生成推荐倾向"}</button></div></div>}

function PreferencePresets({setWeights,notify,onCustom,onPreset}:{setWeights:(weights:number[])=>void;notify:(text:string)=>void;onCustom:()=>void;onPreset:()=>void}){const presets=[{label:"冲结果",weights:[60,25,15]},{label:"找增量",weights:[20,60,20]},{label:"发挥优势",weights:[20,30,50]},{label:"均衡推荐",weights:[34,33,33]}];const [selected,setSelected]=useState("均衡推荐");return <div className="preference-presets">{presets.map(p=><button key={p.label} type="button" className={"btn quiet"+(selected===p.label?" active":"")} onClick={()=>{setSelected(p.label);onPreset();setWeights(p.weights);notify("已切换为"+p.label)}}>{p.label}</button>)}<button key="custom" type="button" className={"btn quiet"+(selected==="自定义"?" active":"")} onClick={()=>{setSelected("自定义");onCustom();notify("已打开自定义策略")}}>自定义</button></div>}

function Rules({weights,setWeights,notify,mode,policy,keywords,note,onSaveKeywords}:any){const names=["项目推进","探索机会","个人适配"];const notes=["阶段、动量、转化、HC 空间与竞争度","新鲜度、方向匹配、有效 HC 与低竞争","顾问关系、容量、历史交付与战略方向"];const total=weights.reduce((a:number,b:number)=>a+b,0);const canApply=total===100;const setWeightAt=(index:number,value:number)=>{const next=Math.max(5,Math.min(80,value));const rest=100-next;const otherIndexes=weights.map((_,i)=>i).filter(i=>i!==index);const otherTotal=otherIndexes.reduce((sum,i)=>sum+weights[i],0)||1;const nextWeights=weights.map((weight,i)=>i===index?next:Math.max(5,Math.round(rest*weight/otherTotal)));const correction=100-nextWeights.reduce((sum,weight)=>sum+weight,0);nextWeights[otherIndexes[0]]+=correction;setWeights(nextWeights)};const [keywordDraft,setKeywordDraft]=useState((keywords||[]).join("、"));const [noteDraft,setNoteDraft]=useState(note||"");const [nlDraft,setNlDraft]=useState("");const [nlResult,setNlResult]=useState<string|null>(null);const [customOpen,setCustomOpen]=useState(false);const applyNaturalLanguage=()=>{const text=nlDraft.trim();if(!text){notify("先写一句你的判断偏好");return}const intent={process:0,explore:0,personal:0};if(/推进|在途|收口|交付|项目|阶段|动能|推进力/.test(text))intent.process+=12;if(/探索|新机会|新方向|广度|尝新|验证/.test(text))intent.explore+=12;if(/个人|匹配|擅长|方向|经历|偏好/.test(text))intent.personal+=12;if(/均衡|平衡|默认|恢复/.test(text)){setWeights([60,25,15]);setNlResult("已恢复默认：推进 60% · 探索 25% · 个人 15%");notify("策略已按均衡偏好重置");return}const strength=Math.max(1,intent.process+intent.explore+intent.personal);const w=[60,25,15];const idx={process:0,explore:1,personal:2} as const;(Object.keys(idx) as (keyof typeof idx)[]).forEach(k=>{w[idx[k]]+=Math.round(intent[k]*60/strength)});const w2=w.map(n=>Math.max(5,Math.min(80,n)));const sum=w2.reduce((a,b)=>a+b,0);w2[w2.indexOf(Math.max(...w2))]+=100-sum;setWeights(w2);const matched=["项目推进","探索机会","个人适配"].filter((_,i)=>w2[i]!==[60,25,15][i]);setNlResult(`Agent 已理解：${matched.length?matched.join("、")+" 偏好":"保持默认侧重"} → 推进 ${w2[0]}% · 探索 ${w2[1]}% · 个人 ${w2[2]}%`);notify("已按自然语言生成策略，确认后保存")};return <><Heading code="POLICY / FELIX V1.3" title="判断规则" desc="" action={<div className={`tag ${canApply?"blue":"orange"}`}>当前合计 {total}%{canApply?"":" · 需为 100%"}</div>}/>{mode==="connected"&&<section className="card section"><div className="card-head"><h2>Brain X 后端策略</h2><span>Policy {policy||"—"}</span></div><div className="card-body"><p className="panel-caption">分数与排序由后端六维确定性评分决定（policy_version：{policy||"—"}）；前端只展示结果，不在浏览器内重新计算。</p>{BACKEND_WEIGHTS.map(w=><div className="mini-item" key={w.dim}><div><b>{w.label}</b><p>后端固定权重，不可在前端调整</p></div><strong style={{marginLeft:"auto",color:"var(--blue)"}}>{w.weight}</strong></div>)}<div className="toolbar" style={{marginTop:14}}><input className="field" value={keywordDraft} onChange={e=>setKeywordDraft(e.target.value)} placeholder="画像关键词（顿号分隔，如：增长、投放、广告）"/><input className="field" value={noteDraft} onChange={e=>setNoteDraft(e.target.value)} placeholder="画像备注（可选）"/><button className="btn primary" onClick={()=>onSaveKeywords(keywordDraft.split(/[、,，\s]+/).filter(Boolean),noteDraft)}><Check/>保存画像</button></div></div></section>}<section className="card section nl-rule-card"><div className="card-head"><h2>自然语言描述偏好</h2></div><div className="card-body"><div className="toolbar nl-toolbar"><textarea className="field nl-input" value={nlDraft} onChange={e=>setNlDraft(e.target.value)} placeholder="例：我更看重在途项目的推进，少花时间在探索新机会上"/><button className="btn primary" onClick={applyNaturalLanguage}><Sparkles/>生成策略</button></div>{nlResult&&<p className="nl-result"><Check/>{nlResult}</p>}</div></section><div className="grid g2"><section className="card"><div className="card-head"><h2>岗位推荐偏好</h2><button className="link" onClick={()=>{setWeights([60,25,15]);notify("已恢复默认策略")}}>恢复默认</button></div><div className="card-body strategy-rules"><PreferencePresets setWeights={setWeights} notify={notify} onCustom={()=>setCustomOpen(true)} onPreset={()=>setCustomOpen(false)}/>{customOpen&&<WeightChatbot setWeights={setWeights} notify={notify}/>}{names.map((n,i)=><label className="rule-row" key={n}><span><b>{n}</b><small>{notes[i]}</small></span><input type="range" min="5" max="80" value={weights[i]} onChange={e=>setWeightAt(i,+e.target.value)}/><output>{weights[i]}%</output></label>)}<button className="btn primary" style={{marginTop:16}} disabled={!canApply} onClick={()=>notify("策略已保存；后端将生成新的 policy_version 和推荐快照")}><Check/>保存并生成新推荐</button>{!canApply&&<p className="rule-validation">三层权重总和需为 100%，才能提交给 Agent 重新计算。</p>}<p className="policy-boundary"><ShieldCheck/>不可调整：HC、已入职、职位关闭、项目归属与数据冲突规则。</p></div></section><section className="card"><div className="card-head"><h2>影响预览</h2><span>由 Agent 返回</span></div><div className="card-body"><div className="conclusion"><div className="spark"><Activity/></div><div><b>只预览软排序变化</b><p>正式保存前展示哪些职位上升、下降，以及仍被硬规则拦截的职位。</p></div></div>{names.map((name,index)=><div className="mini-item" key={name}><div><b>{name}</b><p>{index===0?"强化在途项目与真实反馈":index===1?"保留新项目验证空间":"只做个人修正，不覆盖项目事实"}</p></div><strong style={{marginLeft:"auto",color:"var(--blue)"}}>当前 {weights[index]}%</strong></div>)}</div></section></div></>}
