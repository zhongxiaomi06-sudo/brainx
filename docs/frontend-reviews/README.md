...
| `JobDetailCard` 居中职位弹窗 | 已完成 | 退回修改 | 已接入 | 待确认 | 部分验证 | 共享组件与正式今日决策包装层均强制最新三条记录在前；本地机器“已推荐”噪声已从生成链路和存量库清除，只保留真实操作，待用户复看与目标环境复核 |
...
| `TodayDecisionQueue` 今日决策队列 | 已完成 | 部分通过 | 已接入 | 待确认 | 已验证（本地真实库副本） | 正式入口已复用 V2 卡片、20条分页、完整队列搜索和服务端真实排序；默认排序直接使用冻结推荐 rank，修复后的完整交互待用户复看，目标环境 OAuth 验证仍待补齐 |
