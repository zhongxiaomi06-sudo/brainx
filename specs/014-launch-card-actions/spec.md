...
- 条件输入：顾问先在群里发送 `找人条件：……`，再点「按条件找人」；插件按群保存最近一次明确条件并在按钮直调时注入，未发送条件则按职位事实执行。
  - 锁定版 OpenClaw 不解析 `form_value`，因此卡片不得展示会吞输入的内联输入框。
  - 后续若现有唯一事件连接能稳定透传 `form_value`，可另行审核内联输入；不得为此给同一飞书应用再开第二条事件连接。
- 保留「打开职位工作台」按钮。
- 候选结果卡的继续找人按钮下方必须用弱化说明文字明确：先发送 `找人条件：……` 再点击；不发送条件则按职位事实继续，并自动排除已推荐人选。
...
6. 候选结果卡的继续找人提示紧跟按钮，使用 `note` 层级，不与候选事实和主动作争夺视觉层级。
