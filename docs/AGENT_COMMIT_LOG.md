...
## 2026-09-24｜fix(连接): 统一设置入口与卡片基线

- 设置中心点击“数据连接”时直接进入正式统一连接中心，避免仍停留在旧 TTC/飞书/人才库摘要页；独立设置组件在 Storybook 中仍保留原有审核场景。
- 将 OAuth 回归文件恢复为统一 CRLF，并在人眼核对标题、按钮和布局后更新 OpenMai 成功/失败两张飞书卡片基线；浏览器桌面/移动链路、快速门禁 16/16 和卡片渲染 19/19 通过。

## 2026-09-24｜feat(supermai): 打通桌面找人回传链路

- 将 SuperMai 从 OpenMai criteria 复用中彻底拆出：飞书创建云端任务，顾问电脑用一次性配对码安装常驻连接器，主动领取本人任务并调用本机 SuperMai/Sourcing 官方 harness；云端不接触招聘平台密码、验证码、Cookie 或浏览器 profile。
- 新增设备、任务、租约、事件、候选结果和登录命令账本；支持设备撤销、离线重领、旧 token 失效、候选幂等、官方资料链接白名单、失败收敛，以及原项目群内带 SuperMai 品牌的结果卡回传；OpenMai 与 SuperMai 结果和继续找人排除项保持隔离。
- 连接中心新增配对码、可执行 ZIP 安装包、设备在线状态和异步官方登录动作；同步规格、运行手册、部署文档、Storybook 和五状态审核台账。快速门禁 16/16、后端与共享逻辑 884/884、前端 56/56、Storybook 24 文件 104/104、生产与 Storybook 构建均通过；本提交后继续执行完整门禁与目标环境部署，真实异机平台搜索留给用户验收。

## 2026-09-24｜feat(入口): 接入首次进入与统一连接中心

- 新增 `/join` 首次入口，将邀请、飞书官方登录、进入工作台和来源授权压缩为三步；当前没有正式安装包时明确复用已安装 SuperMai，不展示虚假下载动作。
- 将连接中心加入正式主导航，统一展示飞书、OpenMai、SuperMai、Reloop 真实状态；移除原页面手工粘贴 TTC token 的交互，BOSS/脉脉/猎聘只从服务端 allowlist 打开官方登录页。
- 同步 Storybook、中央审核台账、当轮记录、施工清单与连接器规格；前端静态测试 56/56、Storybook 24 文件 103/103、生产构建、浏览器端到端和快速门禁 16/16 通过。本地 Mia 显示 4/4 就绪，BOSS 已登录；未执行真实找人任务、未发布、未 push。

## 2026-09-24｜feat(连接): 提供 SuperMai 官方登录启动动作

- 新增鉴权后的 SuperMai 平台启动接口，只接受 BOSS、脉脉、猎聘三个内建标识，由本机连接器打开对应官方登录页；调用方不能传入任意 URL。
- 保持账号密码、验证码、Cookie 与 token 只在第三方官方页面和本机客户端中处理，BrainX 接口只返回是否已打开及下一步用户动作。
- 补充平台 allowlist、恶意 URL 忽略、未登录拒绝与路由边界回归，专项 7/7 通过；本轮不发布、不 push。
...
- 后端与共享逻辑测试 875/875、前端静态与适配测试 56/56、Storybook 交互测试 97/97、飞书群卡片渲染回归 19/19 通过；两个生产构建、浏览器链路和服务烟雾测试通过。
- 将规格 `034` 标记为 Verified，并确认施工手册阶段 10 本地实现完成；用户视觉审核、目标环境发布、生产迁移和真实成本/漏斗/备份验证仍未完成，本轮未发布或 push。

## 2026-09-24｜fix(看板): 避免非管理员探测受限端点

- 在本人档案响应中增加最小 `operations_admin` 能力位，前端只为管理员请求运营快照；普通顾问不再为隐藏入口主动触发 403，受限 GET/投影/追溯本身仍独立拒绝。
- 补充管理员与普通顾问能力位回归；修复完整门禁浏览器链路把预期 403 计为控制台错误的问题，不扩大看板数据或写入权限。

## 2026-09-24｜feat(看板): 建立事件投影运营看板

- 新增 `0059` 窄事件流、checkpoint、实体读模型、快照、投影运行和备份证据；触发器捕获后续状态并幂等回填当前历史事实，worker 增量消费，显式重建不改事实账本。
- 管理员 API 只读投影并提供有界脱敏追溯；重复、迟到旧状态、业务更正、增量与重建一致性均有回归，普通顾问读取/投影/追溯独立拒绝。
- 设置中心接入管理员运营看板，覆盖加载/新鲜度、失败/积压、容量/备份、真实漏斗、LIVE/SHADOW 排序与成本、Token/P95 和样本成熟度；后端 875/875、前端 56/56、Storybook 97/97、阶段专项 4/4 通过，完整门禁将在实现提交上复跑。本轮不连接生产、不执行生产迁移/备份/恢复、不发布、不 push。

## 2026-09-24｜docs(看板): 规格化事件投影运营看板

- 新增规格 `034`：以独立归一事件流、checkpoint、实体态和租户快照承载阶段 10 看板，日常 GET 只读投影，不扫描在线业务表；重复、乱序、更正和重建必须保持同一结果。
- 冻结同步/积压/失败、容量增长、备份证据、真实漏斗、LIVE/SHADOW 排序分歧、Token/成本/P95 和样本成熟度口径；未知值保持未知，SHADOW 不冒充真实曝光或结果。
- 管理员 GET、投影和重建统一显式 allowlist，事件追溯只返回脱敏信封；本轮不连接生产、不执行生产迁移/备份/恢复、不发布、不 push。

## 2026-09-24｜docs(验证): 记录阶段 09 完整门禁

- 在 Algorithm A 忠实呈现实现提交 `cb49aff` 上运行完整质量门禁：25/25 通过，报告结论“通过”，Push 条件“满足”。
- 后端与共享逻辑测试 871/871、前端静态与适配测试 54/54、Storybook 交互测试 91/91、飞书群卡片渲染回归 19/19 通过；生产构建、Storybook 静态构建、浏览器链路和服务烟雾测试通过。
- 将规格 `033` 标记为 Verified，并确认施工手册阶段 09 三项完成；用户视觉审核、目标环境发布和真实 A 数据验证仍未完成，本轮未切生产读取、发送、发布或 push。

## 2026-09-24｜feat(展示): 接入 Algorithm A 忠实呈现

- 新增默认关闭的 A 展示投影：只读 `LIVE` 原序，游标冻结引擎、运行、搜索和 rank；生成、弃权、失败、旧结果及读取时职位/权限失效均显式呈现，SHADOW 和旧分数排序被隔离。
- 工作台、飞书、scheduler、CLI 与预览工具消费同一 A 语义，显示理由、权衡、证据引用、待核实项和建议动作，不补造 score、概率或置信；主动设置支持画像、公司/职位/城市排除和容量，旧六维只保留为 `baseline-1.1` 历史。
- A 忽略/撤销沿用同一幂等事件边界；前端五状态台账、当轮记录、施工清单、Storybook 说明和独立卡片基线同步。实现回归后端 871/871、前端 54/54、Storybook 91/91、卡片 19/19 通过；完整门禁将在本实现提交上复跑。本轮不切生产读取、不发送、不发布、不 push。

## 2026-09-24｜docs(展示): 规格化 Algorithm A 忠实呈现

- 新增规格 `033`：正式展示只读 `LIVE` A run，游标绑定引擎、运行、搜索和原始 rank；SHADOW、旧分数排序和前后端二次评分均不得进入 A 展示。
- 工作台与飞书消费同一原序投影，如实区分候选数与实际推荐数，展示理由、证据、不确定项和异步/旧结果状态，不补造分数、概率、置信或理由。
- A 设置改为主动画像、排除公司/职位/城市和容量，旧六维只保留为明确的 baseline 历史；Storybook 与五状态审核文档必须同提交同步。本轮不切生产读取、不发送、不发布、不 push。
