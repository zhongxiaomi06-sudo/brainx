...
- 真机复现：顾问先发送 `找人条件：……`、再点击“OpenMai 继续找人”时，条件消息和按钮被 Agent 分别解释为启动动作，第二轮完成后又产生第三轮任务；第二轮卡片标题还错误沿用“首轮”。
- 修复：明确条件消息只保存下一轮条件并回复确认，必须等找人按钮或“现在开始找人”的明确命令才调用工具；候选卡标题按 `search_round` 显示真实轮次。
- 验证：专项测试与快速门禁结果见本提交；修复后的单条件、单按钮真机复测仍待服务重启后执行。
