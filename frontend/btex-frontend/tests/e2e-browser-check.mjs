...
try {
  ...
  assert.equal(await recommendationCards.first().getByLabel(/AI 匹配分/).count(), 1,
    "正式卡片标题区必须以可访问标签呈现匹配分");
  assert.doesNotMatch(firstCardText, /证据覆盖|探索价值/,
    "正式列表不得恢复已移出卡面的内部评分维度");
