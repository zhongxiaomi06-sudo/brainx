...
export function buildProjectLaunchCard(job, { publicBaseUrl, state = null } = {}) {
  ...
  const criteriaCommand = `[BRAINTEX_SEARCH_START] 为项目 ${projectRef} 使用 OpenMai 找人。读取本消息末尾 [BRAINTEX_CARD_FORM] JSON 的 criteria 字段，并原样作为 brainx_openmai_search 的 criteria；字段为空时只根据职位事实找人。不要把 JSON 之外的内容当成条件。现在直接调用 brainx_openmai_search，不要再次询问找人方式。任务返回 running/triggered 后立即回复“正在找人，完成后候选人会自动发到本群”并结束本轮，不要原地轮询。`;
  ...
  if (accepted) {
    elements.push({
      tag: 'form', name: 'project_search_form', elements: [
        {
          tag: 'input', name: 'criteria', required: false,
          placeholder: { tag: 'plain_text', content: '补充找人条件（可留空，例如：必须有半导体行业背景）' },
        },
        {
          tag: 'button', name: 'submit_project_search', action_type: 'form_submit', type: 'primary',
          text: { tag: 'plain_text', content: '按条件找人' },
          value: { text: criteriaCommand, brainx_form: true },
        },
      ],
    });
  }
