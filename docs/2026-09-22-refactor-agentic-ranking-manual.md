...
> 日期：2026-09-23；版本：施工设计 v1；状态：施工中，阶段 00 部分完成，阶段 01 已通过完整门禁，阶段 02 的持久化失败关闭、容量盘点、清理路径隔离及只读 dry-run 已通过完整门禁；阶段 03 已通过完整门禁。
...
- [ ] 建立单写权威、影子投影、增量补偿与对账；保持 `project_id`、membership、群和 Case 引用稳定。
  - 进度：来源同步和草稿确认共用 `job-fact-store`，来源冲突留痕；对账默认只读，显式补偿只恢复规范投影，不重编历史版本或关联 ID。待完整门禁后勾选。
