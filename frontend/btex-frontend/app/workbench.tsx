...
import { BACKEND_WEIGHTS, FALLBACK_DISMISS_REASONS, brainxFetch, connectSSE, fetchJobDetail, getPickTray, getSnapshot, makeIdempotencyKey, mapRecommendation, mapReplayData, mapRadarRow, mapClientRow, nextRecommendationBatch, getRadar, getClients, getTalentSupply, updateOpportunityFacts, updateOpportunityMembership, updateWorkbenchPreferences, sendRecommendationFeedback, undoRecommendationFeedback, streamAssistant, rerunOpenmai, BrainxApiError, type TalentSupplySnapshot, type AssistantMessage, type BackendEngagementResponse, type BackendOutcomeResponse, type BackendPickTray, type BackendProfileUpdate, type BackendRecommendationRun, type BackendReplay, type BackendSessionStatus, type BrainxReplay, type BrainxSnapshot, type ManualFactField, type OpenmaiResult, type RadarJob, type RadarClient } from "./brainx-api";
...
export default function DecisionWorkbench(){
 ...
 useEffect(()=>{if(!hydrated)return;if(brainxMode==="connected"){localStorage.setItem("decision-workbench",JSON.stringify({tray,folders,folderMode,weights,decisionActions,membershipRelations,sidebarWidth,dismissedRecommendationIds}));void updateWorkbenchPreferences({tray,folders,folderMode}).catch(()=>{});return}localStorage.setItem("decision-workbench",JSON.stringify({done,snoozed,extraTasks,weights,decisionActions,membershipRelations,sidebarWidth,tray,dismissedRecommendationIds,folders,folderMode,engagement,events:decisionEvents,outcomes,sync,auth,notifications}))},[hydrated,brainxMode,done,snoozed,extraTasks,weights,decisionActions,membershipRelations,sidebarWidth,tray,dismissedRecommendationIds,folders,folderMode,engagement,decisionEvents,outcomes,sync,auth,notifications]);
 ...
 return <div className={`app btex-app concept-shell ${navOpen?"nav-open":""} ${assistantOpen?"assistant-open":""} ${panelMotion==="open"?"decision-panel-open":""} ${panelPresent?"decision-panel-present decision-panel-compact":""} panel-motion-${panelMotion} ${sidebarResize?"is-resizing":""} ${mobileNavOpen?"mobile-nav-open":""} ${mobileDrawerDrag.current?"mobile-nav-swiping":""}`} style={{"--sidebar-width":`${navOpen?sidebarWidth:68}px`,"--mobile-drawer-progress":mobileDrawerProgress??1} as React.CSSProperties} onPointerDown={beginMobileSwipe} onPointerMove={moveMobileSwipe} onPointerUp={event=>endMobileSwipe(event)} onPointerCancel={event=>endMobileSwipe(event,true)}>
  <aside className="rail-nav" aria-label="主要导航">
   ...
   <button className={`rail-brand${recommendationRefreshing?" is-refreshing":""}`} onClick={handleBrandClick} aria-label="刷新推荐" title="刷新并换一批推荐" disabled={recommendationRefreshing}><img className={`rail-brand-logo${brandSpin?" is-spinning":""}`} src="/btex-logo.svg" alt="B-tex BrainX" onAnimationEnd={()=>setBrandSpin(false)}/></button>
