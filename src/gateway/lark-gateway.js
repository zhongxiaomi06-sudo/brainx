...
export const BOT_OPEN_ID = 'ou_bot'; // 机器人自身 open_id 约定（mentions 含此即 @机器人）

/**
 * 处理一条已解密的飞书消息事件。
 * @returns {{ack:boolean, action:'queued'|'denied'|'duplicate', reason?:string}}
 */
export function processLarkEvent(db, evt) {
  ...
  if (ctx.bot_mode === 'MENTION_ONLY' && !(evt.mentions ?? []).includes(BOT_OPEN_ID)) {
