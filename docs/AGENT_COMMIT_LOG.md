...
## 2026-09-24｜docs(验证): 记录阶段 08 完整门禁

- 在 Algorithm A 影子评估实现提交 `6d4dd9c` 上运行完整质量门禁：25/25 通过，报告结论“通过”，Push 条件“满足”。
- 后端与共享逻辑测试 864/864、前端静态与适配测试 53/53、Storybook 交互测试 86/86、飞书群卡片渲染回归 18/18 通过；生产构建、Storybook 静态构建、浏览器链路和服务烟雾测试通过。
- 将规格 `032` 标记为 Verified，并确认施工手册阶段 08 三项完成；未连接生产、执行生产迁移/回填、切读、发送、发布或 push，影子证据不构成全员上线批准。

## 2026-09-24｜feat(评估): 建立 Algorithm A 影子评估闭环

- 新增 `0058` 冻结评估计划和影子运行账本；A run 以 `LIVE/SHADOW` 明确隔离，旧基线只在 A 的冻结候选池上计算，不写正式 decision、recommendation 或 impression。
- 评估拆分 judged universe 召回覆盖与池内 Agent 选择覆盖，复用 `ranking-metrics-v2` 报告基线/A NDCG、Top-K 分歧，并保存模型、prompt、工具、策略、Token、成本和延迟证据。
- 支持参考运行名次稳定性及七类场景回放；计划级汇总按预登记门槛检查样本、覆盖、非劣、成本、延迟、稳定性与分组差异。影子不生成真实结果或 propensity，最高只建议受控灰度。
- 专项回归 44/44 通过；整仓测试在受限沙箱因本地监听被拒而不可作为结果，完整门禁将在实现提交上以允许本地监听的环境复跑。本轮不连接生产、不迁移生产库、不切读、不发送、不发布、不 push。

## 2026-09-24｜docs(评估): 规格化 Algorithm A 影子证据

- 新增规格 `032`：评估计划先冻结显式样本、覆盖、非劣、成本、延迟、稳定性和分组门槛，不设置未经业务确认的生产默认值。
- 基线与 A 复用同一授权、时点和候选池，分开报告召回遗漏与 Agent 选择遗漏；支持重复、候选顺序扰动、长文本、缺失画像、冷启动、来源冲突和恶意内容回放。
- A 影子 run 必须与 LIVE 隔离，正式推荐、投递、impression 和真实曝光计数保持不变；影子数据不伪造真实转化，最高只建议受控灰度。本轮不连接生产、不切读、不发布、不 push。

## 2026-09-23｜docs(验证): 记录阶段 07 完整门禁

- 在 Algorithm A 最小闭环实现提交 `e152543` 上复跑完整质量门禁：25/25 通过，报告结论“通过”，Push 条件“满足”；首次执行仅浏览器登录后加载等待超时为 24/25，未作为完成依据。
- 成功轮次后端与共享逻辑测试 853/853、前端静态与适配测试 53/53、Storybook 交互测试 86/86、飞书群卡片渲染回归 18/18 通过；生产构建、Storybook 静态构建、浏览器链路和服务烟雾测试通过。
- 将规格 `031` 标记为 Verified，并将施工手册阶段 07 三项标记完成；Algorithm A 仍默认关闭，未切正式读取、未连接生产、未执行生产迁移/回填、未发送、发布或 push。

## 2026-09-23｜feat(推荐): 完成 Algorithm A 最小闭环

- 新增 `0057` 独立 A generation/run/item 账本和默认关闭的 `agentic-ranking-v1` 编排；模型调用不占数据库事务，也不写入 `decision_runs/recommendations`，因此不切正式 Web/飞书读路径。
- 服务端先完成完整快照、可见性、在招、HC、明确排除、忽略、已承接与容量硬过滤，再冻结多路召回、覆盖清单、画像/信号/负载/权限/职位事实/证据和模型策略版本；授权 Context Registry 正文只在运行时提供，账本仅存引用。
- Agent 最终列表严格校验 schema、run、候选、事实版本、连续 rank 和证据；最多两次修复且计入用量，合法弃权和不足 20 条不补齐。发布前 generation/CAS 与硬状态变化会标记 STALE，回放不再次调用模型。专项回归 43/43 通过，未切读、发送、发布或 push。

## 2026-09-23｜docs(验证): 记录阶段 06 完整门禁

- 在 Agent 上下文与完整用量实现提交 `d51b02f` 上运行完整质量门禁：25/25 通过，报告结论“通过”，Push 条件“满足”。
- 后端与共享逻辑测试 841/841、前端静态与适配测试 53/53、Storybook 交互测试 86/86、飞书群卡片渲染回归 18/18 通过；生产构建、Storybook 静态构建、浏览器链路和服务烟雾测试通过。
- 将规格 `030` 标记为 Verified，并将施工手册阶段 06 三项标记完成；本轮未连接生产、切读、发布或 push。

## 2026-09-23｜feat(Agent): 建立上下文与完整用量账本

- 新增 `0056` 追加迁移和逐尝试用量记录器，覆盖成功、失败、重试、取消、缓存与未知用量；Agent loop 聚合所有轮次，强制收尾和重试共用调用/Token 预算，默认仍不自动重试。
- 新增 Context Registry：记录来源版本、授权范围哈希、摘要、内容哈希、大小、有效期、截断与撤权；每次读取重验租户/作用域/时效，run 只保存上下文引用。
- Algorithm A 专用注册表严格暴露五个只读窄工具，服务端注入身份并复用职位可见性；限制页数、结果量、超时和取消，人才供给只返回脱敏聚合字段。专项回归 40/40 通过，未连接生产、发布或 push。

## 2026-09-23｜docs(验证): 记录阶段 05 完整门禁

- 在画像、曝光与结果账本实现提交 `9875a5b` 上运行完整质量门禁：25/25 通过，报告结论“通过”，Push 条件“满足”。
- 后端与共享逻辑测试 836/836、前端静态与适配测试 53/53、Storybook 交互测试 86/86、飞书群卡片渲染回归 18/18 通过；生产构建、Storybook 静态构建、浏览器链路和服务烟雾测试通过。
- 将规格 `029` 标记为 Verified，并将施工手册阶段 05 三项标记完成；本轮未连接生产、执行生产迁移/回填、切读、发布或 push。

## 2026-09-23｜feat(反馈): 建立画像曝光与结果账本

- 新增 `0055` 追加迁移和领域账本：主动画像逐版保存且与个人模型凭据分离；短期信号记录来源、样本、置信、算法、时间窗、到期和撤回；项目负载从当前承接与行动实时计算。
- 新增 served/visible 真实曝光事件，事件 ID 重放幂等且冲突失败；Case 权威事件按租户、来源实例和来源事件去重，更正追加不覆盖历史，无明确决策关联时只记录未归因结果。
- 兼容 `job_outcomes` 投影并补齐 Case、来源、更正和归因字段；专项回归覆盖画像版本、信号时效、负载、曝光、结果幂等和归因，未连接生产、执行生产迁移/回填、发布或 push。

## 2026-09-23｜docs(架构): 规格化三阶段 Agent 推荐闭环

- 新增规格 `029`：主动画像与个人模型分离，短期信号可过期/撤回，负载从项目实时计算，真实曝光和 Case 结果/更正/归因可幂等回放。
- 新增规格 `030`：每次模型尝试记录成功、失败、重试、取消和缓存，用量缺失保持 unknown；上下文每次按租户/作用域重校验，A 只获得五个只读窄工具。
- 新增规格 `031`：默认关闭且不切正式读的 `agentic-ranking-v1`，Agent 决定最终项和顺序，校验器不后置评分/重排/补齐，generation/CAS 阻止旧运行覆盖新结果。
- 三个阶段都先写可失败回归，分别创建原子实现提交并在提交上跑完整门禁；不新增依赖，不连接生产、不执行生产迁移/回填、不发布、不 push。

## 2026-09-23｜docs(验证): 记录阶段 04 完整门禁

- 在来源版本与证据链实现提交 `a5a3dbd` 上运行完整质量门禁：25/25 通过，报告结论“通过”，Push 条件“满足”。
- 后端与共享逻辑测试 832/832、前端静态与适配测试 53/53、Storybook 交互测试 86/86、飞书群卡片渲染回归 18/18 通过；生产构建、Storybook 静态构建、浏览器链路和服务烟雾测试通过。
- 将规格 `028` 标记为 Verified，并将施工手册阶段 04 三项标记完成；本轮未连接生产、执行生产迁移/回填、切读、发布或 push。

## 2026-09-23｜feat(职位事实): 建立来源版本与证据链

- 新增统一来源信封和 `0054` 追加迁移；TTC 与本地 CSV/fixture 统一产生规范事实、来源记录、不可变版本、字段证据和冲突记录，不完整 TTC 分页不再伪装为完整快照。
- 来源同步和群/JD 草稿确认复用单一事实写模块；确认前仍不进权威投影，LLM 提炼缓存按租户与群/顾问授权范围隔离。
- 雷达改从规范字段还原城市与 Pipeline，不再读取 `raw_json.ttc`；对账默认 dry-run，显式补偿可恢复漂移或缺失的当前投影，不修改 membership 和历史版本。
- 验证：专项回归 89/89、整仓后端 832/832 通过；快速门禁在提交前复跑。本轮未连接生产、执行生产迁移/回填、切读、发布或 push。

## 2026-09-23｜docs(架构): 规格化版本化职位事实

- 复核 TTC、fixture/CSV、群消息/JD 草稿确认和 `job_facts` 写入链，确认已有来源接入但缺少统一信封、不可变版本、字段证据、提炼缓存与投影对账。
- 新增规格 `028`：统一来源完整性契约，以单一写模块原子维护兼容投影、版本和证据；来源冲突必须留痕，补偿默认 dry-run 且不改写 membership、群、Case 或历史 ID。
- 保持现有同步、推荐、确认和 `baseline-1.1` 行为；本单元不新增依赖，不连接生产、不执行生产迁移/回填、不切读、不发布、不 push。
- 验证：文档链接、格式和快速门禁在提交前执行；实现先增加能在旧结构上失败的最小回归，最新提交再跑完整门禁。

## 2026-09-23｜docs(验证): 记录阶段 03 完整门禁

- 在推荐边界实现提交 `3df967e` 上运行完整质量门禁：25/25 通过，报告结论“通过”，Push 条件“满足”。
- 后端与共享逻辑测试 825/825、前端静态与适配测试 53/53、Storybook 交互测试 86/86、飞书群卡片渲染回归 18/18 通过；生产构建、Storybook 静态构建、浏览器链路和服务烟雾测试通过。
- 将规格 `027` 标记为 Verified，并将施工手册阶段 03 两项标记完成；本轮未连接生产、迁移、发布或 push。
...
- 新增规格 `027`：以唯一 recommendation use case、显式 SQLite repository 和集中配置解析统一四类入口，并抽取推荐 route factory；旧导出保留为兼容 facade。
- 冻结 `baseline-1.1` 的资格、评分、顺序、冻结、节流、HTTP、CLI 与推送行为；本单元不新增依赖、表或迁移，不连接生产、不发布、不 push。
- 验证：文档链接、格式和快速门禁在提交前执行；实现必须先增加能在旧结构上失败的最小回归，最新提交再跑完整门禁。

## 2026-09-23｜docs(验证): 记录 server 路由拆分完整门禁

- 在实现提交 `967ef84` 上运行完整质量门禁：25/25 通过，报告结论“通过”，Push 条件“满足”。
- 后端与共享逻辑测试 820/820、前端静态与适配测试 53/53、Storybook 交互测试 86/86、飞书群卡片渲染回归 18/18 通过；生产构建、Storybook 静态构建、浏览器链路和服务烟雾测试通过。
- 将规格 `026` 标记为 Verified，并同步阶段 03 施工进度；本轮未连接生产、迁移、发布或 push。

## 2026-09-23｜refactor(架构): 拆分登录与人才路由

- 新增 `authRoutes()`，原样承接登录页、OAuth 状态/授权/回调和开发 session；保留 `exchangeCode` 测试注入、花名册失败关闭、Cookie、重定向与开放路由边界。
- 新增 `talentRoutes()`，原样承接人才状态/健康/查询/同步/简历和职位供给；仓库根路径显式注入，继续执行路径限制、职位可见性及人才持久层失败关闭。
- `server.js` 从 625 行降至 483 行，只组装两个领域 route factory；删除其质量门禁超限例外，并以结构回归防止 handler 回流或入口重新超限。
- 验证：结构回归先因模块不存在而失败；专项测试 36/36、快速门禁 16/16 通过。本轮未改路由清单、数据库、前端或生产配置，未连接生产、迁移、发布或 push。

## 2026-09-23｜docs(架构): 规格化 server 路由边界拆分

- 复核 625 行 `src/server.js`，确认 OAuth/session 与人才库路由已有独立领域依赖，却仍和 HTTP 传输、业务路由、静态代理及进程启动混在总入口。
- 新增 `026-server-route-boundaries` 规格、方案和任务：先抽取登录与人才两个 route factory，保持方法、路径、开放清单、鉴权、错误码、Cookie、重定向和依赖注入完全兼容。
- 要求入口降到 500 行以内并删除对应质量门禁存量例外；本单元不改业务规则、数据库、前端或生产配置，不连接生产、不迁移、不发布、不 push。
- 验证：文档链接、`git diff --check` 和快速门禁在提交前执行；实现先增加能在旧结构上失败的最小回归，完整门禁在实现提交后执行。

## 2026-09-23｜docs(验证): 记录清理 dry-run 完整门禁

- 在清理路径隔离与只读计划实现提交 `1e49001` 上执行完整质量门禁，25/25 全部通过，报告结论为“通过”、Push 条件为“满足”。
- 后端与共享逻辑 817/817、前端 53/53、Storybook 86/86、飞书卡片 18/18 通过；浏览器链路、生产构建、Storybook 构建和服务烟雾测试通过。
- 后端与前端生产依赖漏洞均为 0；结构、安全、格式、Lint、类型和 500 行限制全部通过。没有执行 push、生产连接、迁移、归档、清理、checkpoint、VACUUM 或发布。

## 2026-09-23｜fix(治理): 隔离旧清理并增加只读计划

- 移除推荐事务内的历史裁剪，保留单轮写入上限；旧 `brainx-retention --apply` 在触库前固定返回 `RETENTION_APPLY_DISABLED`，无 apply 时转到只读盘点。
- 新增 `retention-plan-v1` 和 `npm run retention:plan`：策略显式提供固定截点、TTL 与最近轮次，只用 SELECT 聚合推荐和节流候选，不输出标识、内容、SQL 或执行命令。
- 推荐候选同时排除最近轮次、展示/真实下发、业务结果、决策事件和追加负反馈引用；缺最新冻结特征列或引用表即失败关闭。字段报告、同步运行和原始上下文仍未实现清理策略。
- 验证：红灯分别抓到隐式三轮裁剪和缺失规划器；专项测试 16/16、快速门禁 16/16 通过。本轮没有连接生产、迁移、归档、删除、checkpoint、VACUUM、发布或 push。

## 2026-09-23｜docs(治理): 规格化安全清理 dry-run

- 复核推荐事务内自动裁剪和旧 `brainx-retention --apply`，确认二者均早于真实曝光、冻结特征和追加负反馈引用，在责任人、TTL、恢复点及最新 schema 未就绪时不应继续拥有删除能力。
- 新增 `025-retention-dry-run-safety` 规格、方案和任务：停止业务事务隐式删除，旧 apply 在打开数据库前失败，只读 dry-run 仅输出聚合候选与保护原因，缺表或列即阻断。
- 字段报告、同步运行和原始上下文继续保持未实现/禁止删除；无论策略内容如何，本单元都不开放执行、归档、checkpoint、VACUUM 或可复制 SQL。
- 验证：文档链接、`git diff --check` 和快速门禁在提交前执行；完整门禁在实现提交后执行，本任务不连接生产、不清理、不发布、不 push。

## 2026-09-22｜docs(验证): 记录数据保留盘点完整门禁

- 在数据保留只读盘点实现提交 `a6a485b` 上执行完整质量门禁，25/25 全部通过，报告结论为“通过”、Push 条件为“满足”。
- 后端与共享逻辑 813/813、前端 53/53、Storybook 86/86、飞书卡片 18/18 通过；浏览器链路、生产构建、Storybook 构建和服务烟雾测试通过。
- 后端与前端生产依赖漏洞均为 0；结构、安全、格式、Lint、类型和 500 行限制全部通过。没有执行 push、生产连接、迁移、归档、清理、checkpoint、VACUUM 或发布。

## 2026-09-22｜feat(治理): 增加数据保留只读盘点

- 新增 `retention-inventory-v1` 与 `npm run retention:audit`：SQLite 只读打开，聚合字段报告、同步运行、限流跳过、推荐/特征快照和原始上下文的行数、字节、日增长及跨表引用，不输出正文或业务标识，也不提供 apply 模式。
- 固定输出权威性、建议责任角色、责任分配、TTL、恢复点和删除阻断；责任人未分配、TTL 未批准、恢复点未验证时所有类别均为 `deletion_ready=false`。
- 本地只读盘点确认字段报告大字段约 122.8 MiB，并发现当前副本缺最新特征快照列与负反馈事件表；文档登记运行时裁剪和旧 retention apply 遗漏的新引用，明确禁止据此清理或迁移。
- 验证：回归先红后绿，严格只读、脱敏、完整/缺表、引用和责任阻断 4/4 通过；`npm run verify:quick` 在提交前执行，完整门禁在本提交后执行。本轮没有 DDL、DML、checkpoint、VACUUM、生产连接、清理、发布或 push。

## 2026-09-22｜docs(治理): 规格化增长与保留责任盘点

- 审计阶段 02 的容量与生命周期入口，确认现有运行时裁剪和 `brainx-retention --apply` 早于曝光、冻结特征、双时间标签和追加负反馈引用，当前不能证明删除集合安全。
- 新增 `024-retention-inventory` 规格、方案和任务：以 SQLite 只读模式输出脱敏容量、增长、跨表引用和责任矩阵；人员未分配、TTL 未批准、恢复点未验证时一律阻断删除。
- 本单元先建立可复核盘点，不执行 DDL、DML、checkpoint、VACUUM、归档或清理，不连接生产环境，也不把本地大小外推为生产容量。
- 验证：文档链接、`git diff --check` 和快速门禁在提交前执行；完整门禁在实现提交后执行，本任务不 push。

## 2026-09-22｜docs(验证): 记录人才持久化完整门禁

- 在人才持久化失败关闭实现提交 `fc3837c` 上执行完整质量门禁，25/25 全部通过，报告结论为“通过”、Push 条件为“满足”。
- 后端与共享逻辑 809/809、前端 53/53、Storybook 86/86、飞书卡片 18/18 通过；浏览器链路、生产构建、Storybook 构建和服务烟雾测试通过。
- 后端与前端生产依赖漏洞均为 0；结构、安全、格式、Lint、类型和 500 行限制全部通过。没有执行 push、生产 RDS 连接、迁移、回填、清理或发布。

## 2026-09-22｜fix(持久化): 禁止人才写入自动降级

- 新增人才后端策略：运行期必须显式选择 MySQL 或双重确认的易失内存；缺配置、缺完整凭据、断连、基础表或迁移历史未就绪均使用稳定错误码失败关闭，不产生内存假成功。
- 移除人才业务运行期自动建表，改为只读校验 18 张基础/增量表和迁移文件清单；后续 MySQL 操作失败统一包装安全错误，不向 HTTP 或 Agent 边界泄露主机、SQL、密码和底层异常正文。
- 健康探针、生产预检、环境示例、RDS 配置脚本、安全/部署/交接文档同步显式后端与临时 DDL 账号边界；离线内存健康检查固定返回非零，不能充当持久化验收证据。
- 验证：新增回归先红后绿；人才后端、迁移、简历、匹配、shortlist、Agent 工具、服务与生产预检专项 63/63 通过，`npm run verify:quick` 在提交前执行；完整门禁在本提交后执行。本轮不连接生产 RDS，不执行迁移、回填、清理、发布或 push。

## 2026-09-22｜docs(持久化): 规格化人才库失败关闭单元

- 复核 `src/talent.js`、健康接口与部署配置，确认缺凭据和连接失败会自动写入易失内存库，应用运行账号连通后还会尝试建表，均不满足阶段 02 的持久化及最小权限边界。
- 新增 `023-talent-persistence-fail-closed` 规格、方案和任务：后端必须显式选择；内存演示双重显式启用；MySQL 缺凭据、断连或 schema 未就绪使用稳定错误码失败关闭。
- 运行期只校验基础表和迁移历史，不执行 DDL；建表仍由临时迁移账号和独立 CLI 负责。本单元不连接生产 RDS，不执行迁移、回填、清理、恢复、发布或 push。
- 验证：文档链接、`git diff --check` 和快速门禁在提交前执行；完整门禁在实现提交后执行。

## 2026-09-22｜docs(验证): 记录负反馈回放完整门禁

- 在负反馈回放实现提交 `d75d9b7` 上执行完整质量门禁，25/25 全部通过，报告结论为“通过”、Push 条件为“满足”。
- 后端与共享逻辑 799/799、前端 53/53、Storybook 86/86、飞书卡片 18/18 通过；浏览器链路、生产构建、Storybook 构建和服务烟雾测试通过。
- 后端与前端生产依赖漏洞均为 0；结构、安全、格式、Lint、类型和 500 行限制全部通过。没有执行 push、生产数据库迁移、历史回填、训练、模型晋升或发布。

## 2026-09-22｜fix(评估): 追加回放排序负反馈

- 新增 0053 追加式反馈事件账本，负反馈、原因更正与撤销分别保存发生/收到时间、稳定原因码、原文、来源和幂等键；现有忽略及推荐反馈表继续作为当前态投影，不回填或猜测旧历史。
- 工作台与飞书推荐卡传递准确 `decision_id`，服务端校验顾问和职位归属；项目级无关联忽略仍保留业务排除，但不会计入任一推荐贡献，旧无决策参数调用保持兼容。
- `ranking-label-v2` 在真实曝光观察窗口内回放负反馈极性，LTR 导出、离线评估、影子日报和决策回放暴露稳定原因证据；重新加入、接单和反馈撤销恢复当前可见性但不删除历史事件。
- 验证：新增回归先红后绿；负反馈回放、标签窗口、快捷卡、成员关系、迁移与排序评估专项 51/51 通过，`npm run verify:quick` 16/16 通过；完整门禁在本提交后执行。本轮不执行生产迁移、回填、训练、模型晋升、发布或 push。

## 2026-09-22｜docs(评估): 规格化负反馈回放单元

- 复核当前忽略和旧推荐反馈链路，确认 `opportunity_ignores` 在重新加入时删行、`recommendation_feedback` 在补充原因时覆盖且撤销时删行，均不能按评估截点回放历史。
- 新增 `022-ranking-feedback-replay` 规格、方案和任务清单：追加记录负反馈、撤销与原因更正；只有明确绑定同一 `decision_id` 的负反馈才计入推荐贡献，项目级忽略保持未归因。
- 保留现有忽略与反馈表作为当前兼容投影，不回填或猜测旧数据；本单元不执行生产迁移、训练、模型晋升、发布或 push。
- 验证：文档链接、`git diff --check` 和快速门禁在提交前执行；完整门禁在实现提交后执行。

## 2026-09-22｜docs(验证): 记录标签时间切分完整门禁

- 在标签时间切分实现提交 `0528aa9` 上执行完整质量门禁，25/25 全部通过，报告结论为“通过”、Push 条件为“满足”。
- 后端与共享逻辑 795/795、前端 53/53、Storybook 86/86、飞书卡片 18/18 通过；浏览器链路、生产构建、Storybook 构建和服务烟雾测试通过。
- 后端与前端生产依赖漏洞均为 0；结构、安全、格式、Lint、类型和 500 行限制全部通过。没有执行 push、生产数据库迁移、历史回填、训练、模型晋升或发布。

## 2026-09-22｜fix(评估): 按观察窗口切分排序标签

- 新增 `ranking-label-v2` 与 0052 迁移：新事件和业务结果分别保存发生时间与收到时间，历史记录不回填；推荐关联写入时校验顾问、职位和 `decision_id` 一致。
- LTR 导出、离线排序评估和影子日报必须显式提供窗口与数据截点，只纳入真实曝光后、窗口内发生且截点前收到的明确关联事实；未曝光、未成熟、旧记录缺时间和成熟但未知分别报告，不把未知当 0。
- 保留正式 `labelFor()` 和 `baseline-1.1` 行为；7 日只用于测试，不设生产默认。本轮不执行生产迁移、回填、训练、模型晋升、发布或 push，忽略撤销、更正和负反馈原因留给下一原子单元。
- 验证：新增回归先红后绿，标签窗口及相关评估专项通过；`npm run verify:quick` 在提交前执行，完整门禁在本提交后执行。

## 2026-09-22｜docs(评估): 规格化标签时间切分单元

- 复核标签、曝光、承接事件和业务结果链路，确认现有离线评估读取当前状态与全部结果，缺少观察窗口、数据截点、收到时间和成熟度，会让未来或迟到结果污染历史轮次。
- 新增 `021-ranking-label-windows` 规格、方案和任务清单：离线评估显式传入窗口与截点，只接受同一 `decision_id` 的双时间事实；未曝光、未成熟、旧记录缺时间和未知结果分别报告。
- 7 日仅为建议和测试参数，不设生产默认；本单元不回填旧数据、不执行生产迁移、训练、模型晋升或发布，忽略撤销和负反馈原因留给下一原子单元。
- 验证：文档链接、`git diff --check` 和快速门禁在提交前执行；完整门禁在实现提交后执行，本任务不 push。

## 2026-09-22｜docs(验证): 记录特征冻结完整门禁

- 在迁移测试修复提交 `2f35528` 上重新执行完整质量门禁，25/25 全部通过，报告结论为“通过”、Push 条件为“满足”。
- 后端与共享逻辑 788/788、前端 53/53、Storybook 86/86、飞书卡片 18/18 通过；浏览器链路、生产构建、Storybook 构建和服务烟雾测试通过。
- 后端与前端生产依赖漏洞均为 0；结构、安全、格式、Lint、类型和 500 行限制全部通过。没有执行 push、生产数据库迁移、历史回填或发布。

## 2026-09-22｜test(迁移): 同步特征快照迁移清单

- 完整门禁发现迁移框架测试仍把仓库清单固定为 53 个文件；补入 `0051_recommendation_feature_snapshots.sql`，并把旧库升级后的记账数更新为 54。
- 只同步测试期望，不改变迁移实现、正式推荐或数据；首次完整门禁其余 24 项均通过，后端 788 项中仅这 2 项旧期望失败。
- 验证：迁移专项与快速门禁在提交前执行；完整门禁在本提交后从最新 commit 重跑，本任务不 push。

## 2026-09-22｜fix(评估): 冻结历史排序特征

- 新增 `0051_recommendation_feature_snapshots.sql`，新推荐与 `ltr-feat-v1` 完整特征快照在同一事务持久化；历史空值保持空，不执行回填或生产迁移。
- LTR 训练导出、影子 NDCG 与分歧日报只读冻结快照；缺失、损坏、错版本或不完整快照按稳定原因码排除，不再用当前 `job_facts` 伪造历史输入。
- 保持正式 `baseline-1.1` 资格、评分、排序与对外返回不变；标签观察窗口、双时间戳和推荐贡献归因留给下一原子单元。
- 验证：特征冻结、排序评估和后端评估专项通过；`npm run verify:quick` 在提交前执行，完整门禁在本提交后执行，本任务不 push。

## 2026-09-22｜docs(评估): 规格化排序特征冻结单元

- 复核 LTR 导出、离线影子评估与分歧日报，确认三条历史路径都重新读取当前 `job_facts`，会让职位后续变化污染过去输入。
- 新增 `020-ranking-feature-snapshots` 规格、方案和任务清单：新推荐事务内冻结 `ltr-feat-v1`，旧记录缺失/损坏时明确排除，不用今天的事实回填伪历史。
- 本单元只编写迁移和代码并在内存库验证，不执行生产迁移、回填、数据写入或发布；标签观察窗口与结果双时间留给下一原子单元。
- 验证：文档链接、`git diff --check` 和快速门禁在提交前执行；完整门禁在实现提交后执行，本任务不 push。

## 2026-09-22｜docs(验证): 记录排序评估完整门禁

- 在代码提交 `21109b8` 上执行完整质量门禁，25/25 全部通过，报告结论为“通过”、Push 条件为“满足”。
- 后端与共享逻辑 782/782、前端 53/53、Storybook 86/86、飞书卡片 18/18 通过；前后端浏览器链路、生产构建、Storybook 构建和服务烟雾测试通过。
- 后端与前端生产依赖漏洞均为 0；结构、安全、格式、Lint、类型和 500 行限制全部通过。没有执行 push，也没有数据库迁移、生产数据写入或发布。

## 2026-09-22｜fix(评估): 修正排序指标与轮数参数

- 新增共享 `ranking-metrics-v2`：DCG 保留原预测位置，IDCG 从完整评估组取理想 Top K；未知标签不当 0、不把第 11 名挤进 Top 10，并在报告中暴露标签覆盖率和限制说明。
- 修正 Python NDCG 按预测排序索引读取标签，训练模型保存指标版本；LTR 导出、规则评估与影子评估绑定同一指标版本，不改变正式 `baseline-1.1` 推荐顺序。
- 修复 `evaluate(db, { runs })` 被模块级默认值覆盖的问题；加入 4 个先红后绿的手算/参数回归用例。
- 验证：排序评估与既有后端评估专项 13/13 通过，`npm run verify:quick` 16/16 通过；完整门禁在本提交后执行，本任务不 push。

## 2026-09-22｜docs(重构): 冻结首个施工单元基线与规格

- 按 9 月 22 日施工总手册启动阶段 00，冻结分支、commit、工作锁、迁移编号、正式推荐/离线评估/LTR 入口与接口兼容边界；确认 Algorithm A 尚无实现或开关，本轮保持关闭。
- 新增 `019-ranking-evaluation-correctness` 的规格、方案和任务清单，明确 NDCG 必须从完整评估组计算 IDCG、未知标签不得挤压预测位置，且本单元不改数据库与正式推荐顺序。
- 改造前完整门禁 25 项通过 18 项、失败 7 项：代码静态项、前端 53 项测试和构建通过；npm audit、监听端口与无头浏览器受当前沙箱权限阻断。未建立脱敏数据库恢复点，因此后续数据写入/迁移仍禁止。
- 验证：文档链接、`git diff --check` 与快速门禁在提交前执行；完整门禁须在提交后的可联网、可监听端口环境重跑，本提交不 push。

## 2026-09-22｜docs(重构): 建立仓库重构与Agent最终推荐施工手册

- 阅读用户提供的架构审计 HTML 与伴随报告，基于 `6ae182b` 复核推荐、评估、标签、Agent 用量、人才库回退及既有账本/outbox，区分历史审计数字、本轮代码事实与未做的生产验证。
- 新增施工总手册、Algorithm A 决策契约、数据契约与迁移手册：00–12 任务卡、依赖、改动入口、验收与回滚；明确 Agent 最终选择/排序、规则仅校验、失败不规则补榜、来源解耦、画像和真实反馈、逐调用用量及单写迁移。
- 同步文档总目录、当前 PRD 与旧文档适用边界；保留历史审核记录，不改变前端完成/发布状态，不修改业务代码或数据。
- 验证：3 篇新文档相对链接、JSON 示例、`git diff --check` 通过；`npm run verify:quick` 16/16 通过（前端静态与适配 53/53），退出码 0。完整门禁按规范在本次提交后执行，结果见本 commit 对应质量报告；本任务不 push。

## 2026-09-22｜merge(主线): 同步远端最新代码并保留本地演示改动

- 将 `origin/main` 最新提交合并到当前 `codex/offer-single-report-sync` 分支，保留本地尚未上传的 6 个提交，不使用覆盖或重置。
- 冲突处理：采用远端 OpenClaw 插件 `1.4.18` 版本号及其测试口径；保留本地四阶段招聘工作流展示、技术栈说明与相关提交记录，同时纳入远端杨东旭 Offer 群固定回复能力。
- 验证：合并后执行 OpenClaw 插件专项测试与快速质量门禁；完整验证结果以本次提交后的质量报告为准。

## 2026-09-17｜docs(演示): 补充工作流技术栈与答疑边界

- 审阅四阶段 16 节点的实际调用链；在最新版 HTML 的加号展开区加入英文技术标签、中文机制与 Q&A，保留默认折叠和箭头链路。
- 明确确定性评分、外部搜索、RDS 人才写入、人工电话、模板报告、群条件内存缓存和人工知识收录边界；不把规划组件或本地实现包装成已上线能力。
- 新增逐节点代码证据与现场追问说明、展示文档回归测试；同步本轮审核、中央台账、目录、施工清单及 Storybook 范围，不修改业务后端。
- 验证：专项 66/66、快速门禁 16/16 均通过；Chrome 1440/1024/390 宽度逐节点键盘展开/关闭、全展开无横向溢出、页面错误 0；人工复核截图。完整门禁在提交后执行，不 push。

## 2026-09-17｜docs(演示): 精修招聘工作流页面视觉

- 保留四阶段 16 节点、技术说明与反馈路径，按用户要求精修为白底、浅灰大圆角、系统字体、细灰箭头和小圆展开钮，反馈入口使用克制蓝色。
- 删除重复编号、英文眉标和彩色图例；调整居中标题、节点比例、留白、角色标签与吸顶导航，修复反馈文字与连接线叠加。
- 同步本轮审核记录、中央台账、目录、施工清单和 Storybook 范围；新版视觉待用户复看，未接入正式应用或发布。
- 验证：快速门禁 16/16；1440/1024/390 无横向溢出，16 节点键盘展开/关闭、反馈定位通过，页面错误 0；桌面与手机截图人工复核通过。完整门禁在提交后执行。

## 2026-09-17｜docs(演示): 精简为四阶段招聘工作流轨迹图

- 按用户确认的原始轨迹图方向，把展示页压缩为四阶段、16 个动作；移除独立口号、优势、演示指标与架构大段，使用连续箭头和反馈链接连接主线。
- 每节点直接展示一句技术逻辑及人机分工，原生 details 承载机制和边界；标清多轮找人、需求回流及人工经验复用，保留单报告与待真机验证口径。
- HTML/CSS 分离且无需 JavaScript；同步文档目录、中央审核台账、审核记录、施工清单及 Storybook 范围说明。用户只确认结构方向，新版视觉仍待复看。
- 验证：快速门禁 16/16，通过；1440/1024/390 宽度无横向溢出，16 节点键盘展开/关闭、反馈锚点检查通过，页面错误为 0；人工复核桌面与手机截图。提交后执行完整门禁。

## 2026-09-17｜docs(演示): 重构招聘工作流展示页视觉系统

- 对照 Apple 产品页的章节节奏、Stripe 的能力分层、Attio 的工作界面叙事与 Ashby 的招聘产品信息架构，重做 BrainTex 四阶段演示页；只吸收设计规律，不复制品牌资产或页面结构。
- 移除黑底渐变、玻璃拟态、发光与 3D 卡片，改为浅色编辑式网格、衬线标题、边线信息板和克制的蓝/绿/橙章节色；首屏直接展示四阶段系统地图，滚动章节保留技术步骤、现有能力和诚实边界。
- 重新编排 2026-09-16 演示证据与统一技术底座，补全窄屏响应式、减少动态效果兼容、滚动进度和章节导航；单文件保持在 500 行以内。
- 验证：HTML 脚本语法、唯一锚点与本地 HTTP 预览正常；`npm run verify:quick` 16/16 通过，前端静态与适配测试 53/53 通过；完整门禁在提交后重跑。

## 2026-09-16｜docs(演示): 新增四阶段 AI 猎头工作流滚动展示页

- 调研当前职位决策、项目建群、OpenMai/Reloop/SuperMai 多轮找人、项目跟进、候选决策群和 Offer 单报告链路，按「职位选择与派单 / AI 推人与人才池 / 触达跟进 / Offer 管理」四章整理技术步骤、自动化边界与差异化优势。
- 新增深色发布会式长滚动 HTML：全屏黏性章节、玻璃导航、滚动进度、渐入与轻 3D 动效、桌面/窄屏响应式、减少动态效果兼容；补入 2026-09-16 三个 Offer 群彩排证据，并说明当前单一可编辑报告已替代彩排阶段的多版本文档。
- 验证：浏览器实际检查首屏、章节滚动、20 个技术步骤、窄屏响应式与控制台错误（0 条）；`npm run verify:quick` 16/16 通过，前端静态与适配测试 53/53 通过。

## 2026-09-16｜test(卡片): 更新远端合并后三张视觉基线

- 合并后 OpenMai 续搜卡和 SuperMai 自由找人卡的操作与提示区增高，Offer 决策群首卡改为单一报告入口；重新生成这 3 张 macOS/Chrome 截图基线。
- 人眼复核：三张卡片的标题、候选人列、按钮、续搜提示和底部说明均完整，无截断、重叠或错位；其余 13 张仅存像素漂移的图片已恢复，不夹带无关变化。
- 验证：`node scripts/quality-gate/card-render/run.mjs --update` 18/18 通过；完整门禁将在本提交后重跑。

## 2026-09-16｜merge(main): 同步远端 Offer 群与找人更新

- 将 `origin/main` 最新 10 个提交合入本地单报告分支，保留远端的组织内链接可编辑、特定 Offer 群背景、SuperMai 自由找人卡和演示数据，同时保留本地每群单一报告、实时编辑稿读取与继续找人提示。
- 飞书文档层合并两条读取路径：通用报告问答分页读 blocks，特定群兼容 `raw_content`；新建文档追加内容后再开放 `tenant_editable`，任一步失败均明确报错。
- 验证：合并前 commit `5865492` 完整门禁 25/25 通过；合并后报告/文档/卡片/特定群 prompt 专项 37/37 通过，快速门禁除合并过程必然存在的 `MERGE_HEAD` 检查外 15/15 通过；合并提交后重跑完整门禁。

## 2026-09-16｜fix(报告): 单一可编辑报告支持实时正文问答

- Offer 决策群改为每群只创建一份稳定飞书文档：重复生成和旧 `REGENERATE` 调用复用现有 `document_id`，不再新增 V2/V3 或重复发送报告卡；显式新增证据只追加到原文档，保留用户人工编辑。
- `brainx_candidate_report` 新增只读 `READ` 模式，分页读取飞书 Docx 当前块正文并在返回 Agent 前隐藏手机号/邮箱；普通报告提问与 `/report` 强制走只读，只有带 `[BRAINTEX_REPORT_CREATE]` 标记的生成按钮允许首次写入。OpenClaw 插件升级至 1.4.17。
- 决策群首卡移除“更新报告”，保留“生成报告 / 查看 TTC 人才”；报告卡去除版本号并明确“唯一报告、可编辑、@机器人可问答”。同步更新权威规格、审核台账、施工清单、Storybook 边界说明和两张卡片截图基线。
- 验证：报告/文档/决策群/插件/运行时专项 24/24，`npm run verify:quick` 16/16；报告卡与决策群首卡渲染回归各 1/1，截图人眼确认无截断。完整门禁与目标飞书真机编辑稿问答在提交后执行。

## 2026-09-16｜docs(验证): 记录云端合并后的完整门禁

- 合并提交 `d8ddae5` 已在允许本机端口、无头浏览器与 npm 漏洞库访问的完整环境重跑质量门禁。
- 验证：25/25 全部通过；后端与共享逻辑 752/752、前端 53/53、Storybook 86/86、飞书卡片渲染 17/17，生产构建、浏览器链路与服务烟雾测试均通过，依赖漏洞为 0。
- 边界：本次只完成 GitHub 云端代码到本地的合并与验证，未部署生产服务器；服务器 SSH 无可用凭据且正式 HTTPS 连接被重置，线上运行提交仍未取得直接证据。

## 2026-09-16｜merge(main): 同步远端人才卡与群准入修复

- 将 `origin/main` 最新 8 个提交合入 `codex/local-live-test`，保留本地 7 个候选卡、人才跑批核查和前端连续搜索相关提交。
- 业务代码自动合并成功；仅本日志因双方都在顶部追加记录产生文本冲突，已保留两边全部记录。
- 验证：合并态快速门禁 16/16，通过安全、结构、语法、Lint、类型和前端 53/53；提交后继续执行完整门禁。

## 2026-09-15｜docs(飞书): 记录新版候选卡真实群验证

- 经用户明确同意，将 `J933IDQ` 的 8 位真实候选新版卡发送到“TTC 测试客户-未来科技-AI 算法工程师”群；飞书返回消息 `om_x100b65a4f99908a4b48919864c755d9`。
- 飞书桌面端实见姓名、匹配摘要、“初筛通过”“加入reloop”、两个继续找人按钮与弱化提示完整显示；点击“8. 颜泽荣”成功进入 TTC 真实详情。两个写动作与第二轮搜索未点击，不伪装为已验证。
- 账号核查：Dykes 的 Gateway 机器人身份已绑定，但飞书 OAuth 登录被“小机器人”应用可用范围拦截，本地尚未生成 Dykes 的网页登录令牌；需应用管理员扩大可用范围后重试，不复制 Mia 凭证绕过权限。

## 2026-09-15｜docs(人才): 记录真实跑批覆盖缺口

- 合并远端后连接真实 RDS 执行人才匹配 dry-run：401 人、91 个活跃职位中仅 36 个有候选，55 个零命中；目标样本 `JHTSEQJ / 荆华密算 / 销售` 仍为零。
- 根因证据：真实库 `talent_tag` 无标签关联，虽然 67 份人才摘要包含销售/商务/市场等信息，但现算法把技能与意向标签合计设为 80% 权重，摘要仅为 20% 兜底；因此本轮不执行 `--write`，避免把空结果写成正式短名单。
- 验证：远端合并专项 42/42；最新合并 commit 完整门禁 25/25 通过；本地 OAuth 已配置、真实 RDS 已连接且 schema ready。账号就绪审计显示 Dykes 已绑定机器人身份，但尚无本人网页登录令牌和 TTC 凭证，真实飞书按钮链路仍需本人授权后复验。

## 2026-09-15｜merge(main): 合并远端人才跑批与群权限修复

- 将远端 `origin/main` 最新两项改动合入本地候选卡与连续搜索分支，保留双方实现及提交记录；业务代码无冲突，仅合并日志顶部追加位置发生文本冲突。
- 新增独立人才匹配跑批模块，并补入 intake 绑定群的群上下文权限放行；本地已有候选卡继续找人提示、二次条件注入和连续搜索改动保持不变。
- 验证：合并相关人才跑批、群权限、二次找人、候选动作与投递专项 42/42；提交后完整门禁 25/25 通过（后端全量、前端、Storybook 86/86、浏览器链路、卡片渲染 16/16、构建与烟雾测试）。

## 2026-09-15｜feat(飞书): 候选卡补充继续找人提示

- 按用户确认，在 OpenMai 候选结果卡的继续找人按钮下方新增飞书原生 `note` 弱化提示。
- 提示明确二次找人方式：先在同群发送 `找人条件：……` 再点继续找人；不发条件则按职位事实继续，并自动排除已推荐人选。
- 验证：候选投递、条件记录和候选动作专项 33/33；macOS 卡片渲染场景通过，人眼确认提示位于按钮下方、弱化显示且无截断。

## 2026-09-15｜fix(飞书): 移除无法生效的找人条件输入框

- 对照 0.99 会议“测试按条件找人”要求复核，确认锁定版 OpenClaw 只传按钮命令、不传卡片 `form_value`；现有内联输入框会吞掉顾问输入，不能继续作为正式能力展示。
- 项目群卡移除假输入框，保留真实可执行流程：先在同群发送 `找人条件：……`，插件按群保存条件，点击“按条件找人”后确定性注入；未发送条件则按职位事实执行。
- 验证：项目群卡片、条件解析与按钮直调专项测试通过；真实飞书客户端回调仍受当前账号应用权限限制，继续标为待真机验证。

## 2026-09-15｜fix(frontend): 补全公司在前的连续搜索

- 本地真实页面复核发现上一提交仍只支持“职位 + 公司”的字段顺序，“荆华密算销售”返回 0；根因是空格虽已消除，但没有覆盖用户实际输入的“公司 + 职位”顺序。
- 搜索新增“公司 + 职位”连续组合；Storybook 等待筛选状态收敛后核对唯一目标行和总行数，修复原断言可能读到更新前列表的假阳性。
- 验证：前端测试 53/53；Storybook 浏览器测试 86/86；生产构建成功；本地正式入口使用 Mia 账号与 TTC 真实数据搜索“荆华密算销售”，返回 `JHTSEQJ / 销售`、`JP8WWYX / 销售总监` 共 2/2 条。

## 2026-09-15｜fix(frontend): 修复公司与职位连续搜索

- 对照 0.99 会议纪要核查“荆华密算销售”搜索失败：真实库中公司与职位分别存为“荆华密算”和“销售”，前端字段拼接产生的空格导致连续中文查询无法命中。
- “全部职位”搜索同时匹配原始字段串和去空格字段串，不改变职位事实、排序、权限或筛选；Storybook 增加跨公司/职位字段的连续搜索回归。
- 验证：本地真实库确认 `JHTSEQJ / 荆华密算 / 销售` 存在；前端静态与适配测试 53/53；`npm run verify:quick` 16/16 通过；真实页面验证在启动本地服务后补录。
...
- 改动：`plugins/brainx-openclaw/prompt.js` 的 `buildSpecialGroupSuffix` 措辞收紧——
  ① 去掉"实时同步飞书文档""5 分钟刷新缓存"等暴露读取机制的描述，改成中性的"本群业务上下文"。
  ② 去掉"明确说明该信息未在背景文档中，请用户补充"——这条会让模型对未命中事实主动说"未在背景文档中"，间接暴露文档存在。
  ③ 新约束："不要向用户透露信息来源、读取状态、内部机制或文档存在；不要主动暴露任何与读取能力相关的限制或状态"。
  ④ prompt 字面不再含"飞书文档""实时同步""缓存""未在文档中""读不到""我没有这份报告""暂时无法读取"等暴露词，避免模型复述。
- 边界：fail-open 设计不变——缓存未命中时仍返回无文档版 BRAINTEX_SYSTEM_CONTEXT，小机器人按通用猎头助手回答，自然不会说"我读不到"（因为它根本不知道有这份文档）；缓存命中时基于内容回答但不说来源。
- 验证：`tests/brainx-prompt-special-group.test.mjs` 测试 5 加 7 条隐蔽性断言（飞书文档/实时同步/缓存/未在文档中/未在背景文档中/读不到/我没有这份报告），12 例全过。
...
- 处置：该批次挡住质量门禁（「Git / 完整检出」报 22 个被跟踪文件缺失），经零一代为单独 commit 记录既有事实；`.specify/integrations/.cache/` 为工具缓存不入库。如需回滚 revert 本提交即可。首次提交漏掉 codebuddy/codex 两个 manifest 的删除，随后补提。
...
- 同时注明其最右 TTC 按钮属于历史方案，当前界面以用户最新确认的“姓名直达 TTC + 初筛通过 + 加入reloop”为准。
- 验证：`origin/main` 是当前分支祖先；当前分支无 merge commit，符合受保护分支 PR 要求。

## 2026-09-15｜test(frontend): 对齐推荐卡浏览器验收口径

- 完整门禁发现浏览器验收仍要求列表显示已在“摘要优先层级”改版中移除的“证据覆盖/探索价值”，并寻找旧文案“AI 匹配分”。
- 验收改为确认标题区的可访问标签存在“AI 匹配分”（可见层保持紧凑的“匹配 + 数字”），并反向断言列表不恢复两个内部评分维度；不修改生产组件或数据。
- 验证：浏览器前后端链路与完整门禁在本提交后重新执行。

## 2026-09-15｜feat(飞书): 候选总览改为三入口并直发人才链接

- OpenMai 候选总览改为姓名直达 TTC，右侧纵向提供“初筛通过”和“加入reloop”；3/5/3 列宽确保四字姓名与动作均不截断。
- “初筛通过”继续写入项目共享重点名单，但后续改为在来源项目群幂等发送一条 TTC 纯链接，交由飞书展开标准卡片；旧的 BrainX 独立候选人卡与渲染场景下线。
- “加入reloop”仅调整入口排版，仍调用既有真实人才库动作；插件版本 1.4.15→1.4.16。
- 验证：候选流程、飞书发送、OpenMai 卡片、插件和质量门禁专项 74/74；卡片渲染回归 16/16，人眼复核姓名与两枚动作无截断；快速门禁与提交后状态见本提交最终记录。

## 2026-09-14｜feat(card): 候选人行最右加 TTC 人才库纯跳转链接 + 恒星力量群权限核查结论

- 触发：用户报告恒星力量群里「点名字没有弹出卡片、所有候选工具 NOT_FOUND_OR_FORBIDDEN、绑定的账号职位不对」，并要求每行最右加 TTC 人才库链接。
- **权限核查结论**（生产逐表取证）：JBC31PR（恒星力量·市场营销/PR主管）是 **linda 的项目**——她 9/11 接单（ACCEPTED 事件 + MY_JOB 成员关系）、9/14 10:06 建群、群 scope 仅授权她本人 sender、她已关注 PL1872647880871571456（即用户说的「之前一次 candidate_workflow 成功」）。mia 与该职位无任何成员关系/推荐/事件 → 她的候选工具按最小权限返回 NOT_FOUND_OR_FORBIDDEN（设计行为，不是按钮故障）；自助接单也接不了（`acceptJob` 先过 `requireVisible`，该职位对 mia 不可见）。「绑定不对」在系统层面不成立（群↔职位↔linda 完全一致）；是否应改归 mia 属业务调配，需 linda 释放或管理员处理，未擅自改绑。
- **卡片改版（历史）**：候选人行当时由两列改三列——名字按钮（2/7）｜匹配与背景（4/7）｜TTC 人才库链接（1/7）。该布局已被 2026-09-15 最新三入口方案取代，权限核查结论继续有效。
- 验证：当时 `openmai-delivery` 17/17，渲染基线经人眼复核。

## 2026-09-15｜fix(frontend): 清理客户洞察未知字段噪音

- 客户职位状态不再露出后端 `UNKNOWN`：与已知状态共存时隐藏，纯未知时显示“待确认”。
- 列表不再为所有空客户类型重复显示“类型未标注”，客户事实侧栏仍保留缺失提示；真实字段和下钻动作不变。
- 验证：Storybook 86/86；生产构建成功；真实 4020 个客户、23245 个职位、首屏 100 行中 `UNKNOWN` 与“类型未标注”均为 0，截图已发当前对话。

## 2026-09-15｜fix(frontend): 职位表改用可读状态与负责人

- 将未进入跟进或关注的 `PENDING` 从“待处理”改为“待判断”，避免把 2 万余条未筛职位表现成紧急待办。
- 负责人列由单个首字圆点改为首字头像加完整姓名，缺失值仍明确显示“待确认”；同步补齐 Storybook 和前端审核证据。
- 验证：Storybook 86/86；生产构建成功；真实页面首屏 100 行、待判断 23245 条，旧“待处理”0 条，负责人姓名正确显示，截图已发当前对话。

## 2026-09-15｜fix(frontend): 推荐列表占满工作区宽度

- 为推荐队列补齐明确的 100% 工作区宽度，消除宽屏下列表缩在中央、与顶部筛选栏不对齐的问题。
- Storybook 增加父容器宽度回归；真实页面测得工作区和队列均为 1180px，20 张卡正常显示。
- 验证：Storybook 86/86；生产构建成功；真实截图已发当前对话。

## 2026-09-15｜refactor(frontend): 推荐卡改为摘要优先层级

- 推荐摘要提前到核心事实之前，主匹配分收进标题区；默认列表移除证据覆盖和探索价值，保留在统一详情与冻结数据。
- 状态、阶段只有真实已知时才显示，减少整页重复“待确认”；同步更新正式入口与独立组件的 Storybook 回归。
- 验证：前端单测 53/53；Storybook 86/86；生产构建成功；真实 20 张卡摘要 20 条、低价值评分 0 条、未知状态/阶段 0 条，截图已发当前对话。

## 2026-09-15｜feat(frontend): 推荐卡增加简明筛选摘要

- 正式精选盘每张推荐卡新增一条由冻结推荐理由压缩的“推荐摘要”，优先说明画像方向、职位关系和现有进展，不新增或猜测事实。
- Pipeline 内部英文阶段统一显示为中文；增加摘要与枚举格式化单测、Storybook 断言，并保持排序、评分和业务动作不变。
- 验证：前端单测 53/53；Storybook 86/86；生产构建成功；本地真实 20 张卡均有摘要且无 `Interview / Recommendation / Sourcing / Onboarding`，截图已发当前对话。

## 2026-09-15｜fix(frontend): 清理职位名称格式标记

- 新增统一职位名称展示清理，覆盖精选盘、我的项目、全部职位与统一详情，去除真实数据中泄漏的 Markdown 加粗、链接、注释和标题标记，不改写后端源数据。
- 增加适配层与 Storybook 回归，真实精选盘第 6 条从 `** AI 销售 / 商业化增长负责人` 正确显示为干净标题。
- 验证：前端单测 51/51；Storybook 86/86；快速门禁 16/16；生产构建成功；真实页面正文无 `**`，截图已发当前对话。

## 2026-09-15｜fix(frontend): 项目列表移除显眼忽略入口

- 依据 0.99 产品会议和用户当前逐项施工指令，从正式“我的项目”卡片动作区移除“忽略”，只保留阶段主动作与“详情”；统一职位详情中的安全忽略入口及后端持久排除能力不变。
- 同步更新 `ProjectsView` 类型、正式入口、Storybook 场景、静态回归、前端审核记录、中央台账、施工清单和组件说明。
- 验证：TypeScript 0 错误；前端单测 51/51；Storybook 85/85；快速门禁 16/16；生产构建成功。真实页面加载 14 个项目，列表“忽略”0 个、启动与详情各 14 个，详情内合法“忽略”1 个；截图已发当前对话。

## 2026-09-14｜fix(talent): 修复真实 MySQL 人才列表分页失败

- 真实数据验收发现：人才库健康检查为 `mysql / ready`，但 `listTalents` 在生产 MySQL 上使用 `LIMIT ? OFFSET ?` prepared statement 时返回 `ER_WRONG_ARGUMENTS`，导致人才列表接口不可用。
- 修复：新增分页查询构造器，对 `limit/offset` 做整数化和上下界收敛后安全内联；状态值仍使用参数绑定，既兼容当前 MySQL，也不放宽注入边界。
- 验证：新增分页 SQL 回归测试；真实 RDS 普通分页返回 5 条、`active` 状态分页返回 5 条；完整门禁结果待本提交后执行并回填最终交接。
...
- 修复：`isBotMentioned` 增加 `@braintex的小机器人` 纯文本形态（并保留 open_id 标签与 `</at>` 两种既有形态）。插件 1.4.10→1.4.11。
- 验证：补纯文本名形态放行用例，相关 24/24。
...
- 改法：`noteCells()` 组装完成后，把**最后一个 field 格强制 wide**（跨两列）作为收尾大格；宫格末行不再出现半格空洞。前四格仍锁死单格（2×2 田字）。
- 验证：单测新增「最后一个标签块恒为收尾大格」断言，前端单测全过；ESLint / TypeScript 通过；预览稿同步（展开态注意事项为大格）。

## 2026-09-13｜fix(frontend): 备注改 2×2 田字宫格——标签块各自成格、零散内容整块收放

- 触发：用户三轮反馈「排版还是很丑」并明确「田 = 横两个、竖着两个，其他的一起收放，不然很乱」。上一版是「标签列 + 内容列」的表格，纵向拉长且零散，不是宫格。
- 改法：`workbench-note-format.ts` 新增 `noteCells()`——每个 `**标签**：值` 组装成**一个独立格子**（标签为格内小标题、正文在格内，默认 3 行截断）；超过 70 字的长块跨两列；`# 标题` 连同其下的 `- 列表` 打包成**一个整块格子**一起收放。渲染默认只露出前 4 格（横两个竖两个），其余按「展开其余 N 项」整批收放。CSS 改 `repeat(2, minmax(0,1fr))` 两列 + 1px gap 网格线。
- 解析修正：同步备注是**一整行**，`##` 与 `- ` 都内联其中。先在含 `#` 的行里把 ` - ` 拆成独立行，再按「空白 + #」切标题块（此前用 `(?=#)` 会把 `##` 切成两半、多出一个孤立的 `#`）。
- 验证：新增宫格组装用例，前端单测 51/51；ESLint / TypeScript 通过；预览稿 `outputs/note-grid-preview.html`（真实备注数据，默认态 + 展开态）。

## 2026-09-13｜fix(frontend): 判断面板区块折叠与备注田字格——开关置底、次要信息默认收起

- 触发：用户二轮反馈——「当前事实」的折叠开关要放到区块下面；备注内容按田字格结构排列；判断依据不重要，与「为什么现在做/风险与缺失/证据来源/候选供给（人才侧参考）」一起全部默认折叠、可点击展开。
- 改法：`DrawerSection` 加 `collapsible / defaultOpen / forceOpen`，折叠开关固定在区块底部（标题只留标题与动作）；「当前事实」默认展开且编辑中强制展开，其余四个区块 `defaultOpen={false}`；「判断依据」从事实表 `dl` 移出，改为事实表底部的 `BasisRow` 折叠项。备注渲染由行内标签对改为 `.note-grid` 田字格（1px gap 透底色成网格线，标签列/内容列，caption 与列表跨两列）。
- 约束：`workbench.tsx` 为贴边文件（493 行），改动用行内属性完成，行数不变。
- 验证：前端 ESLint / TypeScript / 单测全过；预览稿 `outputs/note-grid-preview.html`（真实备注数据）供用户目视验收。
...
- 根因：项目群的 `requireMention=false` 群级覆写（`openclaw-group-access.js:56`）是**按钮回调的载荷**——卡片回调合成消息没有 @ 元数据，全局 mention 门会把按钮一起挡掉；但它同时放行了所有闲聊。不能简单删掉覆写（按钮会死），也不能改第三方 feishu 插件（手册禁令）。
- 方案（自有插件 `mention-silence.js`，挂 `before_agent_reply`）：群会话里消息满足任一放行条件才走 agent——含 `<at `（@ 了机器人）/ 含 `[BRAINTEX_` 标记或 `brainx_` 工具指令（全部按钮命令特征）/ `/` 控制命令 / 「找人条件」开头；**会话追问保护**：最近 10 分钟内有可见动作的会话，后续普通消息放行（按钮→追问→「确认」多轮流程不掐断）。其余群消息 `handled:true` 吞掉：不调模型、不回复。私聊（:direct:）不适用。
- 插件版本 1.4.5→1.4.6，`files[]` 同步收录新文件。
- 验证：新增 `openclaw-mention-silence.test.mjs` 4 例（闲聊吞/@ 放行/按钮命令放行/窗口内追问放行+超窗沉默/私聊不适用）；相关 45/45。

## 2026-09-13｜feat(card): 候选人推荐卡标题改 Reloop 品牌

- 触发：york 建议「推人选的时候，要不然叫Reloop」，用户确认「每次候选人的推荐都改成 reloop 的候选人推荐」。
- 改动：投递卡标题 `BrainTex · 第 N 轮候选人已就绪/不足/请补充职位信息/搜索失败` → `Reloop 候选人推荐 · 第 N 轮已就绪/…`；人才卡头 `BrainTex · 候选人卡片` → `Reloop · 候选人卡片`。与「加入reloop」按钮同一品牌线。
- 验证：测试断言同步 45/45；渲染基线重建并人眼复核（新标题、左置名字按钮、斑马纹）。

## 2026-09-13｜feat(card): 找人结果卡排版重构——名字按钮左置 + 斑马纹分行

- 触发：用户看第 2 轮投递卡后指出「点击名字在左边，然后排版重新构建，这个排版不好看」。
- 改版（`src/openmai-delivery.js`）：候选人行由「信息左 + 名字按钮右」翻转为「**名字按钮左 + 信息右**」，表头同步为「点名字关注 | 匹配与背景 · 核心匹配」；数据行加灰/白**斑马纹**（`background_style` 交替），多候选人扫读不串行。按钮/指令串/intro note/质量提示均不变。
- 验证：`openmai-delivery` 17/17（列序、斑马纹、按钮文案与指令串断言同步）；渲染基线重建并人眼复核（左列按钮不截断、斑马纹清晰）；卡片渲染门禁 17/17。
- 部署提示：本卡由 brainx-worker 投递构建，生产需重启 worker。

## 2026-09-13｜fix(frontend): 判断面板事实区收敛为白名单 + 合成行「判断依据」（第二轮）

- 触发：用户用真实数据打开判断面板，指出字段仍然过多、并确认第一轮精简**没有部署上**；选定 B 版口径「只留下几行精简的文字，备注信息留下来」。
- **事实区白名单**：新增无 JSX 纯模块 `app/workbench-facts-model.ts`（`judgementFactOrder` + `judgementRows`），判断面板只展示 `职位关系 / 主做顾问 / 职位状态 / 当前阶段 / 剩余 HC / 历史 Pipeline / 下一步动作 / 城市 / 备注`（有值才出现），`备注` 完整保留、不截断。
- **合成行「判断依据」**：由 `决策层级` + `事实可信度` 合成（如「本周关注 · 数据部分缺失」），替代原「决策层级 / 决策层级原因 / 事实可信度 / 事实更新时间」四行；`数据来源 / 最近活动（含时间、来源）/ Offer 状态 / 入职状态 / 事实可信度规则` 一并退出判断面板。
- 数据层不变：`job.facts` 键位保留，队列卡片（`workbench-today.tsx`）继续消费 `最近活动/最近活动时间/事实更新时间/决策层级/事实可信度`，层级派生与排序不受影响；事实编辑与「恢复同步值」逻辑不变。
- 验证：`tsc --noEmit` 0 错误；`npm test`（btex-frontend）**46/46**（新增 `judgementRows` 回归测试：行序、备注保留、合并行、无层级数据时不显示合成行）；Storybook 构建成功；**无头渲染断言**（构建产物 + Playwright/系统 Chrome 打开 Facts story）实测行标签 `["职位关系","当前阶段","剩余 HC","历史 Pipeline"]`，9 个应删行全部不出现；story 内新增对应交互断言供 CI 执行（临时 worktree 的 node_modules 为符号链接、位于服务根之外，`storybook:test` 无法在该环境运行，本地以纯函数单测 + 无头渲染替代，CI 干净检出照常跑）。
- 协作说明：本次在**独立 worktree**（基线 `f56dc2f`）完成，未触碰 kimi-code 持有的共享工作区（`logs/agent-work.lock` 自 13:00 起为 kimi-code）；主工作区分支已推进，合并时以远端为准。
...
- [飞书群卡片渲染回归门禁](2026-09-12-feishu-card-render-gate.md)：阻断判据补第 6 类「文字排版 6 条」；第 5 节从「已知缺陷与修复方向」改写为「首轮运行抓到的缺陷与处置」，记录 F2/F3 的修复做法、新抓到的 11 行文字墙，以及**门禁自身的时间戳归一化缺陷**及其修复。
- [飞书群机器人卡片按钮审核](frontend-reviews/2026-09-12-feishu-card-buttons-audit.md)：按台账规则**只追加不改写**，新增「处置进展（第二轮）」一节 —— F2/F3 处置与证据、五维状态同步（用户审核仍为「待复看」、目标环境未发布、真实数据未验证）、以及本轮新发现的 **F8 表格单元格词内换行**（含候选方案：把「经验/城市」与「学历」合并成一列「背景」，列数降到 4；属信息架构调整，需用户拍板）。原文结论与 F4/F5/F6/F7 未决项保持不动，仅把 F2/F3 勾选并指向新章节。
- [前端审核台账](frontend-reviews/README.md)：中央状态行更新为「待复看（技术核查曾判不通过）」，发布列改为「旧版已发布／新版未发布」以反映「代码已改、未部署」的真实状态；同步列出 F1/F4/F5/F6/F7/F8 仍未处置。
- [质量门禁操作手册](standards/QUALITY_GATE_OPERATIONS.md)：第 2 节渲染回归说明补文字排版断言与规范链接，配置清单补 `typography.mjs` 与「存量登记已清空」。
- [文档书总目录](README.md)：任务阅读路由新增「改飞书群机器人卡片的文案、按钮或排版」一行；交付文档清单登记 [卡片文字排版规范](standards/CARD_TYPOGRAPHY.md)。
- 验证：纯文档改动，未触碰代码；所有链接均为仓内相对路径且目标文件存在（`typography.mjs`、`CARD_TYPOGRAPHY.md`、审核记录均已在本分支落地）。
...
- 该文件与截图同在 `.quality-gate/reports/` 下，属门禁产出物、不入库；需要留档时打开报告目录即可。
- 验证：`node scripts/quality-gate/card-render/run.mjs` 退出码 0（17/17 通过、2 项存量登记），`gallery.html` 正常生成。

## 2026-09-12｜docs(review): 飞书群机器人卡片按钮全量审核，技术核查结论为不通过

- 触发：用户要求「前端的按钮重点检查消息记录，然后判断是否通过审核」。锁定对象为**飞书群内机器人发出的卡片消息**（群聊消息记录），先核实 Web 前端全仓无「消息记录」字样后与用户确认口径。
- 审核范围：13 张卡片构建函数（每日推荐 / 同步异常 / 重大变化 / 项目群职位卡 ×2 / 找人结果卡 ×2 / Reloop 短名单 / 候选人卡 / 决策群首卡 / 决策报告 / 进群绑定 / 使用指引 / 项目提醒 / 阶段提醒 ×3）。
- **结论：不通过（需修后再审）**。三按钮结构与回调链路符合 T10 且经 D5 生产实证，但存在两类功能级缺陷：
  - **F1 主次颠倒**：候选人卡与决策群首卡把 `primary` 给了纯跳转按钮（「查看链接」「查看 TTC 人才」），真实业务动作（「初筛通过」「一键加入人才库」「生成报告」「更新报告」）反而是 default 灰按钮。
  - **F2/F3 按钮文字被截断**：每日推荐卡「接单并建群」、找人结果卡「重点关注」被省略号吃掉，顾问读不到按钮名；两处已由新增的[卡片渲染回归门禁](2026-09-12-feishu-card-render-gate.md)硬捕获并登记（2026-09-20 到期）。
- 次要问题：F4 单按钮独占一行共 8 处；F5 项目卡 3 段 action 渲染成 3 行、层级被拉平；F6「回放」按钮文案与 T1「回放 tab 已下线」口径冲突；F7 emoji 风格不统一。
- 文档：[飞书群机器人卡片按钮审核](frontend-reviews/2026-09-12-feishu-card-buttons-audit.md)；[前端审核台账](frontend-reviews/README.md) 新增中央状态行与链接。
- 验证：纯文档改动，未触碰代码与部署；卡片缺陷证据来自渲染门禁实跑截图与构建函数代码位置。
...
- 前置核实：PR 侧 CI 两项均通过（`quality-gate` / `docker-build`）；GitHub 判定 `CONFLICTING`（PR 落后 origin/main 10 个提交）；本地 `main` 领先 `origin/main` 4 个提交。用 `git merge-tree` 预演出 **13 个冲突文件**，全部落在插件声明、工具注册表、启动卡与测试夹具——形态是「两边各自新增不同模块」，属可并集型，不是语义冲突。
- 并集规则：①`src/agent-gateway/tool-registry.js` 同时保留 PR 的 `createCandidateReportToolHandlers` + `reportDependencies` 与本地的 `sendInteractiveCard`、`brainx_bind_group_project`、`brainx_launch_project_chat`；②插件三件套（`openclaw.plugin.json` / `runtime.js` / 生产模板 `deploy/openclaw/openclaw.production.json` / 契约夹具）工具名单取并集，`brainx_candidate_report` 与两个本地新工具并存；③插件版本本地 `1.4.1` 与 PR `1.3.10` 合并后 **bump 到 1.4.2**（新增工具必须换版本号）；④迁移：本地 `0049_group_scope_job_action.sql` 与 PR `0049_candidate_reports.sql` **编号重复但按文件名记账**（`schema_migrations` 以文件名为键，`0017` 已有先例），两个文件都保留，迁移清单断言 51 → **53**。
- 唯一语义冲突 `src/project-launch.js`：PR 给找人按钮加了 `[BRAINTEX_SEARCH_START]` 标记（插件据此立刻回一条群状态）与「任务返回后结束本轮、不要原地轮询」纪律；本地在同一函数里已演进 specs/014 的接单门控与 Reloop 按钮。裁定＝**异步入口（OpenMai / SuperMai / 按条件找人）带标记 + 不轮询；Reloop 走同步读取 `brainx_candidate_shortlist`，立即返回，不加标记也不加等待话术**——这也解释了 PR 的识别正则为何只覆盖 `OpenMai|SuperMai`。本地卡片结构与未接单/已接单分岔全部保留。
- 测试断言同步（按合并后代码实测，不沿用任一侧旧值）：网关工具数 26 → **27**、插件清单 25 → **26**（差一个是内部工具 `brainx_send_candidate_resume`，按设计不进插件声明）、`plugin_version` → **1.4.2**；`tests/project-launch.test.mjs` 保留「未接单只给接单按钮」的断言，另补一段已接单卡片断言（4 个按钮及顺序、仅 OpenMai/SuperMai 带标记、Reloop 不含等待话术）。
- 改动文件（冲突面）：`src/project-launch.js`、`src/agent-gateway/tool-registry.js`、`plugins/brainx-openclaw/{runtime.js,openclaw.plugin.json,package.json}`、`deploy/openclaw/openclaw.production.json`、`tests/{project-launch,openclaw-plugin,framework,agent-gateway-http,agent-golden-workflow}.test.mjs`、`tests/fixtures/openclaw-production/plugin-contract.json`、`docs/AGENT_COMMIT_LOG.md`。
- 验证：受影响专项 **82/82 通过**；`node --check` 与 4 个 JSON 解析全部通过；完整门禁结果见本提交后的新鲜报告。
- 未做／待跟进：①PR #20（Storybook 组件库，8/26）本轮未合并（按用户确认只处理 #60）；②`prompt.js` 仍只描述「OpenMai 找人 / SuperMai 找人」两个入口，未提到 Reloop，属本地 main 既有缺口，本轮未改；③「按条件找人」按钮实际走异步 OpenMai 搜索，但没有即时群状态（其指令文案不匹配现有识别正则），同上属既有缺口。
...
- 验证：专项回归通过；完整门禁结果见本提交后的新鲜报告。

## 2026-09-10｜fix(找人): 点击后立即回显搜索状态

- 根因：OpenClaw 对卡片动作先加平台回执表情，Agent 又被旧提示要求在同一轮持续轮询，项目投递 worker 的自动回群事实未写入工具响应，导致用户长时间看不到明确进度。
- 修复：插件在收到项目找人卡片命令后直接向原群幂等发送“正在处理找人请求，通常需要 3–5 分钟”；兼容已发出的旧卡片。Agent 触发异步搜索后立即结束本轮，候选结果继续由 BrainX worker 自动投递。
- 验证：新增命令解析、飞书群投递、幂等、失败隔离和卡片命令回归；专项、快速、完整门禁及本地 OpenClaw 重启结果见本提交后续证据。

## 2026-09-10｜merge(main): 合并自建岗 TTC 真身与判据降级

- 合并：纳入远端 `main@83d52f9` 的自建岗位 TTC 真身绑定与判据降级，保留真 TTC 职位的原有失败语义。
- 保留：本分支的接单卡稳定签名、一键建群、继续找人轮询、重点关注与 Offer 报告闭环全部保留；代码无同文件冲突。
- 验证：合并提交后重新执行签名专项与上传前完整门禁，结果以后续新鲜报告为准。

## 2026-09-10｜fix(推荐): 保持本地接单卡签名稳定

- 真机根因：本地工作台每次启动都临时生成新的 `BRAINX_FEEDBACK_SECRET`，已发送卡片仍带旧签名，重启后点击会在建群前被“签名无效”拦截。
- 修复：正式环境继续强制独立持久密钥；仅当显式允许 HTTP 回环且 `BRAINX_BASE_URL` 确认为本机地址时，从已有 `BRAINX_DEV_AUTH` 做域隔离稳定派生，普通本地重启不再轮换卡片签名。
- 真机验证：向 Dykes 重发真实职位卡，并调用卡内同一接单 URL；HTTP 200，项目 `JGHTW36` 进入 `READY`，项目群与初始卡消息均已落库。旧临时密钥卡不可恢复，须使用重发卡。
- 验证：补充本地派生稳定性、凭证轮换失效和公网禁止降级回归；专项、快速与完整门禁结果见本提交。

## 2026-09-10｜chore(质量): 收紧服务端行数基线

- 调整：远端主线合并后 `src/server.js` 已缩短至 632 行，将存量超限登记从 649 同步收紧到实际值，防止后续无意回长。
- 边界：本提交只更新质量基线和提交记录，不改变任何运行时业务逻辑。
- 验证：合并后的首次完整门禁 24 项中 23 项通过，仅该基线检查要求收紧；修正后重新执行快速与完整门禁。
...
- 修复：按钮命令、系统上下文、工具描述和运行中响应统一要求——只有首次启动传 `true`，所有轮询必须改为 `false` 或省略；继续保留 BrainX 自动排除历史 TTC 编号的边界。
- 验证：专项测试与快速门禁结果见本提交；修复后的真实下一轮会产生费用，待后续获准岗位验收时复测。

## 2026-09-10｜fix(找人): 阻止条件消息重复启动搜索

- 真机现象：顾问先发送 `找人条件：……`、再点击“OpenMai 继续找人”后，第二轮完成又产生第三轮任务；当时先增加条件消息不得主动触发的防御性约束，并修复第二轮卡片标题仍错误沿用“首轮”的确定问题。
- 后续纠正：完整通道日志只记录到一次继续按钮点击，第三轮真实根因是 Agent 轮询时重复传 `continue_search=true`，已由后一独立提交修复；本条不再把条件消息误记为已证实根因。
- 验证：专项测试与快速门禁通过；轮次标题需在下一次真实搜索投递时复测。

## 2026-09-10｜fix(推荐): 修复一键接单链接双斜杠

- 真机复现：Dykes 收到 5 职位原生卡，点击“接单并建群”后因 `BRAINX_BASE_URL` 自带尾斜杠，链接变为 `//api/v1/feedback/quick` 并返回 404，未产生接单或建群写入。
- 修复：生成签名链接前统一去除基址尾部斜杠；同步回归、中央审核台账、单次审核记录、施工清单与 Storybook 边界说明。
- 验证：`node --test tests/feedback-loop.test.mjs` 10/10 通过；修复后真实建群与重复点击仍在本轮继续复测。

## 2026-09-10｜feat(推荐): 卡片一键接单并创建项目群

- 卡片：每日推荐的每个职位增加“接单并建群”，同时保留查看职位、回放和忽略；缺签名密钥时安全降级为打开工作台。
- 闭环：签名点击自动加入本人项目，复用既有确认、接单、唯一项目群、OpenClaw 群准入和职位投放；重复点击不重复建群，群内仍需显式选择 OpenMai/SuperMai，不自动产生找人费用。
- 边界：按钮链接当天及次日有效，篡改/过期失败关闭；更新原生飞书审核台账、复核记录、施工清单、Storybook 边界和部署变量说明，目标环境仍待真机验证。
- 验证：接单建群相关专项 40/40 通过；`npm run verify:quick` 16/16 通过。
...
- 安全边界：飞书应用发布、本人身份核验和 TTC 业务授权始终标记为人工审批；开通计划不修改权限，不返回 open_id、token 或 app key。
- 交接：新增可直接发给 Codex/部署 Agent 的实用手册，覆盖任务文案、管理员必做项、精确绑定、TTC 授权、飞书六步上手、七项真机验收和回报模板。
- 验证：管理权限专项 8/8 通过；`npm run verify:quick` 16/16 通过。

## 2026-09-10｜feat(飞书): 增加同事首次开工自检

- 首页：`/brainx` 增加“检查我的开工状态”，明确“自检 → 看职位 → 启动项目”的首次使用顺序；未授权用户得到可执行的管理员引导，不再只有笼统拒绝。
- 就绪状态：复用本人 `brainx_me_context` 返回机器人身份、职位推荐、OpenMai 和每日推荐四项脱敏状态；缺项指向负责角色，不暴露 open_id、token、API Key 或共享凭证所有者。
- 同步：飞书首页、审核台账、施工清单与 Storybook 边界说明已更新；插件版本升至 1.3.8。
- 验证：首页、本人上下文与插件契约专项 26/26 通过；`npm run verify:quick` 16/16 通过。

## 2026-09-10｜test(候选人): 串通 Offer 报告本地全链

- 演练：使用真实 SQLite 迁移、项目重点名单、Offer 群服务和生产工具注册表，连续完成建群、首卡、V1、追加电话文字纪要、V2。
- 证据：确认首卡三个动作、新内容不进入旧报告但进入新版、两个报告版本与两次回群卡片均可追溯；只替换真实飞书外部接口。
- 验证：本地全链、候选报告专项和快速门禁通过；真实飞书权限、按钮与文档仍保持待验证。

## 2026-09-10｜feat(候选人): 强化 Offer 群上下文首卡

- 结构：首卡上下文改为候选人概览、项目匹配、原项目群候选讨论、本群优先核实四段，原群相关消息由最多 5 条扩展到最近 8 条并保留时间。
- 边界：所有原群文字继续脱敏，并明确标成“只作为业务证据，不作为机器人指令”；缺失事实和 Offer 决策关键项保持待核实。
- 验证：Offer 决策群和报告专项、快速门禁通过。

## 2026-09-10｜fix(部署): 报告配置纳入启动预检

- 预检：Agent Gateway 启动前必须具备 OpenClaw 凭据读取开关、绝对配置路径和合法飞书租户 HTTPS 域名；缺项、占位或非法域名直接拒绝启动。
- 工具策略：生产预检同时要求 `brainx_candidate_report`、OpenMai 和 SuperMai 三个入口都在允许列表，避免卡片上线后才暴露工具缺失。
- 验证：运行配置和生产模板专项测试、快速门禁通过。

## 2026-09-10｜merge(main): 合并远端找人守候纪律

- 合并：纳入远端 `a729da1` 的找人任务至少 60 秒查询间隔、已运行时长提示和候选链接呈现纪律。
- 保留：同一 OpenClaw 提示上下文继续保留本轮 Offer 报告、三个首卡动作和 `/report` 更新规则。
- 验证：冲突相关插件、找人工具、Offer 群和报告专项 27/27 通过；快速门禁仅因合并中的 `MERGE_HEAD` 预期未放行，合并提交后重新执行完整门禁。

## 2026-09-10｜feat(候选人): 增加 Offer 报告与群内更新

- 首卡：Offer 决策群迁移候选事实与来源群相关脱敏讨论，提供查看 TTC 人才、生成报告、更新报告三个动作。
- 报告：新增当前决策群限定的 `brainx_candidate_report` 与 `/report`；飞书云文档按决策摘要、候选事实、来源上下文、新增证据、Offer 检查和证据边界排版，每次生成新版本。
- 安全与部署：工具不接受模型传入对象 ID 或 URL，联系方式、邮箱和简历原文不进入报告；生产需发布飞书文档权限并配置租户 HTTPS 基址。插件版本升至 `1.3.7`。
- 验证：迁移、首卡动作、脱敏、报告版本、飞书文档写入、插件与生产白名单专项 43/43 通过；`npm run verify:quick` 16/16 通过。完整门禁留到 push 前执行。

## 2026-09-10｜feat(候选人): 重点关注同步发送人才卡
...
- 对话建群：顾问当轮明确说“为这个人建群”或点名候选人要求建决策群即构成确认；候选人唯一明确时直接执行，未保留候选人自动进入项目重点名单后复用原有幂等建群和脱敏上下文迁移。指代可能对应多人时只追问姓名。
- 安全：后端自行根据稳定候选编号恢复 TTC 链接，不接受模型传入 URL；人才卡不含联系方式、简历原文或 PDF，只能发到职位来源项目群。错误群建群会在写入重点名单前失败，不留下半成品。OpenClaw 插件版本升至 `1.3.6`，插件与网关动作契约同步。
- 验证：候选卡片、自然语言建群、自动重点名单、链接、脱敏、来源群、失败无副作用和插件契约专项 29/29 通过；`npm run verify:quick` 16/16 通过。

## 2026-09-09｜fix(找人): 合并后保留失败提示与守候纪律

- 根因：本地双找人入口与远端守候纪律合并后，启动失败分支只保留了重试提示，未继续说明成功重试后的预计收敛时长和查询节奏，触发守候纪律回归失败。
- 修复：OpenMai 与 SuperMai 的启动失败提示继续如实说明“本轮未启动”，同时补充成功重试后通常 3-5 分钟收敛、约每分钟读取进度；成功触发分支仍保留项目群自动回传说明。
- 验证：冲突相关专项测试通过后运行 `npm run verify` 完整门禁；最终结果以本条后续验证记录为准。
...
- 布局：扩大卡片动作列并允许窄屏安全换行，详情固定放在动作组最右侧；忽略、阶段行动和详情不再复用同一个按钮语义。
- 本地实证：重建并重启本地生产前端，10 个真实项目均出现常驻详情；`P294569` 点击详情后正确显示职位事实、真实职位编号、主做顾问、HC 和备注。
- 验证：Storybook 21 个文件、85 个场景全部通过；生产构建通过；`npm run verify:quick` 16/16 通过。
...
- 闭环：只有来源职位项目群中的已授权发送人，才能为已经“保留”的候选人幂等建群；新群加入同项目已绑定协作者、登记 OpenClaw 与 BrainX 群范围，并投放围绕继续评估、面试或 Offer 的开场卡。
- 上下文与安全：新增 `candidate_decision_groups` 可恢复状态；摘要包含重点候选安全快照及原群最近明确提及该候选人的最多 5 条消息，电话和邮箱会被清除，不迁移简历原文、其他候选讨论或完整群历史；工具响应不向模型返回飞书群 ID。
- 验证：候选决策群、迁移、幂等、来源群限制、重点名单前置、候选卡和插件契约专项 49/49 通过；`npm run verify:quick` 16/16 通过。
...
- 安全与费用：OpenMai 的安全候选编号可进入排除集；SuperMai 只有带官方 TTC 人才链接的编号才进入。非法、重复或无法确认的编号不使用；完全没有可确认编号时返回 `cannot_continue`，不启动任务。项目级运行中守卫和跨顾问轮次保持生效。
- 契约修复：检查发现上一提交只更新了 OpenClaw 插件参数，BrainX Agent Gateway 仍是旧白名单，真实 `criteria/job_id` 会被拒绝；本提交同步网关与插件契约并加入精确一致性回归，插件版本升至 `1.3.3`。
- 验证：迁移、排除累计、跨顾问第二轮、无编号失败关闭、提示词传递、结果卡按钮、项目启动及插件—网关契约专项 108/108 通过；`npm run verify:quick` 16/16 通过。

## 2026-09-09｜feat(项目群): 接入双找人入口与可选条件

- 改动：飞书项目卡新增“OpenMai 找人”和“SuperMai 找人”两个真实按钮；顾问可先在同群发送 `找人条件：……`，点击后将最新一条此类顾问消息作为本轮补充条件，未填写时 OpenMai 使用职位事实、SuperMai 自动生成判据。
- 闭环：OpenMai 修复补充条件未真正进入请求的问题；SuperMai 项目模式使用真实项目编号落库，复用既有 worker 把结果投递回原项目群。项目已有运行中或已完成的共享搜索时，其他协作者不会重复触发任务和费用。
- 边界：锁定版 OpenClaw 的卡片回调尚未把表单输入值传给 Agent，因此本轮使用群消息条件入口，不伪造不可执行的卡片输入框；内联输入、下一轮 TTC 编号排除名单、候选收藏/保留和真实飞书真机验收仍为后续任务。
- 验证：找人入口、插件契约、项目卡片、OpenMai 条件请求、SuperMai 项目落库及结果投递专项回归 57/57 通过；跨顾问共享搜索专项 12/12 通过；`npm run verify:quick` 16/16 通过。

## 2026-09-09｜fix(项目群): 建群后等待选择找人方式

- 根因：正式项目启动流程把“创建飞书项目群”和“启动 OpenMai”绑定为一次动作，导致顾问尚未选择找人渠道或补充条件时就可能产生不准确结果和搜索费用。
- 改动：建群、职位卡投放和接单完成后统一返回 `awaiting_method`，不再检查 TTC 凭证、调用 OpenMai 或自动重试旧搜索；工作台与项目群职位卡明确提示后续选择 OpenMai 或 SuperMai。历史正在运行或已完成的共享搜索仍按原状态复用，不重复扣费。
- 边界：本提交只完成“停止自动搜索”的第一项任务；群内双按钮、可选条件输入、轮次排除名单和候选人保留状态均未提前实现，已在审核记录与施工清单中标为待办。
- 验证：项目启动与生产配置回归 19/19、前端静态与适配测试 44/44、Storybook 交互测试 85/85、`npm run verify:quick` 16/16 通过；完整门禁中的后端 602/602、生产构建、Storybook 构建、浏览器链路与服务烟雾测试均通过。完整门禁仍受存量前端依赖审计阻断（1 个中危、1 个高危、1 个严重）；首次运行时另检出并已修复本任务造成的旧 Storybook 文案断言。

## 2026-09-09｜fix(feishu): 整理人才表格并直达 TTC 详情

- 根因：人才搜索长文本经通用消息链路后把反斜杠、Markdown 表格分隔符和 HTML 空格实体直接展示在飞书中；旧总览还保留逐人发送简历与 PDF 附件路径，不符合最新交互要求。
- 改动：筛选说明保留为纯文字，候选人统一渲染为飞书原生整洁表格，列出姓名/岗位、经验/城市、学历、核心匹配点、匹配度，并在每行最右侧放置只打开 TTC 人才详情的“查看人才”。OpenMai 自动投递停止逐人话题与 PDF，OpenClaw 生产插件和 allowlist 移除发送简历工具，插件版本升至 1.3.1；第三方或无效链接不生成按钮。
- 文档：同步招聘闭环、工具外露白名单、三类找人 Skill、Storybook 说明、施工清单与前端审核台账，并新增 2026-09-09 复审记录；真实飞书桌面/手机视觉及目标环境 TTC 登录跳转仍待验收。
- 验证：相关后端、插件、配置与 Skill 回归 65/65、`npm run verify:quick` 16/16 通过。受控环境完整门禁 24 项中 23 项通过：后端 602/602、前端 44/44、Storybook 85/85、生产构建、浏览器链路和烟雾测试均通过；唯一失败为存量前端依赖审计（1 个中危、1 个高危、1 个严重），需要单独升级 Next.js 等依赖，本任务未扩大范围且不允许 push。
...
## 2026-09-08｜test(openmai): 对齐团队授权提示断言

- 根因：完整门禁发现 TTC 团队授权上线后的真实缺凭据提示已变为“没有个人或已授权的团队 TTC 寻访凭证”，旧回归仍断言历史文案。
- 改动：测试改为精确核对当前提示，不放宽无凭据时禁止网络请求和失败关闭的安全断言。
- 验证：单项回归通过；完整门禁将于本提交后重新运行。

## 2026-09-08｜fix(openmai): 增加岗位画像补充入口

- 根因：OpenMai 对信息不足的测试职位只返回“请选择岗位画像 / 补充核心搜索条件”，工作台没有输入入口；后端又把任意非空回答当作成功，飞书因此错误显示“首轮候选人已就绪”。
- 改动：跟进页增加画像输入与显式重跑按钮，画像随最新任务保存并进入 OpenMai 提示词；短澄清回答落为 `needs_input`，群内改发橙色补充提示，项目不再误标完成。抽出 OpenMai 路由、结果判定和前端面板，收紧存量文件行数基线。
- 安全：画像限制 2000 字并明确按不可信业务数据处理；职位可见性、承接状态、TTC 授权和重复费用守卫保持不变。
- 验证：专项 44/44、前端静态 44/44、快速门禁 16/16 通过；本机正式工作台确认输入框和按钮可见。真实向 OpenMai 发送画像仍需用户单独确认。

## 2026-09-08｜feat(feishu): 支持逐人查看并发送 TTC 简历

- 根因：OpenMai 真实结果不提供简历 URL，原卡片无法发送附件；逐行按钮初版又把 action 嵌入 column，被飞书以 `200410` 拒绝。按钮回调本身不含 @ 元数据，随后还会被群 mention 门丢弃。
- 改动：每名候选行后增加独立“查看 / 发送简历”；点击后优先用 OpenMai URL，否则按候选编号实时回查 TTC 附件，安全下载并幂等发送到当前项目群。只为已登记项目群关闭 mention 门，其他群维持全局 @ 限制；新增工具、契约、30 秒附件动作窗口和安全错误码。
- 安全：TTC Bearer 只用于 API 首跳，不跟随至文件域；只接受官方 HTTPS 主机、12 MiB 以内且通过 `%PDF-` 内容校验的文件；工具继续校验明确确认、群聊、sender、职位与候选范围，PDF 不进入模型上下文。
- 验证：专项 32/32 与快速门禁 16/16 通过；真实项目 `JCU85O0` 的 9 行、9 组双按钮卡片发送成功，点击谢伟峰后 TTC 回查并向原群发送 `169.2 KB` 真实 PDF，飞书显示附件与“简历已发送”，审计状态 `SUCCEEDED`。手机、无附件提示和服务器发布仍待验证。

## 2026-09-08｜fix(feishu): 整理候选人表格卡片

- 根因：OpenMai 的面向人 Markdown 表格被直接塞进飞书卡片，客户端不会按表格渲染，导致竖线、分隔符和长文本裸露；同批无附件候选人又收到逐人卡，群时间线冗长。
- 改动：结构化候选人统一呈现为“候选人 / AI 初评”双列表格，总览卡末尾只保留一个工作台按钮；没有真实 PDF 时不再发送重复逐人话题，有真实 PDF 时仍保留安全附件话题。
- 验证：OpenMai 投递专项 17/17 通过；用本机真实项目的 9 名候选向真实飞书项目群发送新版预览成功，返回 10 行表格元素和 1 个动作。用户视觉复看、窄屏和真实 PDF 仍待验收。
...
- 改动：增加显式本机开关，从既有 OpenClaw 配置读取同一机器人 App 凭证；调用者显式传入凭证时仍优先使用，缺文件或字段继续失败关闭，不复制或提交密钥。
- 真机证据：对齐应用后，Dykes 工作台成功创建 `海明微半导体-功率模块开发` 群并由机器人投放职位卡；群按 `MENTION_ONLY`、Dykes sender 和项目 scope 登记，OpenMai 因缺 Dykes 本人 TTC 凭证保持未启动。
- 验证：飞书机器人专项 8/8、快速门禁 16/16 通过。

## 2026-09-08｜fix(settings): 增加本地账号切换入口

- 根因：工作台设置中心只展示当前身份，没有真实退出入口；浏览器曾以 Mia 开发会话登录后，用户无法切换到本轮要求的 Dykes 本地验收身份。
- 改动：个人资料页增加“退出并切换账号”，调用真实会话退出接口后刷新；本地开发回到花名册身份选择，正式环境回到飞书授权入口，不改任何项目或凭据数据。
- 验证：设置中心 Storybook 验证 logout 动作；前端静态测试验证退出请求与刷新路径，快速门禁通过后提交。

## 2026-09-08｜fix(project): TTC 未连接时先创建飞书项目群

- 根因：“建群、投放职位、启动 OpenMai”共用一个 TTC 前置门槛，导致顾问即使飞书身份和应用权限都已就绪，也无法先创建项目群。
- 改动：明确确认后先幂等建群并投放职位；缺本人 TTC 时返回可恢复的 `credentials_required` 状态，页面显示群已就绪并提供“启动 OpenMai 找人”。连接 TTC 后再次点击复用原群，不重复建群。
- 验证：项目启动专项覆盖缺确认零外部写入、缺 TTC 只建一次群且不调用搜索；前端静态与 Storybook 增加群已就绪状态，快速门禁通过后提交。

## 2026-09-08｜fix(deeplink): 放行显式启用的本机回环链接

- 根因：本机工作台使用 `http://127.0.0.1:3000`，项目群预检沿用生产 HTTPS 硬门槛，导致真实本机联调在建群前被错误阻断。
- 改动：新增 `BRAINX_ALLOW_HTTP_LOOPBACK=1` 本机开关，只允许 `127.0.0.1` 与 `localhost` 的 HTTP 深链；局域网地址、普通 HTTP 域名、危险协议和带凭据 URL 仍失败关闭，生产 HTTPS 规则保持不变。
- 验证：深链专项覆盖显式放行与非回环拒绝；快速门禁通过后提交。

## 2026-09-08｜fix(openclaw): 将自然语言推荐路由到真实职位

- 根因：BrainTex 插件只注册工具、没有提供业务语境；模型收到“推荐三个职位”时可能按通用职业规划回答，完全绕过顾问授权数据。
- 改动：为飞书 Agent 注入静态 BrainTex 业务规则，将“推荐三个”“推荐职位”“今天先做什么”默认路由到 `brainx_daily_brief`，并明确工具失败时不得用泛化建议冒充真实结果；同时保留找人及写动作的确认边界。
- 验证：新增飞书语境注入和非飞书隔离回归；OpenClaw 插件专项与快速门禁通过后提交。

## 2026-09-08｜fix(openclaw): 恢复团队共享默认模型

- 决策：按最新产品要求恢复服务器托管的公司 StepFun 模型；六名白名单顾问无需在个人电脑配置 API Key，首次私聊创建的隔离 Agent 继承 `stepfun/step-3.5-flash`，失败时回退 `stepfun/step-3.7-flash`。
- 安全：公司凭据仅通过 `/etc/brainx/openclaw.env` 的 `STEPFUN_API_KEY` SecretRef 注入；个人模型继续作为本人私聊的可选覆盖，停用后回退公司默认，群聊绝不借用个人密钥。
- 部署：升级安装器不再删除公司默认模型；启动预检要求公司 Key 存在、非占位且长度合格，配置不完整时失败关闭，避免“服务 active 但所有员工都无法问答”。
- 验证：共享模型/运行配置/个人模型专项 19 项中 19 项通过（其中 3 项在获准的本机临时端口环境复验）；`npm run verify:quick` 16/16、前端静态与适配测试 42/42、`git diff --check` 通过。
...
- 修复：删除 CI 对宿主机非 UTC 时区的错误假设；保留 `mysqlLocalDatetime` 对 MySQL 墙钟值的直接断言，使同一测试在 UTC 与 Asia/Shanghai 环境均有效。
- 安全：不接纳跨顾问 TTC JWT 兜底链，继续要求每位顾问使用本人凭证；新增回归测试，证明他人已有有效凭证时仍会失败关闭且不会发起外部请求。
- 验证：受影响专项 51 项中 46 项直接通过，5 项仅因沙箱禁止临时端口失败，放开本机临时端口后 7/7 通过；快速门禁除合并过程必然存在的 `MERGE_HEAD` 外，其余 15/15 项通过；完整门禁在合并提交后执行。

## 2026-09-08｜fix(feishu): 保证一个职位只启动一个共享项目群

- 根因：项目启动记录按“顾问 + 职位”读取和约束，第二位协作者点击同一职位时会另建群、覆盖职位 `chat_id`，并可能再次触发付费搜索；协作者在“我的项目”中也看不到已存在的共享群状态。
- 改动：将项目群读取、项目摘要和 OpenMai 回群统一改为按职位复用唯一群；数据库触发器阻止新增重复记录，历史冲突失败关闭；首轮发起人保留重试责任，其他协作者复用运行中或已完成的搜索且不要求重复连接 TTC、不重复触发 OpenMai。
- 验证：新增第二协作者复用、数据库重复写阻断、共享搜索零重复调用及跨执行人回群回归；专项测试 48/48、`npm run verify:quick` 16/16、`git diff --check` 通过。

## 2026-09-07｜fix(openmai): 释放服务重启遗留的僵死搜索

- 根因：OpenMai 调用在 API 进程内异步执行，服务中途重启会遗留永久 RUNNING；工作台在该状态禁用按钮，业务无法自行恢复。直接自动重跑又可能和仍在上游执行的任务重复计费。
- 改动：业务 worker 每轮只把超过一小时（大于完整调用与异步轮询最大窗口）的运行任务原子转为失败，保留原 task_id 并生成明确重试提示；随后沿既有持久投递链回群，由顾问决定是否再次付费搜索。
- 验证：新增超时释放、窗口内不误判与幂等回归；OpenMai 投递、任务和项目启动专项 28/28、`npm run verify:quick` 16/16、`git diff --check` 通过。
...
- 改动：统一权限清单、部署手册和招聘闭环说明：当前版本必须确认并发布 `im:chat`/后台对应建群写权限，项目 Sheet 和群全量读取仍后置；同步记录候选数量、真实送达与无重复计费的最新边界。
- 验证：文档链接与 `git diff --check` 通过，`npm run verify:quick` 16/16 通过；前一功能提交完整门禁 24/24、后端 574、前端 42、Storybook 83 均通过。

## 2026-09-07｜fix(feishu): 结果真实送达后才标记寻访完成

- 根因：OpenMai 结果一进入投递队列，项目就提前显示 DONE；即使飞书发送连续失败，用户仍会看到“候选人已回传项目群”。重试按钮又会复用已完成搜索，既不能恢复投递，也可能诱导错误补搜。
- 改动：新结果排队时保持运行态，只有摘要、候选话题和附件完成后才写 DONE；连续五次投递失败改为稳定错误码和可操作提示。用户明确重试时重置原投递队列，不重新调用 OpenMai。
- 边界：OpenMai 本身失败会立即显示失败，不等待错误卡送达；候选不足继续按独立质量门槛进入可重试状态。
- 验证：新增五次投递耗尽与零额外搜索回归；OpenMai 投递、任务与项目启动专项 26/26、`npm run verify:quick` 16/16、`git diff --check` 通过。

## 2026-09-07｜fix(openmai): 完整处理无尾空行的流式结果

- 根因：OpenMai SSE 的最后一帧若随连接关闭直接结束、没有额外空行，旧代码只读取 `deferred`，会漏掉最终正文、会话/消息编号和错误，进而误报“没有读取到会话结果”或静默吞掉上游失败。
- 改动：把所有完整帧和连接尾帧统一交给同一解析器，完整处理 session、增量/替换正文、最终 canonical 正文、异步定位和错误；无效 JSON 仍安全忽略。
- 验证：新增尾帧正文、异步定位和错误回归；OpenMai 投递、任务与项目启动专项 24/24、`npm run verify:quick` 16/16、`git diff --check` 通过。

## 2026-09-07｜fix(openmai): 防止迟到任务覆盖最新寻访结果

- 根因：OpenMai 终态原来只按顾问和职位更新；旧进程中的超时任务若在新一轮搜索启动后才返回，会把最新任务误写成旧任务的成功或失败，并发出错误完成通知。
- 改动：成功和失败落库同时校验当前 `task_id` 与 `running` 状态，只有仍为当前运行任务的回调才能写终态；已被新任务替代或已完成的迟到回调不改库、不发 SSE。
- 验证：新增旧任务/迟到回调回归；OpenMai 投递、任务与项目启动专项 22/22、`npm run verify:quick` 16/16、`git diff --check` 通过。

## 2026-09-07｜fix(openmai): 候选不足时保留结果并要求明确重试

- 根因：6–10 人只存在于提示词，OpenMai 即使只返回 1–5 名结构化候选人，系统仍会把整批标成完成，工作台和项目群都无法区分“结果已返回”与“首轮数量已达标”。
- 改动：对新机器块结果执行 6 人最低质量门槛；不足时仍投递已有候选人话题和简历，但摘要卡显示橙色告警，项目进入带稳定错误码的可重试状态。用户再次明确点击后才强制发起新任务。
- 费用与兼容：不会自动补搜或隐藏增加 OpenMai 消耗；旧历史结果没有机器块时保持原状态，避免上线后批量误判存量数据。
- 验证：OpenMai 投递与项目启动专项 20/20、`npm run verify:quick` 16/16、`git diff --check` 通过。
...
- 改动：非法地址、非 PDF 或超限等确定性附件异常改为在对应候选人话题内发送幂等“简历附件待核验”卡，保留该人评估并继续处理后续候选；新增通用飞书互动卡话题回复能力。网络或飞书暂时故障仍保留恢复性重试。
- 安全与可靠性：异常卡不含下载地址、底层错误或凭证信息，姓名继续脱敏联系方式；只有异常提示也无法送达时才保留整批恢复性重试，避免静默丢附件。
- 验证：飞书发送与 OpenMai 投递专项 17/17、`npm run verify:quick` 16/16、`git diff --check` 通过。

## 2026-09-07｜feat(feishu): 自动拉取已绑定项目协作者入群

- 根因：旧自动建群只包含发起人和机器人，群 Agent scope 也只允许发起人；真实猎头项目由多名顾问协作，其他项目成员仍需手工入群且入群后不能使用机器人。
- 改动：创建职位群时自动查询同一职位的有效 `MY_JOB/TEAM_SHARED` 项目成员，把已完成 Agent 身份绑定的协作者一并加入，并同步写入该群允许的 sender 列表。
- 权限：协作者必须同时满足在职、项目关系有效、open_id 已核验、同租户、同飞书 account、同 App key 的 ACTIVE 绑定；不从昵称猜人，不邀请未绑定或跨租户成员。
- 验证：项目启动、飞书发送和 Agent 群授权专项 19/19、`npm run verify:quick` 16/16、`git diff --check` 通过。
...
- 改动：候选写动作同时接受本人授权职位下最新成功 OpenMai 结果中的结构化候选编号；首次加入后沿同一 `consultant_candidate_cases` 状态继续推进，话题卡显式展示非敏感候选编号供机器人准确调用。
- 权限：OpenMai 结果必须同时匹配请求顾问、职位和候选编号；职位可见性、明确确认、合法状态机继续强制，伪造编号、跨顾问和跨项目仍统一拒绝。
- 验证：候选流程、OpenMai 投递与 Agent 黄金链路专项 11/11、`npm run verify:quick` 16/16、`git diff --check` 通过。

## 2026-09-07｜feat(feishu): 按候选人话题沉淀评估与简历

- 根因：真实猎头项目群按候选人组织协作，简历、AI 判断和后续联系记录落在同一话题；旧实现把一批候选摘要和 PDF 平铺到群里，无法持续跟进单个人选。
- 改动：保留整批概览，并为机器块中的每名候选人发送独立评估卡；真实 PDF 使用飞书 `reply_in_thread` 回复进对应话题，卡片深链携带候选编号，无附件时明确标注而不伪造。
- 安全与可靠性：候选人卡继续清除电话和邮箱；话题卡和附件分别使用稳定业务 UUID，整批重试不重复铺消息；回复目标必须是合法飞书消息 ID。
- 验证：飞书发送与 OpenMai 投递专项 13/13、`npm run verify:quick` 16/16、`git diff --check` 通过。

## 2026-09-07｜test(migrations): 同步项目群三项迁移验收清单

- 根因：项目群、寻访状态和 OpenMai 投递已新增 0039–0041 三个迁移，但完整测试仍把权威文件清单与旧库升级总数固定在 0038/40 个，导致完整门禁 554 项中 2 项失败。
- 改动：迁移记账断言纳入 0039–0041，并把旧库兼容升级后的当前总数更新为 43；业务迁移本身未改动。
- 验证：迁移专项通过；完整 `npm run verify` 复跑通过后提交本记录。
...
- 改动：OpenMai 请求固定为 6–10 人并逐人输出匹配判断、推荐理由和风险；有真实附件时通过机器块返回 PDF 地址，worker 在摘要卡后逐份上传并以幂等文件消息发回原私有项目群。
- 安全：只接受配置的 TTC HTTPS 域，禁止跳转，单份上限 12 MiB 并校验 `%PDF-` 内容；无真实附件必须为 null，不得编造。机器块不进入群摘要，TTC JWT 只随受信域下载请求使用。
- 配置：worker 环境补齐受信简历域和同一飞书应用凭证；飞书权限清单补充 PDF 用途说明。
- 验证：飞书发送、OpenMai 投递与提示契约专项通过；`npm run verify:quick`、`git diff --check` 通过。
...
- 状态：项目列表返回持久启动摘要；前端展示创建中、OpenMai 找人中、结果已回群和失败重试，并把身份、凭证、权限或地址缺项以真实阻断原因反馈给当前用户。
- 审核：补充独立审核记录、中央台账、施工清单和 Storybook 场景；本机浏览器目视核对待开始与找人中布局，仍诚实保留“用户未审核、目标环境未发布、真实租户部分验证”。
- 验证：项目启动与归属专项 9/9、前端静态检查 42/42、Storybook 20 文件 83/83、Lint、`npm run verify:quick` 16/16 及 `git diff --check` 全部通过。

## 2026-09-07｜feat(openmai): 自动把候选结果回传原飞书项目群

- 改动：新增 OpenMai 群投递持久账本和 worker；自动发现项目群对应的成功/失败结果，生成飞书卡片并回到原群，项目启动状态同步更新为 DONE 或 FAILED。
- 隐私：群卡默认清除电话、邮箱和隐藏注释，候选人清单与简历链接保留；超长结果截断并引导到工作台查看完整内容，明确标注候选事实仍需顾问核验。
- 可靠性：每个 OpenMai task × chat 只创建一个投递，发送使用稳定幂等键；失败按指数退避最多尝试五次，服务重启后仍能继续，不再依赖内存回调。
- 验证：OpenMai 投递、项目启动、飞书发送与迁移专项 16/16 通过；`npm run verify:quick` 16/16 通过；`git diff --check` 通过。

## 2026-09-07｜feat(workflow): 一键启动项目群接单与 OpenMai 寻访

- 改动：项目启动接口升级为一次明确确认完成“创建/复用飞书项目群 → 接单 → 建立首轮行动 → 触发 OpenMai”；启动记录独立保存找人状态和任务编号。
- 体验：系统为首轮寻访生成固定、可审计的目标与下一行动，用户不再进入第二个表单重复填写目标、行动和期限。
- 安全与失败边界：未明确确认、本人 TTC 凭证未连接、项目已结束等情况均在启动前或对应步骤显式阻断；群就绪和找人状态分开保存，不把部分成功伪装成完整成功。
- 验证：项目启动、接单、membership 与迁移专项 22/22 通过（HTTP 测试经本机回环权限原样复跑）；`npm run verify:quick` 16/16 通过；`git diff --check` 通过。

## 2026-09-07｜feat(project): 打通职位创建飞书项目群并登记 Agent 范围

- 改动：新增项目启动状态表和 GET/POST 启动接口；已加入项目可创建飞书群、投放职位卡片、绑定 `project_id ↔ chat_id`、登记群消息上下文并为当前负责人激活职位/候选人/面试用途的群 Agent 范围。
- 可靠性：飞书调用前先持久化启动记录；建群和发卡分别使用稳定幂等键；群已创建但投放失败时保留 `chat_id`，重试只补发卡片和本地登记，不重复建群。
- 安全：缺项目归属、顾问 open_id、Agent 身份绑定、飞书应用凭证或生产 HTTPS 地址时，均在任何外部写入前阻断并返回可操作错误。
- 文档：同步更新招聘闭环权威说明，将 9 月 7 日用户决定记录为“自动项目群提升为当务之急”，并诚实保留真实租户权限与真机验收状态。
- 验证：项目启动、飞书发送与迁移专项 11/11 通过；`npm run verify:quick` 16/16 通过；`git diff --check` 通过。
