...
## 2026-09-12 · `test(提醒): 固定项目提醒测试时钟`

- 根因：项目提醒测试用实时系统日期生成活动时间，却用固定的 2026-09-10 作为扫描时间；日期跨过夹具基线后，活动会落在扫描时间之后，导致完整门禁稳定失败。
- 修复：测试统一使用固定时钟派生静默时间、近期活动、截止日期、周键和冷却期；生产提醒逻辑不变。
- 验证：项目提醒专项与完整门禁通过后，允许同步当前分支到远端。

## 2026-09-11 · `docs(飞书): 记录人才链接本机加载状态`

- 本机证据：OpenClaw 已加载 `brainx-openclaw` 1.3.18，BrainX Agent Gateway 监听 `127.0.0.1:3102`，飞书 WebSocket 已连接。
- 边界：未代替用户在真实项目群点击“初筛通过”产生新消息，因此自动链接展开仍保持待真机确认，不把安装状态误记为真实数据验收完成。

## 2026-09-11 · `fix(飞书): 为人才链接隔离幂等键`

- 修复：人才链接使用独立 `candidate-link-*` 消息 UUID，避免已投放过旧自制卡的候选人在升级后被飞书按旧 UUID 去重；同一职位、候选人与项目群后续重复操作仍只投递一次链接。
- 验证：候选动作专项回归覆盖新 UUID、纯链接正文与无自制卡载荷；快速门禁通过后提交。

## 2026-09-11 · `fix(飞书): 初筛后改发人才库链接`

- 行为：保留 `KEEP_FOR_REVIEW` 的项目共享重点名单写入；来源项目群不再接收 BrainTex 自制候选卡，而是仅接收一条受控 TTC 人才链接，由飞书现有链接展开规则生成经典人才卡。
- 兼容：保留 `SEND_TALENT_CARD` 动作名，避免现有按钮协议失效；动作实际结果改为人才链接状态与证据。
- 验证：飞书发送、候选动作、OpenMai 投递与 OpenClaw 插件专项 38/38 通过；`npm run verify:quick` 16/16 通过；真机链接展开在安装 1.3.18 后继续验证。

## 2026-09-11 · `fix(飞书): 对齐候选按钮真实协议`

- 真机根因：1.3.16 的候选行使用 `column_set`，回调值是普通 `{ text }`；锁定版 OpenClaw 飞书原生卡清洗器会丢弃 `column_set`，且回调只允许 `ocf1` 信封，导致真卡只显示标题和说明、整行候选人与按钮消失。
- 修复：1.3.17 改为清洗器明确支持的顶层 Markdown 候选摘要与顶层按钮；“初筛通过 / 收藏”使用 OpenClaw `ocf1` quick-command 信封，“查看人才”继续使用安全 `open_url`。
- 验证：专项测试 45/45、快速门禁 16/16 通过；本机重装 1.3.17 后，飞书桌面端完整显示三按钮，“收藏”回传结构化动作并回复“已收藏”，网关无工具调用，数据库重点名单与工作流日志均为 0 条。

## 2026-09-11 · `feat(飞书): 稳定职位摘要并补齐人才动作`

- 职位卡：OpenClaw 插件升级至 `1.3.16`，新推荐固定输出“职位简介 / 适合原因 / 需要确认”三段，禁止把 score、QOD 或内部参数当作正文；解析器继续兼容旧字段并统一归一化，避免模型措辞变化后退化成纯文本。
- 人才卡：OpenMai 正式投递和普通对话候选表均移除行首序号，每行显示“查看人才 / 初筛通过 / 收藏”；初筛通过复用受控重点名单和人才卡回群链路，收藏仅返回演示确认，不调用工具或写入后端。
- 状态：正式代码已接入，卡片整体视觉未审核，目标服务器与真实飞书新按钮尚未验证。
- 验证：飞书卡片、OpenMai 投递、OpenClaw 插件、表单桥和候选动作专项 45/45 通过；`npm run verify:quick` 16/16 通过；提交后在洁净工作区运行完整门禁。

## 2026-09-11 · `fix(飞书): 修复对话推荐卡实际发送`

- 根因：最终回复已被整理成结构化卡片，但入站投递桥从 OpenClaw 渠道注册表取得的对象不包含 `sendPayload`，所以真机只收到无内容兜底，无法看见按钮。
- 修复：版本锁定兼容桥改为加载同一官方飞书包实际导出的 `feishuChannelRuntime.feishuOutbound`；保留普通文字和回复位置逻辑，并支持从旧桥幂等升级。
- 真机结果：Dykes 私聊收到 5 个真实职位的原生卡片，每项均显示“接单并建群”；点击 `J1IINVY` 后无需二次确认，项目群和群内初始职位卡均已就绪，且未自动启动找人。
- 验证结果：结构化卡片、OpenClaw 插件与生产配置专项 35/35 通过；`npm run verify:quick` 16/16 通过。

## 2026-09-11 · `feat(画像): 自定义方向并刷新机器人岗位推荐`

- 改动：正式设置中心的“方向画像”新增关键词输入框和保存按钮，支持顿号、逗号或换行分隔，执行现有数量与长度边界；保存档案后立即生成新的冻结推荐轮次。
- 推荐一致性：精选盘刷新父页面快照；飞书机器人 `brainx_daily_brief` 继续读取最新完成轮次，因此后续对话推荐与工作台使用同一份画像结果。已有画像备注由后端保留，本入口不改写；结构化排除项和硬约束仍未开放。
- 验证结果：Agent 岗位工具 16/16、前端静态与适配 45/45、Storybook 交互 85/85、Storybook 生产构建通过；`npm run verify:quick` 16/16 通过。
...
- 验证结果：OpenClaw 插件与推荐卡专项测试 16/16 通过；`npm run verify:quick` 16/16 通过。提交后重装本地插件，并在 Dykes 私聊重发 5 岗推荐。

## 2026-09-11｜fix(插件): 补齐候选表格运行模块打包

- 真机根因：本机重装 `1.3.12` 后 OpenClaw 启动时报告缺少 `candidate-table.js`；源码引用正确，但 npm `files` 白名单遗漏该运行模块，自动测试直接从源码执行所以没有提前暴露。
- 修复：把候选表格模块纳入发布包，插件升至 `1.3.13`；新增发布文件闭包回归，逐个核对已打包 JavaScript 的本地 import 目标也在 npm 白名单中。
- 验证：插件专项、实际 `npm pack` 内容、OpenClaw 本机重装和真实启动结果见本次提交后的新鲜证据。

## 2026-09-11｜feat(群接管): 新群自动进入职位绑定流程

- 改动：业务 worker 分页轮询机器人所在群；首次启用只建立历史群基线，之后的新群幂等发送“绑定现有群”卡。已开通顾问可在卡内提交本人已承接/共享的项目编号，成功后当前群升级为完整项目 scope 并自动投放项目职位卡。
- 安全：待绑定群只开放 `brainx_bind_group_project`；工具不接收 `chat_id`，只能使用可信当前群上下文。无 ACTIVE 身份、无权职位、跨群重复绑定和其他工具均失败关闭；受管飞书表单桥 V2 仅透传 `criteria` 与 `job_id`。
- 版本与状态：BrainX OpenClaw 插件升至 `1.3.12`；正式代码已接入，目标环境未发布，真实 `im:chat` 权限、轮询发现、桌面端/手机端表单仍待真机验收。
- 验证：群列表分页、首轮基线、新群单卡、待绑定最小授权、绑定后权限升级与项目卡、插件/安装器/worker 专项 33/33 通过；快速门禁 16/16 通过。完整门禁在提交后重新执行。

## 2026-09-11｜fix(飞书): 打通项目卡找人条件回传

- 根因：飞书 `card.action.trigger` 会返回 `action.form_value`，但锁定版 `@openclaw/feishu@2026.7.1` 在事件归一化时丢弃该字段，原卡片又没有把输入框与按钮放进同一个 `form`，所以界面看似可输入、Agent 实际拿不到条件。
- 修复：项目卡改用原生 `form` + `form_submit`；安装器对固定插件版本和固定源码形状应用幂等窄兼容桥，只为显式 `brainx_form=true` 的动作提取最多 2000 字 `criteria`，以结构化标记进入 Agent 上下文。普通消息和其他卡片不变；版本或源码漂移时安装失败关闭，不增加第二条 WebSocket。
- 版本与状态：BrainX OpenClaw 插件升至 `1.3.11`；正式代码和部署流程已接入，目标环境未发布，真实飞书桌面端/手机端回调仍待验收。
- 验证：卡片结构、字段边界、兼容桥幂等、上游漂移失败关闭和真实 `@openclaw/feishu@2026.7.1` npm 包补丁/复核通过；项目卡、插件专项 18/18，快速门禁 16/16 通过。

## 2026-09-11｜feat(接单): 统一推荐卡与工作台接单建群

- 改动：自然语言职位推荐卡逐职位增加“接单并建群”；生产 Agent 接单工具与工作台接单统一复用项目群工作流，成功后回群选择 OpenMai、Reloop 或 SuperMai，不再在接单阶段自动消耗 OpenMai。
- 边界：按钮点击是本次明确确认；建群失败保留已完成的接单并返回明确错误；Agent 只拿到脱敏群就绪状态，不返回 `chat_id`。
- 验证：按钮解析、接单幂等、不自动找人、建群成功/部分失败、项目群卡片和项目启动专项测试通过；快速门禁在提交前执行。
