...
const nav = [
 ["today","工作台",Sparkles],["jobs","职位市场",BriefcaseBusiness],["clients","人才库",Users],
...
export default function DecisionWorkbench(){
 ...
 const feedbackJob=async(job:DecisionJob,reason?:string)=>{const clean=reason?.trim().slice(0,200)||"快速划过";setInlineFeedbackJobId(job.id);try{if(brainxMode==="connected"){const key=reason?.trim()?`recommendation-feedback-reason:${job.id}:${clean}`:`recommendation-feedback:${job.id}`;await sendRecommendationFeedback(job.id,clean,brainxRun.snapshotId,makeIdempotencyKey(key))}}catch(error){setInlineFeedbackJobId(current=>current===job.id?null:current);notify(`反馈失败：${error instanceof Error?error.message:"后端未响应"}`)}};
...
function OpportunityRow({job,completed,engagement,open,onAction,onFeedback,feedbackJobId,onUndoFeedback,inTray,onToggleTray,folderMode,folders,onAssignFolder}:{job:DecisionJob;completed:string[];engagement:EngagementState;open:(job:DecisionJob,tab?:"judgement"|"engagement"|"trail"|"replay")=>void;onAction:(job:DecisionJob,action:DecisionAction)=>void;onFeedback:(job:DecisionJob,reason?:string)=>void;feedbackJobId:string|null;onUndoFeedback:(job:DecisionJob)=>void;inTray:boolean;onToggleTray:(id:string)=>void;folderMode:boolean;folders:PickFolder[];onAssignFolder:(jobId:string,folderId:string)=>void}){
 ...
 useEffect(()=>{if(feedbackJobId!==job.id){setSelectedFeedbackReason("");setCustomFeedbackReason("")}},[feedbackJobId,job.id]);
 const selectFeedbackReason=(reason:string)=>{setSelectedFeedbackReason(reason);setCustomFeedbackReason("");void onFeedback(job,reason)};
 const submitCustomFeedback=()=>{const reason=customFeedbackReason.trim();if(!reason)return;setSelectedFeedbackReason(reason);void onFeedback(job,reason)};
 return <article className={`pick-card concept-job-card${inTray?" in-tray":""}${feedbackJobId===job.id?" feedback-active":""}`} onClick={()=>{if(feedbackJobId!==job.id)open(job)}}>
  ...
  {feedbackJobId===job.id&&<div className="pick-card-hide-feedback" aria-label="不感兴趣操作" onClick={event=>event.stopPropagation()}><div className="pick-card-feedback-reasons" role="group" aria-label="快捷反馈原因">{quickFeedbackReasons.map(reason=><button key={reason} type="button" className={selectedFeedbackReason===reason?"selected":""} aria-pressed={selectedFeedbackReason===reason} onClick={()=>selectFeedbackReason(reason)}>{reason}</button>)}</div><div className="pick-card-feedback-custom"><input value={customFeedbackReason} onChange={event=>setCustomFeedbackReason(event.target.value)} onKeyDown={event=>{if(event.key==="Enter"){event.preventDefault();submitCustomFeedback()}}} placeholder="其他原因，直接输入" aria-label="其他不感兴趣原因" maxLength={200}/><button type="button" disabled={!customFeedbackReason.trim()} onClick={submitCustomFeedback}>提交</button></div><div className="pick-card-feedback-actions"><Check/><button type="button" onClick={()=>onUndoFeedback(job)}>撤销</button></div></div>}
 </article>
}
