...
export function ClientInsightsReview({ clients, onOpenJobs }: ClientInsightsReviewProps) {
  ...
  const [rowLimit, setRowLimit] = useState(CLIENT_BATCH_SIZE);
  ...
  const rendered = visible.slice(0, rowLimit);
  ...
  return <div className="client-insights-review">
    ...
    <section className="client-table-card">
      <div className="client-toolbar">
        <label><Search /><span className="sr-only">搜索客户</span><input value={query} onChange={event => { setQuery(event.target.value); setRowLimit(CLIENT_BATCH_SIZE); }} placeholder="搜索客户或已知类型" /></label>
        <label className="client-check"><input type="checkbox" checked={onlyActive} onChange={event => { setOnlyActive(event.target.checked); setRowLimit(CLIENT_BATCH_SIZE); }} />仅看有活跃职位</label>
        <label className="client-sort"><ArrowDownUp /><span className="sr-only">客户排序</span><select value={sort} onChange={event => { setSort(event.target.value as ClientSort); setRowLimit(CLIENT_BATCH_SIZE); }}><option value="active">活跃职位优先</option><option value="hc">已知 HC 优先</option><option value="recent">最近快照优先</option></select></label>
      </div>
