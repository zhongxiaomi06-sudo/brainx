...
import { BACKEND_WEIGHTS, FALLBACK_DISMISS_REASONS, brainxFetch, connectSSE, fetchJobDetail, getSnapshot, makeIdempotencyKey, mapReplayData, mapRadarRow, mapClientRow, getRadar, getClients, getTalentSupply, updateOpportunityFacts, updateOpportunityMembership, sendRecommendationFeedback, streamAssistant, rerunOpenmai, BrainxApiError, type TalentSupplySnapshot, type AssistantMessage, type BackendConsultants, type BackendEngagementResponse, type BackendOutcomeResponse, type BackendProfileUpdate, type BackendRecommendationRun, type BackendReplay, type BackendSessionStatus, type BrainxReplay, type BrainxSnapshot, type ManualFactField, type OpenmaiResult, type RadarJob, type RadarClient } from "./brainx-api";
...
export default function DecisionWorkbench(){
 ...
 const activeDecisionJobs=useMemo(()=>(brainxJobs||decisionJobs).map(job=>{const relation=membershipRelations[job.id];if(!relation)return job;return {...job,facts:{...job.facts,"职位关系":relation==="MY_JOB"?"我的职位":"团队共享"},eligibility:"ELIGIBLE" as const}}),[brainxJobs,membershipRelations]);
...
function EngagementPanel({job,state,outcomes,mode,legalMap,onCommand,onMembership,onUpdated,notify}:{job:DecisionJob;state:EngagementState;outcomes:Outcome[];mode:"connecting"|"connected"|"offline";legalMap:Record<string,EngagementCommand[]>;onCommand:(job:DecisionJob,command:EngagementCommand)=>void;onMembership:(job:DecisionJob,relation:MembershipRelation)=>Promise<void>;onUpdated:()=>Promise<void>;notify:(text:string)=>void}){
 const actions=mode==="connected"?legalMap[job.id]||[]:legalActions(job,state).filter(action=>action!=="COMPLETE"&&action!=="RELEASE");
 return <CommitmentLoopPanel job={job} state={state} outcomes={outcomes} mode={mode} legal={actions} onCommand={command=>onCommand(job,command)} onMembership={relation=>onMembership(job,relation)} onUpdated={onUpdated} notify={notify}/>;
}
