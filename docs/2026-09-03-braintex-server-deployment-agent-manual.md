...
当前第一主链必须在飞书后台确认 `im:chat` 已包含创建私有群能力（后台若拆分展示，则批准对应的 `im:chat:create`）并发布新版本，否则“在飞书启动寻访”一定失败。仍不申请文档、云盘、知识库、多维表格写入或群全量消息权限。机器人入群不等于获得 BrainX 数据授权；项目群必须 @ 机器人，只处理同租户、同 App 且已绑定的白名单 sender。
