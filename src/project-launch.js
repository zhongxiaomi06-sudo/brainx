...
export function buildProjectLaunchCard(job, { publicBaseUrl, state = null } = {}) {
  ...
  const criteriaCommand = `[BRAINTEX_SEARCH_START] 为项目 ${projectRef} 使用 OpenMai 找人。优先使用卡片输入框里顾问填写的条件；如果你没有拿到输入值，就读取本群最近一条由顾问明确发送的“找人条件：”；两者都没有则只根据职位事实找人。现在直接调用 brainx_openmai_search，不要再次询问找人方式。任务返回 running/triggered 后立即回复“正在找人，完成后候选人会自动发到本群”并结束本轮，不要原地轮询。`;
