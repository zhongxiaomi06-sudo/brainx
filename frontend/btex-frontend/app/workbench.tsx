...
import { BACKEND_WEIGHTS, FALLBACK_DISMISS_REASONS, brainxFetch, connectSSE, fetchJobDetail, getSnapshot, makeIdempotencyKey, mapReplayData, mapRadarRow, mapClientRow, getRadar, getClients, getTalentSupply, updateOpportunityFacts, sendRecommendationFeedback, undoRecommendationFeedback, streamAssistant, rerunOpenmai, BrainxApiError, type TalentSupplySnapshot, type AssistantMessage, type BackendConsultants, type BackendEngagementResponse, type BackendOutcomeResponse, type BackendProfileUpdate, type BackendRecommendationRun, type BackendReplay, type BackendSessionStatus, type BrainxReplay, type BrainxSnapshot, type ManualFactField, type OpenmaiResult, type RadarJob, type RadarClient } from "./brainx-api";
...
export default function DecisionWorkbench(){
 ...
 useEffect(()=>{if(!hydrated)return;if(brainxMode==="connected"){localStorage.setItem("decision-workbench",JSON.stringify({tray,folders,folderMode,weights,decisionActions,sidebarWidth}));return}localStorage.setItem("decision-workbench",JSON.stringify({done,snoozed,extraTasks,weights,decisionActions,sidebarWidth,tray,folders,folderMode,engagement,events:decisionEvents,outcomes,sync,auth,notifications}))},[hydrated,brainxMode,done,snoozed,extraTasks,weights,decisionActions,sidebarWidth,tray,folders,folderMode,engagement,decisionEvents,outcomes,sync,auth,notifications]);
