...
test('migrations：schema_migrations 逐文件记账，重开不重跑', () => {
  ...
  assert.deepEqual(rows, ['0001_init.sql', '0002_push_log.sql', '0003_consultants.sql',
                          ...
                          '0025_entity_links.sql', '0026_cases.sql',
                          '0027_event_dlq.sql', '0028_processed_events_pk_fix.sql',
                          '0029_chat_contexts.sql', '0030_lark_messages.sql',
                          '0031_job_facts_drafts.sql']);
});
...
test('migrations：旧库 user_version=2 兼容——前 2 个文件标记已应用，只补跑新增', () => {
  ...
  assert.equal(rows.length, 33); // 全部记账（含 Step 0/1 事件、实体、Case、DLQ、群消息与草稿表）
