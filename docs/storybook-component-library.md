...
`RecommendationQueueV2Review` 已按产品架构形成 Storybook 审核组件：初始摘要只保留“推荐队列”、岗位数量和更新时间，不暴露运行编号、策略版本或重复说明；卡片采用紧凑横向层级展示决策等级、身份、六项职位事实、最多两条可追溯理由、一条风险、规则化可信度和独立动作，不再以综合分、探索值或证据覆盖百分比作为主信息。队列固定每页20条，覆盖首批、下一页、5条末页、新运行提示、关键事实缺失、数据过期、分页失败、动作成功/失败和窄屏场景。该组件只接收目标契约数据，不连接生产 API，也未替换正式 `TodayDecisionQueue`；现有后端尚缺运行内总数、正式决策层级、可信度规则和统一最近活动，因此用户再次视觉确认和后端契约补齐前不得接入正式页面。字段顺序和交互规则见[推荐队列与职位决策产品架构](recommendation-queue-product-architecture.md)，状态证据见[紧凑布局复审记录](frontend-reviews/2026-08-28-recommendation-queue-v2-density.md)。
