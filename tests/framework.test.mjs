...
test('migrations：schema_migrations 逐文件记账，重开不重跑', () => {
  ...
  assert.deepEqual(rows, ['0001_init.sql', '0002_push_log.sql', '0003_consultants.sql',
                          ...
                          '0036_consultant_model_profiles.sql', '0037_supermai_credentials.sql',
                          '0038_job_facts_drafts_p2p.sql', '0039_project_launches.sql',
                          '0040_project_launch_search.sql', '0041_openmai_deliveries.sql',
                          '0042_project_launch_one_chat.sql', '0043_ttc_credential_grants.sql',
                          '0044_openmai_search_brief.sql', '0045_openmai_search_rounds.sql',
                          '0046_project_candidate_focus.sql', '0047_candidate_decision_groups.sql',
                          '0048_project_launch_openclaw_status.sql',
                          '0049_candidate_reports.sql',
                          '0049_group_scope_job_action.sql',
                          '0050_bot_chat_intake.sql',
                          '0051_recommendation_feature_snapshots.sql',
                          '0052_ranking_label_times.sql',
                          '0053_ranking_feedback_events.sql',
                          '0054_versioned_job_facts.sql',
                          '0055_profile_exposure_outcomes.sql',
                          '0056_agent_context_usage.sql',
                          '0057_agentic_ranking_v1.sql',
                          '0058_agentic_shadow_evaluation.sql',
                          '0059_operations_dashboard.sql',
                          '0060_supermai_desktop_relay.sql']);
});
...
test('migrations：旧库 user_version=2 兼容——前 2 个文件标记已应用，只补跑新增', () => {
  ...
  assert.equal(rows.length, 63); // 全部迁移文件记账（0001...0060，含 0017/0049 重复编号）
