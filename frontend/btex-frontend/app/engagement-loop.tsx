...
export function CommitmentLoopPanel({job,state,outcomes,mode,legal,onCommand,onMembership,onVerify,onUpdated,notify}:{job:JobRef;state:EngagementState;outcomes:Outcome[];mode:Mode;legal:EngagementCommand[];onCommand:(command:EngagementCommand)=>void;onMembership:(relation:MembershipRelation)=>Promise<void>;onVerify:()=>void;onUpdated:()=>Promise<void>;notify:(text:string)=>void}){
 ...
 return <div className="commitment-loop">
  ...
  {editor&&<section className="commitment-editor">
   ...
   {editor==="terminal"&&<div className="commitment-fields"><label>终局结果<div className="commitment-terminal-options" role="group" aria-label="终局结果"><button type="button" className={terminalStage==="入职"?"selected":""} aria-pressed={terminalStage==="入职"} onClick={()=>setTerminalStage("入职")}>入职</button><button type="button" className={terminalStage==="关闭"?"selected":""} aria-pressed={terminalStage==="关闭"} onClick={()=>setTerminalStage("关闭")}>关闭</button></div></label>{terminalStage==="关闭"&&<label>关闭原因<select value={closeReason} onChange={e=>setCloseReason(e.target.value)}>{closeReasons.map(v=><option key={v}>{v}</option>)}</select></label>}<label>结果摘要<textarea value={summary} onChange={e=>setSummary(e.target.value)} placeholder="记录最终发生了什么，以及必要的上下文"/></label><button className="primary" disabled={busy} onClick={submitTerminal}>{busy?"提交中…":"确认终局并完成承接"}</button></div>}
