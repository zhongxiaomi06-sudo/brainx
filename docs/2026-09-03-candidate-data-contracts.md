...
`candidate_decision_groups` 按租户、项目和候选编号唯一保存来源群、目标群、脱敏摘要及可恢复创建状态。只有来源项目群中的已授权发送人，才能为已进入 `project_candidate_focus` 的候选人执行 `CREATE_DECISION_GROUP`。

- 摘要以重点候选安全快照为底座，只从来源群最近消息中选择明确含候选编号或姓名的内容；最多保留 5 条并清除电话、邮箱。
- 目标群继续绑定原 `position_id`，但群用途缩小为候选核对、候选动作和面试准备；群成员来自同项目且已完成当前飞书应用身份绑定的协作者。
- 原始简历、联系方式、其他候选讨论及完整原群历史不进入目标群或新表。
- 飞书建群、OpenClaw 准入或摘要投放失败时保存阶段并允许重试；相同候选重复点击不得创建第二个群。
