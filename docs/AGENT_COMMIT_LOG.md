...
- **范围裁决**：003 复用已完成的 Step 0/1、规则提炼与确认能力，不重写既有模块；LLM 提炼质量、OpenMai、外部群披露与约面/一面建模明确排除，待真实消息形成 gold set 后另立后续规格。
- **Spec Kit 状态**：新增 `.specify/feature.json` 激活 003；建立 `checklists/requirements.md`，17 项规格质量检查全部通过，无 `[NEEDS CLARIFICATION]`，可以进入 `$speckit-plan`。
- **验收口径**：核心包含自动触发、1000 次重放无重复、越权默认拒绝、确认/拒绝终态、失败可查可重试，以及 AI 开关关闭时仍可演示完整基础闭环。
- **验证**：`npm run verify:quick` 16/16 通过；`git diff --check` 通过；Spec Kit 前置检查准确提示下一阶段需生成 `plan.md`。
