...
候选表格每行同时提供“□ 保留”。按钮点击即是本次明确确认，Agent 通过 `brainx_candidate_workflow` 的 `KEEP_FOR_REVIEW` 把候选人写入项目共享重点名单；后续项目群问答从 `brainx_candidate_shortlist.focused_candidates` 读取，不依赖易丢失的模型会话记忆。明确取消时使用 `REMOVE_FROM_REVIEW`。
