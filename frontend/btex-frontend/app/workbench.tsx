...
function DecisionToday({activeJobId,completed,jobs,engagement,sync,open,onAction,onCommand,onFeedback,showVerification=true,tray,onToggleTray,onRemoveTray,onConfirmTray,folders,folderMode,onFolderMode,onAssignFolder,onCreateFolder}:{activeJobId:string|null;completed:string[];jobs:DecisionJob[];engagement:Record<string,EngagementState>;sync:SyncStatus;open:(job:DecisionJob,tab?:"judgement"|"engagement"|"trail"|"replay")=>void;onAction:(job:DecisionJob,action:DecisionAction)=>void;onCommand:(job:DecisionJob,command:EngagementCommand)=>void;onFeedback:(job:DecisionJob,reason?:string)=>void;showVerification?:boolean;tray:string[];onToggleTray:(id:string)=>void;onRemoveTray:(id:string)=>void;onConfirmTray:()=>void;folders:PickFolder[];folderMode:boolean;onFolderMode:()=>void;onAssignFolder:(jobId:string,folderId:string)=>void;onCreateFolder:(name:string)=>void}){
 ...
 const isContext=activeJobId!==null&&pendingShown.some(job=>job.id===activeJobId);
  ...
  {sync.state==="INCOMPLETE"||sync.state==="ERROR"?<section className="decision-blocked"><AlertTriangle/><div><b>{sync.state==="INCOMPLETE"?"本次同步不完整":"同步失败"}</b><p>为避免误导，当前不展示新的项目判断。</p></div><button className="btn" disabled={!jobs.length} onClick={()=>jobs[0]&&open(jobs[0],"judgement")}>查看上次快照</button></section>:<DecisionZone tone="pending" title="未接单" subtitle="" jobs={pendingShown} isContext={isContext} completed={completed} engagement={engagement} open={open} onAction={onAction} onCommand={onCommand} onFeedback={onFeedback} tray={tray} onToggleTray={onToggleTray} folderMode={folderMode} folders={folders} onAssignFolder={onAssignFolder}/>}
  </div>
}
...
function DecisionCard({job,completed,engagement,open,onAction,onCommand,onFeedback,inTray,onToggleTray,folderMode,folders,onAssignFolder}:{job:DecisionJob;completed:string[];engagement:EngagementState;open:(job:DecisionJob,tab?:"judgement"|"engagement"|"trail"|"replay")=>void;onAction:(job:DecisionJob,action:DecisionAction)=>void;onCommand:(job:DecisionJob,command:EngagementCommand)=>void;onFeedback:(job:DecisionJob,reason?:string)=>void;inTray:boolean;onToggleTray:(id:string)=>void;folderMode:boolean;folders:PickFolder[];onAssignFolder:(jobId:string,folderId:string)=>void}){
 ...
 const actionComplete=!!action&&completed.includes(`${job.id}:${action.id}`);const isAccepted=engagement==="ACCEPTED";const canAccept=legalActions(job,engagement).includes("ACCEPT");const acceptHint=isAccepted?"已接单，进入交付列表":canAccept?"直接接单并自动找人":"需先确认项目归属或补齐关键事实后才能接单";
 return <article className={`pick-card${inTray?" in-tray":""}`} onClick={()=>open(job)}>
  ...
  <div className="pick-card-foot"><small>{job.recentSignal}</small><span className="pick-card-actions"><button className={`pick-card-accept${isAccepted?" accepted":""}`} onClick={e=>{e.stopPropagation();if(canAccept)onCommand(job,"ACCEPT")}} disabled={!canAccept} title={acceptHint}>{isAccepted?<><Check/>已接单</>:<><CheckCircle2/>接单</>}</button><button className={`pick-card-action${actionComplete?" complete":""}`} onClick={e=>{e.stopPropagation();if(action)onAction(job,action)}} disabled={!action||actionComplete}>{action?(actionComplete?"已记录":action.label):"暂无可用动作"}{action&&<ChevronRight/>}</button></span></div>
 </article>
}
