...
- [ ] T001 固定 OpenClaw 2026.7.1-2 插件契约夹具与禁止工具基线到 tests/fixtures/openclaw-production/
...
- [ ] T011 [P] [US1] 先写 HTTP 方法、body cap、服务 token、assertion、未知工具和健康检查契约测试到 tests/agent-gateway-http.test.mjs
- [ ] T012 [US1] 实现唯一生产工具注册表与回环 HTTP Gateway 到 src/agent-gateway/tool-registry.js、src/agent-gateway/server.js、bin/brainx-agent-gateway.mjs
- [ ] T013 [P] [US1] 先写绑定/撤销顾问和登记/撤销群 scope 的管理员命令测试到 tests/agent-admin.test.mjs
- [ ] T014 [US1] 实现显式管理员身份与群范围命令到 src/agent-gateway/admin.js、bin/brainx-agent-admin.mjs
