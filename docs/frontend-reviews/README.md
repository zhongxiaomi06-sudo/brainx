...
| `TodayDecisionQueue` 今日决策队列 | 已完成 | 部分通过 | 已接入 | 待确认 | 已验证（本地真实库副本） | 正式入口已复用 V2 卡片、20条分页和完整队列搜索；用户退回原搜索后已完成本地修复与真实数据复核，修复后的搜索体验待用户复看，目标环境 OAuth 验证仍待补齐 |
| `RecommendationQueueV2` 完整推荐队列与卡片 | 已完成 | 部分通过 | 已接入 | 未发布 | 已验证（本地真实库副本） | 本地正式入口已接通最多200条冻结结果、20条游标分页、完整队列搜索及版本化展示契约；真实库跨页搜索与零结果已验证，修复后的搜索体验待用户复看，目标环境尚未发布 |
