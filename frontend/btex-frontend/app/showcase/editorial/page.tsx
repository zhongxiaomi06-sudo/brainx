...
export default function BtexEditorialShowcase() {
  ...
  return <main className="editorial-showcase" id="top">
    ...
    <section className="editorial-section editorial-about" id="about"><p className="section-index">01 / WHAT IT DOES</p><div className="about-layout"><h2>先判断事实，<br /><em>再安排注意力。</em></h2><p className="about-copy">B‑tex 不把所有职位塞进一个排行榜。它先收口关闭、入职、HC 与归属事实，再把真正值得推进、需要核验和暂不推荐的机会放进不同工作区。</p><article><span>事实优先</span><p>UNKNOWN 不会被写成 0；没有加入项目，就不会伪装成可直接跟进。</p></article><article><span>行动优先</span><p>分数只作为判断线索，职位行首先告诉你：现在具体该做什么。</p></article></div></section>
    <section className="editorial-section editorial-principle" id="logic"><p className="section-index">02 / THE LOGIC</p><div><h2>从一条信号，<br />到一次<span>可解释的行动。</span></h2><p>不用在表格、项目和历史记录间来回寻找。每个职位都有一张当下判断，也保留那一刻的快照。</p></div><div className="logic-steps"><article><b>01</b><i>↙</i><h3>先排除硬条件</h3><p>关闭、已入职、HC 为 0 与重复项目不进入正式推荐。</p></article><article><b>02</b><i>↗</i><h3>再组织可信机会</h3><p>推进、探索、个人适配和判断可靠度都能回到事实与证据。</p></article><article><b>03</b><i>→</i><h3>只给允许的下一步</h3><p>验证、关注、开始跟进或完成，由服务端规则决定，不由前端猜测。</p></article></div></section>
    ...
    <section className="editorial-section editorial-roadmap" id="scope"><p className="section-index">04 / BUILD WITH TRUST</p><h2>当前能做什么，<br /><em>就清楚展示什么。</em></h2><div><article><b>01</b><h3>今天的判断</h3><p>按方向组织正式 Top 3、需要确认和排除机会。</p></article><article><b>02</b><h3>跟进与回放</h3><p>演示关注、开始跟进、结果记录与冻结的历史判断。</p></article><article><b>03</b><h3>统一前端表面</h3><p>为后端同步、权限、通知与轨迹预留可替换的数据适配层。</p></article></div><p className="roadmap-note">这是前端可演示版本：交互与本地状态可用，真实接口、飞书授权和推送由后端后续接入。</p></section>
