...
- 素材已覆盖 14/14 个标准环节，但按“五维采全”口径完成度仍为 0%；只有明确标注“本人 2026-09-01 确认”的内容可当确认事实。
...
- 这是待本人确认的素材稿，不作为团队自动化规则。
- 对架构有价值的需求线索包括：公司→部门/业务线→候选人的 Mapping；同一人多份简历去重；区分“候选人本身强弱”和“岗位匹配度”；一人多岗对比；推荐和面试材料的证据包；高价值候选人的长期跟进。
- Offer 谈判和入职后维护仍是明确素材缺口；触达、推荐和漏斗数据也缺少完整量化证据。

因此，架构必须容纳这些信息，但在本人确认前只能标记为 `DRAFT` 或 `PENDING_VERIFICATION`，不能直接进入自动执行策略。
...
知识项分为 `DRAFT / CONFIRMED / PUBLISHED / RETIRED`。Felix/Miya 素材只有经过本人确认并发布后，才能参与自动话术或判断；素材稿可以用于界面提示，但必须显示来源和确认状态。
