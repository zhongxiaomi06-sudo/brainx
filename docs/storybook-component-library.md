...
`PickTray` 精选盘只用于收藏、移除和按文件夹整理职位，不再提供批量接单。Storybook 交互测试会校验批量“确定”按钮不存在；正式接单继续通过单个职位的合法动作完成，以便逐项记录目标、首个行动和期限。状态证据见[删除批量接单审核记录](frontend-reviews/2026-08-28-remove-batch-accept.md)。

`RecommendationQueueV2Review` 已按产品架构形成 Storybook 组件并部分接入正式“今日决策”：正式页面已按用户复审删除筛选区上方整块重复概览，只保留组件自身的“推荐队列”、岗位数量和更新时间摘要；卡片采用紧凑扫描结构，圆形两位排名与职位名称对齐，决策等级和信号格向内收并固定在右上视觉中线，六项事实压成统一事实带，最多两条可追溯理由与判断状态并排，三个动作固定在右下角。组件已删除右上角详情箭头和卡内“加入我的项目已记录”成功提示，整卡、Enter 与 hover 在正式入口生效；按钮区阻止冒泡，综合分、探索值和证据覆盖百分比不再作为主信息。Storybook 固定每页20条，覆盖首批、下一页、5条末页、新运行提示、关键事实缺失、数据过期、分页失败、动作失败、窄屏和正式入口复用场景。正式入口映射当前已加载的真实推荐事实和合法动作，但后端仍缺运行内总数、正式决策层级、可信度规则和统一最近活动，因此完整200条分页与全链路状态仍未完成。字段顺序和交互规则见[推荐队列与职位决策产品架构](recommendation-queue-product-architecture.md)，状态证据见[删除概览审核记录](frontend-reviews/2026-08-28-remove-today-overview.md)和[视觉重做复审记录](frontend-reviews/2026-08-28-recommendation-queue-v2-visual-reset.md)。
