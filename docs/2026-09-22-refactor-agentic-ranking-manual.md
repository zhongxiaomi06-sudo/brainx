...
- [ ] 实现资格过滤 → 多路召回 → 信息覆盖清单 → Agent 补查/比较 → Agent 最终列表 → 校验 → 原子发布。
  - 进度：实现已完成，服务端先处理可见性、完整快照、在招、HC、排除、忽略、已承接和容量，再合并明确方向、历史相似、近期需求、供给和探索通道；冻结覆盖清单后把五个只读工具交给 Agent。待本阶段完整门禁后勾选。
- [ ] 按[算法契约](2026-09-22-algorithm-a-contract.md)记录版本、预算、修复与弃权；首版完成 20 条以内即可，不用规则补齐 200 条。
  - 进度：独立 A 运行账本冻结画像、信号、负载、权限、职位事实、候选集、证据、模型、prompt、工具、策略和预算版本；最多两次修复，合法弃权和不足 20 条均原样保存。待本阶段完整门禁后勾选。
- [ ] 发布临界区复核硬状态、版本与并发 generation；校验器不得另算总分或改变 Agent 顺序。
  - 进度：短事务按 generation/CAS 复核授权、画像、负载、来源、忽略、承接、职位状态和事实版本；校验器只判合法性，不评分、不重排、不补齐，A 表不进入正式基线读路径。待本阶段完整门禁后勾选。
