...
- [x] 扩展 `agent/loop.js`、`llm.js` 的每轮记录，覆盖成功、失败、重试、取消、缓存命中；预算包含最后收尾和修复调用。
  - 进度：Agent loop 在每次模型请求前建账并记录终态，多轮/重试统一聚合；未知用量不补零，缓存与 reasoning 保留子集口径，强制收尾也消耗同一预算。
- [x] 上下文注册表记录来源、版本、授权、摘要、有效期和截断；群/文档映射先盘点再迁出硬编码。
  - 进度：已增加 Context Registry，保存来源版本、授权范围哈希、摘要、内容哈希、大小、有效期和截断；每次读取重验租户/作用域/过期/撤权，run 只关联引用。
- [x] 为 A 注册专用只读窄工具，不能直接复用拥有写能力的完整聊天工具集。
  - 进度：已为 A 建立严格五工具注册表，身份由服务端闭包注入，拒绝任意工具名、跨身份参数和不可见职位，并限制超时、取消与结果大小。实现提交 `d51b02f` 完整门禁 25/25 通过，后端 841/841、前端 53/53、Storybook 86/86、卡片 18/18 通过，Push 条件满足。
...
- [x] 实现资格过滤 → 多路召回 → 信息覆盖清单 → Agent 补查/比较 → Agent 最终列表 → 校验 → 原子发布。
  - 进度：服务端先处理可见性、完整快照、在招、HC、排除、忽略、已承接和容量，再合并明确方向、历史相似、近期需求、供给和探索通道；冻结覆盖清单后把五个只读工具交给 Agent。
- [x] 按[算法契约](2026-09-22-algorithm-a-contract.md)记录版本、预算、修复与弃权；首版完成 20 条以内即可，不用规则补齐 200 条。
  - 进度：独立 A 运行账本冻结画像、信号、负载、权限、职位事实、候选集、证据、模型、prompt、工具、策略和预算版本；最多两次修复，合法弃权和不足 20 条均原样保存。
- [x] 发布临界区复核硬状态、版本与并发 generation；校验器不得另算总分或改变 Agent 顺序。
  - 进度：短事务按 generation/CAS 复核授权、画像、负载、来源、忽略、承接、职位状态和事实版本；校验器只判合法性，不评分、不重排、不补齐，A 表不进入正式基线读路径。实现提交 `e152543` 完整门禁复跑 25/25 通过，后端 853/853、前端 53/53、Storybook 86/86、卡片 18/18 通过，Push 条件满足。
- **验收**：没有 A 最终输出不能产生 A 推荐；有效输出顺序逐项一致；越权、缺证据、重复、超时、并发失效都不发布；合法不足量如实返回。
- **回滚**：关闭新运行/发布开关，保留已保存且重新校验有效的旧 A 结果，否则明确不可用。

### 08｜离线、影子与效果证据

- [x] 用相同授权、时点和候选池比较旧基线与 A；另做全链路召回覆盖评估，区分召回漏掉与 Agent 选错。
  - 进度：冻结计划必须预登记样本、覆盖、非劣、成本、延迟、稳定性和分组差异门槛；基线仅在 A 的冻结候选池上执行旧评分，分别报告 judged universe retrieval recall、池内 selection recall、NDCG 与 Top-K 分歧。
- [x] 同一输入重复运行，测试候选顺序扰动、长文本、缺失画像、冷启动、来源冲突和恶意内容。
  - 进度：影子账本支持显式场景及参考运行，保存同 candidate-set 的名次不稳定度；七类场景均有自动回归，长文本和恶意内容不扩大工具或硬约束权限。
- [x] 影子不投递、不计真实曝光，不用影子推测结果冒充真实转化。
  - 进度：A run 新增 `LIVE/SHADOW` 隔离；影子运行前后核对正式 run、推荐、impression、真实曝光和 push 计数，任何变化都判硬违规。报告固定声明无真实业务结果和 propensity，满足离线门槛也最高只建议受控灰度。实现提交 `6d4dd9c` 完整门禁 25/25 通过，后端 864/864、前端 53/53、Storybook 86/86、卡片 18/18 通过，Push 条件满足。
- **验收**：硬约束违规为 0；效果、成本、延迟及分组偏差满足预先登记门槛；样本不足报告不确定性，不能全员发布。
- **回滚**：停止影子执行；保留评估快照、差异和模型版本。

### 09｜工作台与飞书忠实展示

- [x] 服务端返回 `engine/run_id/rank/reason/evidence/uncertainties/generated_at`，前端以 A 顺序展示，不使用旧 score 二次排序。
- [x] 旧六维权重控件不接 A；设置改为主动偏好、排除项和容量。旧分数仅在明确历史基线中显示。
- [x] 冻结分页、异步生成、旧结果标记、无结果、失败恢复、权限撤销和动作幂等进入 Storybook/卡片场景。
...
- **回滚**：恢复兼容读取/关闭 A 展示；旧接口不认识 A 时明确拒绝或路由到已标识的历史结果，不填假 score。
  - 进度：A 展示投影只读 `LIVE`，绑定冻结游标并在读取时重验职位与权限；Web、飞书、scheduler、CLI 和预览工具按原 rank 消费同一语义，理由、权衡、证据引用、待核实项和建议动作均来自 A，旧 score、概率和置信不补造。`BRAINX_AGENTIC_READ` 默认关闭。
  - 进度：设置中心按引擎区分 A 主动画像、公司/职位/城市排除和容量上限，旧六维明确为 `baseline-1.1` 历史设置；画像保存继续追加版本，A 忽略/撤销保持幂等。
  - 进度：Storybook 已覆盖原序、生成中、旧结果、撤权 rank 缺口、动作失败和窄屏；飞书 A 卡已登记独立视觉基线。用户审核、目标环境发布和真实 A 数据验证仍按[本轮审核记录](frontend-reviews/2026-09-24-agentic-ranking-presentation.md)保持未完成，不能被本地接入冒充。
  - 进度：实现提交 `cb49aff` 完整门禁 25/25 通过，后端 871/871、前端 54/54、Storybook 91/91、卡片 19/19 通过，Push 条件满足；本轮没有切生产读取、发送、发布或 push。

### 10｜运营、业务、效果与成本看板

- [x] 由事件消费者产生读模型，不扫描在线原始大表；投影包含 checkpoint、更新时间、口径版本和租户范围。
- [x] 展示同步新鲜度、失败/积压、容量增长、备份、真实漏斗、推荐分歧、成本/Token/延迟及样本成熟度。
- **验收**：抽样可从聚合回查事件；重复、乱序、更正和重建不会多算；影子与真实曝光分开；管理权限独立验证。
- **回滚**：暂停投影、重建读模型；不修改事实账本来“修平”看板。
  - 进度：`0059` 以窄触发器把同步、任务、A 运行/推荐项、用量、真实曝光、决策、业务结果、影子评估、职位版本、投递和备份证据归一为租户事件流；worker 按 checkpoint 维护实体态和快照，GET 不扫描在线业务表。
  - 进度：重复键幂等，迟到旧状态不覆盖新状态，更正替换原逻辑结果；显式重建只重放派生读模型。管理员可按受限指标追溯脱敏事件信封，普通顾问的读取、投影和追溯均独立拒绝。
  - 进度：设置中心管理员看板与 Storybook 加载/完整/积压/空/失败/窄屏六场景已接入；目标环境迁移、真实成本/漏斗、备份恢复和用户视觉审核仍按[本轮记录](frontend-reviews/2026-09-24-operations-dashboard.md)保持未完成。
  - 进度：实现提交 `edf006c` 与权限探测修复 `f913081` 完整门禁 25/25 通过，后端 875/875、前端 56/56、Storybook 97/97、卡片 19/19 通过，浏览器链路、生产构建、静态 Storybook 和服务烟雾均通过，Push 条件满足；本轮未执行目标环境迁移、发布或 push。
