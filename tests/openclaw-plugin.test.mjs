...
test('plugin package and manifest declare exactly the approved tools', () => {
  ...
  assert.equal(new Set(manifest.contracts.tools).size, 27);
...
test('BrainTex prompt routes natural-language job recommendations to authorized data', () => {
  ...
  assert.match(prompt, /登记下一次搜索的可选条件/);
  assert.match(prompt, /不要回复/);
  assert.match(prompt, /条件已由系统静默记录/);
  assert.match(prompt, /不要在这条消息上调用任何找人工具/);
  assert.match(prompt, /否则会与随后按钮形成重复付费任务/);
  assert.match(prompt, /criteria 已由插件按本群最近一次/);
  assert.match(prompt, /不要自行编造排除名单/);
...
test('tool request is fixed to loopback and produces a BrainX-verifiable assertion', async () => {
  ...
  assert.deepEqual(body.client, {
    plugin_version: '1.4.12', openclaw_version: '2026.7.1-2', model_ref: 'openai/gpt-5',
  });
