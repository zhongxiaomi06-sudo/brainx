...
> 日期：2026-09-22；版本：施工设计 v1；状态：手册已编制，下面的工程任务均待实施。
...
| 历史清理范围较窄 | `src/recommend.js` 的 `pruneRecommendationHistory` | 补字段报告、同步与限流记录生命周期，先盘点引用再删 |
...
- [ ] 修复 Python 排序索引与 JS IDCG 范围；核对 `evaluate({runs})` 使用局部参数而非全局常量；导出、影子与评估共享指标定义。
- [ ] 把历史特征读取改为冻结快照；标签加入观察窗口、数据截点、收到时间和样本成熟度。
- [ ] 接入忽略、撤销、未曝光、未成熟和未知；有互动不自动归因为本轮推荐贡献。
...
- [ ] `src/talent.js` 按环境显式选择持久层；生产缺凭据、断连、表未就绪均失败，不能返回写入成功；建表归迁移账号。
- [ ] 盘点 `ttc_field_reports`、`sync_runs`、限流跳过运行、推荐快照、原始上下文的体积、引用和保留责任。
...
- [ ] 先接 TTC 和一种独立的本地夹具/Excel CSV 来源，按[数据手册](2026-09-22-ranking-data-migration.md)落 source、映射、版本和字段证据。
- [ ] 对群消息/JD 复用 `job-extract` 的草稿确认链；内容哈希未变不重复调用模型，软推断不能提升为权限/有效性事实。
- [ ] 建立单写权威、影子投影、增量补偿与对账；保持 `project_id`、membership、群和 Case 引用稳定。
- **验收**：来源切换后，规范事实相同则下游输入等价；删除 TTC 字段依赖后测试仍可运行；分页中断不把缺页职位关停；冲突不静默覆盖。
- **回滚**：切回旧读路径并继续保存新版本与迁移游标；不 drop 新表或重建业务 ID。

### 05｜画像、曝光与结果闭环

- [ ] 用户主动画像版本、短期推断信号与模型凭据分开；负载由项目领域实时提供。
- [ ] 复用 `decision_runs/recommendations/recommendation_impressions/decision_events/job_outcomes`，增加缺失版本和关联；记录真实曝光与动作来源。
- [ ] 候选人、面试、Offer、Onboard 从权威事件接入，按 Case 去重、更正及归因；无可靠关联则标记未归因。
